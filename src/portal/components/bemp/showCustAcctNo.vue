  ...
  <h-msg-box v-model="tempShowCustAcctNoWin" width="1000" class="h-form-table-layer" :maximize="true"
    ...
    <div :class="{'h-form-overhd':!ifShowMore}">
      <h-form :model="formItem" :label-width="100" ref="formItem" cols="4" class="h-form-search" v-if="isSearch">
        <h-form-item label="客户账号" prop="custAcctNo">
  ...
  export default {
    ...
    watch: {
      showCustAcctNoWin(val) {
        if (val === true) {
          ...
          } else if (this.isAllOtherBankAcct) {
            // 只查他行账号
            this.formItem.isOtherBankAcct = "1";
          }
          ...
          this.formItem.createBrchNoOnly = this.createBrchNoOnly;
          this.formItem.createBrchNo = this.createBrchNo;
    ...
    methods: {
      ...
      doubleHandleClick(arr) {
        ...
        this.formItem.isOtherBankAcct = "0";
      },
      //查询
      formSearch() {
        this.currentSelectRow = [];
