...
<!--        </tr>-->
        ...
        <tr>
          ...
          <td>{{format("BanEndorsementMarkCode",billInfo.banEndrsmtMark)}}-{{format("IsAllowSplitBill",billInfo.isAllowSplitBill)}}</td>
          <td class="th">承兑人承诺</td>
          <td>
            <span v-if="billInfo.acptDt">本汇票已经承兑，到期无条件付款</span>
<!--            todo 签收前显示-->
            <span v-else>尚未完成承兑</span>
            <span class="h-ticket-dq" v-if="billInfo.acptDt">{{formatDate(billInfo.acptDt)}}</span>
          </td>
        </tr>
