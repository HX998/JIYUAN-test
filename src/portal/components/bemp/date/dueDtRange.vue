...
<template>
  <h-form-item :label="label" :prop="prop" :required="required" :class="className">
    <h-date-picker v-model="itemValue" :readonly="readonly" :placeholder="placeholder" :type="type" :format="format"
                   :disabled="disabled" :placement="placement" :options="options" :showFormat="showFormat" @on-change="onChange"
                   :clearable="clearable" :editable="editable" @on-clear="clearVal(clearValTag)"
                   :autoPlacement="autoPlacement" :transfer="transfer"></h-date-picker>
  </h-form-item>
</template>
  ...
  export default {
    ...
    props: {
      label: {
        type: String,
        default() {
          return this.$t("m.i.billInfo.dueDt");
        }
      },
      prop: {
        type: String,
