...
<script>
  ...
  export default {
    ...
    props : {
      ...
      mounted() {
        getDictListByGroups("DraftTypeCode,PayType,PayAcctTypeCode").then(res => {
      ...
      methods: {
        getBatchInfo() {
          //获取批次详细信息
          post({ id: this.batchId ,funcNo:this.funcNo}, this.queryBatchUrl).then(res => {
            if (res) {
              ...
              if (retCode === "000000") {
                ...
                if(this.viewDetailForm.payType === "2") {
                  this.viewDetailForm.buyPayPcet = "0";
                  this.viewDetailForm.salerPayPcet = "100";
                } else if(this.viewDetailForm.payType === "3") {
                  this.viewDetailForm.salerPayPcet = 100 - this.viewDetailForm.buyPayPcet;
