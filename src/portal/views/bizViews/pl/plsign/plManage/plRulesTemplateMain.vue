      ...
      <h-col span="24">
        <div>
          ...
          <h-datagrid :columns="columns"
                      highlight-row
                      url="/pl/plsign/plManage/plRulesTemplateMain/func_paginQueryCustRulesKpi"
                      :bindForm="formItem"
      ...
      <div>
        <h-form :model="addForm" :label-width="90" ref="addForm" cols="2" class="h-form-search">
          ...
          <h-form-item :label="$t('m.i.pl.kpiCodeArray')" prop="kpiCodeArray" required>
            <h-select v-model="addForm.kpiCodeArray" placeholder="" multiple @on-change="kpiCodeChange">
          ...
          <h-form-item :label="$t('m.i.billInfo.billMoney')" prop="billAmtArray" :required="selectedKpiCode.isShowBillAmt" v-if="selectedKpiCode.isShowBillAmt">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //获取指标名称
      getKpi() {
        post({ code: "1" }, "/pl/plsign/plManage/plRulesTemplateMain/func_queryRuleKpiDtos").then(res => {
          if (res && res.data) {
      ...
      addFormSubmit() {
        this.$refs["addForm"].validate(valid => {
          if (valid) {
            let url = this.optType === "add" ? "/pl/plsign/plManage/plRulesTemplateMain/func_addCustRulesKpi" : "/pl/plsign/plManage/plRulesTemplateMain/func_updateCustRulesKpi";
      ...
      handleDel() {
        post({id: this.currentSelectRow.id}, "/pl/plsign/plManage/plRulesTemplateMain/func_delCustRulesKpi").then(res => {
          if (res) {
            let retCode = res.data.retCode;
            if (retCode === "000000") {
              this.$msgTip.success(this);
