        ...
        <div>
          ...
          <h-datagrid
            ...
            <div slot="toolbar" class="pull-left">
              ...
              <h-button type="primary" @click="agreementOperate('renew')" :disabled="formItem.deleteFlag === '1'">
                {{$t("m.i.pl.continue")}}
              </h-button>
              <h-button type="primary" @click="cancelAgreement" :disabled="formItem.deleteFlag === '1'">{{$t("m.i.pl.incontinue")}}
              </h-button>
              <h-button type="primary" @click="freezeAgreement" :disabled="formItem.deleteFlag === '1'">{{$t("m.i.pl.freez")}}
              </h-button>
              <h-button type="primary" @click="percentRule" :disabled="formItem.deleteFlag === '1' || formItem.showPercentRule">{{$t("m.i.pl.setDis")}}
              </h-button>
              <!--<h-button type="primary" @click="exportExcel()">{{$t("m.i.common.exportExcel")}}
              </h-button>-->
            </div>
          </h-datagrid>
        </div>
    ...
    <h-msg-box v-model="addOrEditWin" width="800" :mask-closable="false" @on-close="handleWinClose"
      ...
      <div>
        <h-form :model="addOrEditForm" :label-width="115" ref="addOrEditForm" cols="2" class="h-form-search">
          <h-form-item :label="$t('m.i.pl.agreementNo')" prop="agreementNo" required v-if="optType!=='add'" key="agreementNo">
          ...
          <h-form-item prop="signEndDt" :label="$t('m.i.pl.signEndDt')" required>
            <h-date-picker type="date" v-model="addOrEditForm.signEndDt" placeholder="" showFormat
                           :editable=false :options="selectSignEndDt" :readonly="optType==='modify'"></h-date-picker>
          </h-form-item>
          <h-form-item prop="signFuncNo" :label="$t('m.i.pl.signFunc')" required key="signFuncNo">
            <h-select v-model="addOrEditForm.signFuncNo" placeholder="" :readonly="optType==='renew'" @on-change="signFuncNoChange" multiple>
          ...
          <h-form-item v-if="isMultiPool!=='1'"  prop="billBailAcctNo" :label="$t('m.i.pl.poolBillBailAcctNo')" :required="this.isContainInPool">
          ...
          <h-form-item v-if="isMultiPool!=='1'&& isSubAcctWorks==='1'" prop="poolBailSubAcctNo" :label="$t('m.i.pl.poolBailSubAcctNo')" key="poolBailSubAcctNo">
          ...
          <h-form-item :label="$t('m.i.pl.maxMoney')" prop="maxMoney" :required="isShowMaxMoney" v-if="isShowMaxMoney" key="maxMoney" :validRules="agreementMoneyRule">
            <h-row>
              <h-col :span="spanLeft">
                <h-typefield type="money" v-model="addOrEditForm.maxMoney" :maxlength="20" :readonly="optType==='renew'"
                             placeholder="数字，小数位最多2位" bigTips divided></h-typefield>
              </h-col>
              <h-col :span="spanRight" offset="1" v-if="isShowSyncButton">
                <h-button type="primary" @click="sysnCreditAmt()" :disabled="optType==='renew'">{{$t("m.i.common.synch")}}</h-button>
              </h-col>
            </h-row>
          </h-form-item>
          ...
          <h-form-item :label="$t('m.i.pl.secondAuthorizerIdcard')" prop="secondAuthorizerIdcard"  :validRules="idCardRule"
            ...
            <h-input v-model="addOrEditForm.secondAuthorizerIdcard" :maxlength="18" :readonly="optType==='renew'"></h-input>
          </h-form-item>
          ...
          <h-form-item :label="$t('m.i.pl.discountPcet')" prop="discountPcet" :required="addOrEditForm.saleOffMode==='1' || addOrEditForm.saleOffMode===''"
                       v-if="addOrEditForm.saleOffMode==='1' || addOrEditForm.saleOffMode===''" :validRules="discountPcetRule" key="discountPcet">
            <h-typefield v-model="addOrEditForm.discountPcet" placeholder="" :readonly="optType==='renew'" :integerNum="3" :suffixNum="2" nonNegative></h-typefield>
          </h-form-item>
          ...
          <h-form-item prop="isCollect" :label="$t('m.i.pl.isCollect')" :required="isShowCollect" v-if="isShowCollect" key="isCollect">
            <h-select v-model="addOrEditForm.isCollect" placeholder="" :readonly="optType==='renew'">
              <h-option v-for="item in YonList" :value="item.key" :key="item.key">{{item.value }}</h-option>
            </h-select>
          </h-form-item>
          <h-form-item :label="$t('m.i.pl.bailPcet')" prop="bailPcet" :validRules="discountPcetRule">
            <h-typefield v-model="addOrEditForm.bailPcet" placeholder="" :readonly="optType==='renew'" :integerNum="3" :suffixNum="2" nonNegative></h-typefield>
          </h-form-item>
          <h-form-item v-if="isResidualShow==='1'" :label="$t('m.i.pl.residual')" prop="residual" :validRules="residualRule">
            <h-input v-model="addOrEditForm.residual" :maxlength="6" :readonly="optType==='renew'"></h-input>
          </h-form-item>
        </h-form>
        <h-form v-if="isMultiPool==='1'" :model="signPoolInfoListForm" :label-width="115" ref="signPoolInfoListForm"
...
<script>
  import {accDiv, accMul, formatNumber, getSingleParamValuesByKeys, post, exportList} from "@/api/bizApi/commonUtil";
  ...
  export default {
    ...
    methods: {
      ...
      exportList(field, exportType) {
        ...
        let url = window.LOCAL_CONFIG.API_HOME + "/pl/plsign/billAgreementMain/func_poolSignInfoExport";
        let options = {};
      ...
      custCorpSelect(info) {
        ...
        if (this.custType === "query") {
          ...
          if (this.isMultiPool === '1') {
            let url = "/pl/plsign/plManage/plInfoMain/func_queryPoolInfo4MultiPool";
            post({
              legalNo:JSON.parse(
              ...
              if (res) {
                ...
                if (retCode === "000000") {
                  ...
                  if (data.length === 0) {
                    this.$msgTip.info(this, {info: "该客户的池信息未设置，请先维护！"});
                    return;
                  }
                  this.signPoolInfoListForm.signPoolInfoList = [];
                  for (let dataKey in data) {
                    this.signPoolInfoListForm.signPoolInfoList.push({poolInfoId: data[dataKey].id,poolNo: data[dataKey].poolNo,poolName: data[dataKey].poolName});
                  }
                } else {
            ...
            post({ id: this.currentSelectRow.id }, "/pl/plsign/billAgreementMain/func_queryBillAgreement").then(res => {
              if (res) {
                ...
                if (retCode === "000000") {
                  ...
                  this.signPoolInfoListForm.signPoolInfoList = res.data.retData.signPoolInfoList;
                } else {
                  this.$msgTip.error(this,{info:res.data.retMsg});
                }
              } else {
      ...
      addFormReset() {
        ...
        this.addOrEditForm.signFuncNo = [];
        // this.$refs.signPoolInfoListForm.resetFields();
        this.signPoolInfoListForm.signPoolInfoList = [];
      },
      submitForm() {
    ...
    mounted() {
      this.getDictListByGroups("SignFuncNo,SaleOffMode,Yon,FreezedFuncNo,CDMedia,DraftTypeCode,bankLevel,bankType,CorpScale,ModeType,custLevel").then(res => {
      ...
      getSingleParamValuesByKeys("bm.cust.is_sub_acct_works,pl.plsign.is_residual,pl.plsign.is_multi_pool").then(res => {
        this.isSubAcctWorks = res["bm.cust.is_sub_acct_works"];
        this.isResidualShow = res["pl.plsign.is_residual"];  // 是否显示期限错配天数
