    ...
    <div>
      <div :class="{'h-form-overhd':!ifShowMore}">
        <h-form :model="ioPoolPickBillForm" :label-width="100" ref="ioPoolPickBillForm" cols="4"
                class="h-form-search">
          <bill-no :label="$t('m.i.billInfo.billNo')"  v-model="ioPoolPickBillForm.likeBillNo" prop="likeBillNo"></bill-no>
          <h-form-item :label="$t('m.i.billInfo.billMoney')" prop="billMoney" v-if="ioPoolPickBillParams.pickBillType==='outPool'">
            <h-typefield type="money" v-model="ioPoolPickBillForm.billMoney" :maxlength="20"
                         placeholder="" bigTips divided></h-typefield>
          </h-form-item>
          <h-form-item class="h-form-operate" :class="ioPoolPickBillParams.pickBillType==='outPool' ? 'two-form' : 'one-form'">
    ...
    watch: {
      ioPoolPickBillWin(val) {
        if (val === true) {
          ...
          this.ioPoolPickBillForm.saveBrchNo  = this.ioPoolPickBillParams.saveBrchNo;
          this.$nextTick(() => {
            this.formSearch();
          });
        }
