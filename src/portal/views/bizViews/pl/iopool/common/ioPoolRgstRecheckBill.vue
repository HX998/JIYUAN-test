...
<template>
  <div>
        ...
        <div>
          ...
          <h-datagrid :columns="columns"
            ...
            <div slot="toolbar" class="pull-left">
              <h-dropdown placement="bottom-start" @on-click="recheckPass">
                <h-button type="primary">{{$t('m.i.common.recheckPass')}}
              ...
              <h-dropdown placement="bottom-start" @on-click="recheckBack">
                <h-button type="primary">{{$t('m.i.common.recheckReturn')}}
                  <h-icon name="unfold"></h-icon>
                </h-button>
                <h-dropdown-menu slot="list">
    ...
    <!-- 退回原因录入弹窗 -->
    <h-msg-box v-model="recheckBackWin" width="800" @on-close="recheckBackWinClose" class="h-form-search-layer"
      ...
      <div>
        <h-form :model="recheckBackForm" :label-width="115" ref="recheckBackForm" cols="1" class="h-form-search">
          <h-form-item prop="impawnRgstReturnReason" :label="$t('m.i.common.returnReason')"  required class="h-form-height-auto">
            <h-input v-model="recheckBackForm.impawnRgstReturnReason" placeholder="" type="textarea" :rows="3"
                     :canResize="false" :maxlength="150"></h-input>
          </h-form-item>
        </h-form>
      </div>
      <div slot="footer">
        <div slot="footer">
          ...
          <h-button type="primary" v-else @click="recheckBackFormSubmit()">{{$t('m.i.pl.returnBack')}}</h-button>
        </div>
      </div>
    </h-msg-box>
  </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        billManagerFormItem : {
          ...
          unimpawnRgstApplTellerNoNotEqual : JSON.parse(window.sessionStorage.getItem("userInfo")).userNo,
    ...
    methods: {
      ...
      recheckBack(type) {
        ...
        this.backType = type;
        this.recheckBackForm.checkFlag = "2";
        this.recheckBackWin = true;
      },
      handleAccountSubmit(params, type) {
        let url = this.ioPoolBillParams.accountType === "inPool" ? "/pl/iopool/inpool/paper/pInPoolRgstRecheckMain/func_checkInPoolRgst" : "/pl/iopool/outpool/paper/pOutPoolRgstRecheckMain/func_checkOutPoolRgst";
      ...
      recheckBackFormSubmit(){
        this.$refs["recheckBackForm"].validate(valid => {
          if (valid) {
            this.submitFlag = true;
            if (this.backType === "select") {
              let listIds = this.$refs.billManagerDatagrid.selectIds;
              let params = {
                batchId: this.ioPoolBillParams.batchId,
                listIds: listIds.join(","),
                checkFlag: this.recheckBackForm.checkFlag,
                impawnRgstReturnReason: this.recheckBackForm.impawnRgstReturnReason
              };
              this.handleRecheckBackSubmit(params, this.backType);
            } else if (this.backType === "batch") {
              if (this.$refs.billManagerDatagrid.tData.length === 0) {
                this.$msgTip.info(this, { info: "该批次下无数据" });
                return;
              }
              if (this.ioFlag === '1'){
                ...
                let params = {
                  ...
                  checkFlag: this.recheckBackForm.checkFlag,
                  unImpawnRgstStatusList: ["BTC02", "BTC05"],
                  unimpawnRgstReturnReason: this.recheckBackForm.impawnRgstReturnReason
                };
                this.handleRecheckBackSubmit(params, this.backType);
              } else {
                let params = {
      ...
      handleRecheckBackSubmit(params, type) {
        let url = this.ioPoolBillParams.accountType === "inPool" ? "/pl/iopool/inpool/paper/pInPoolRgstRecheckMain/func_checkInPoolRgst" : "/pl/iopool/outpool/paper/pOutPoolRgstRecheckMain/func_checkOutPoolRgst";
        post(params, url).then(res => {
      ...
      recheckBackWinClose() {
        ...
        this.recheckBackForm.checkFlag = "";
        this.$refs.recheckBackForm.resetFields();
      },
      //返回主页面
      returnMain() {
        this.resetSearch();
        this.$emit("returnRgstRecheckMain", "");
      },
      billInfoWinClose() {
