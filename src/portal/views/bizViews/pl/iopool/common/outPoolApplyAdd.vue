        ...
        <h-form :model="addOrEditForm" :label-width="115" ref="addOrEditForm" cols="3" class="h-form-search">
          <div>
            ...
            <show-branch v-model="addOrEditForm.saveBrchNo" :label="$t('m.i.pl.saveBrchName')" prop="saveBrchNo" required title="机构名称"
                         :brchNo.sync="addOrEditForm.saveBrchNo" :brchName.sync="addOrEditForm.saveBrchName" :showIcon="batchParams.batchType==='add'"
                         :readonly="batchParams.batchType==='modify'"
                         :filterable="true" :showCheckBox="false" :msgBoxWin="tempOutPoolAddOrEditWin">
            </show-branch>
  ...
  export default {
    ...
    methods: {
      ...
      jumpToPickBillWin() {
        ...
        this.ioPoolPickBillParams.saveBrchNo = this.addOrEditForm.saveBrchNo;
      ...
      ioPoolPickBillChange(info) {
        ...
        for (let i = 0; i < info.length; i++) {
          pickBillDtoList.push({billId: info[i].billId, hldrId: info[i].hldrId});
        }
        ...
        post({pickBillDtoList: pickBillDtoList, batchId: this.batchId}, url).then(res => {
          if (res) {
            let retCode = res.data.retCode;
            if (retCode === "000000") {
              this.$msgTip.success(this);
      ...
      infoExport() {
        ...
        this.param = "outPoolListExportInfo";
