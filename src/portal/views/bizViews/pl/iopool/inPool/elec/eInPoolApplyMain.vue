      ...
      <h-col span="24">
        <div>
          ...
          <h-datagrid
            :columns="columns"
            highlightRow
            url="/pl/iopool/inpool/elec/eInPoolApplyMain/func_pagingQueryInPoolBatchList"
            :bindForm="formItem"
        ...
        <!-- 新增修改弹窗 -->
        <e-in-pool-apply-add :inPoolAddOrEditWin="inPoolAddOrEditWin" :batchParams="batchParams"
                             @inPoolAddOrEditWinClose="inPoolAddOrEditWinClose"></e-in-pool-apply-add>
      </h-col>
    </h-row>
  </div>
</template>
...
<script>
  ...
  export default {
    name: "eInPoolApplyMain",
    components: {
      EInPoolApplyAdd: () => import(/* webpackChunkName: "pl/iopool/inPool/elec/eInPoolApplyAdd" */ '@/views/bizViews/pl/iopool/inPool/elec/eInPoolApplyAdd')
    },
    data() {
      return {
        formItem : {
    ...
    mounted() {
      this.getDictListByGroups("CDMedia,DraftTypeCode,Yon,BillOrigin").then(res => {
    ...
    methods: {
      ...
      handleDel() {
        ...
        post({ id: id }, "/pl/iopool/inpool/elec/eInPoolApplyMain/func_deleteInPoolBatch").then(res => {
          if (res) {
