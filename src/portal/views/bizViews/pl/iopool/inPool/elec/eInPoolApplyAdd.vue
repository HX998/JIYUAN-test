    ...
    <h-msg-box v-model="tempInPoolAddOrEditWin" :maximize="true" @on-close="handleClose"
      ...
      <div>
        <h-form :model="addOrEditForm" :label-width="115" ref="addOrEditForm" cols="3" class="h-form-search">
          <div>
            ...
            <common-select v-model="addOrEditForm.billOriginArray" :label="$t('m.i.billInfo.billOrigin')" prop="billOriginArray"
                           :readonly="batchOptType==='modify'"
                           :dictList="batchParams.billOriginArray"
                           :multiple="(this.isMixPack && batchOptType==='add')||batchOptType !=='add'" required ></common-select>
        ...
        <h-datagrid
          :columns="columns"
          :auto-load="false"
          url="/pl/iopool/inpool/elec/eInPoolApplyMain/func_pagingQueryInPoolBill"
          :bindForm="formItem"
          :rowSelect="true"
          :hasSelect="hasSelect"
          ref="datagrid">
          <div slot="toolbar" class="pull-left">
            <h-button type="primary" v-if="this.batchParams.isCommit === 'noCommit'" @click="ioPoolPickBill">{{$t('m.i.pl.pickBill')}}
            </h-button>
  ...
  mounted() {
    ...
    methods: {
      ...
      queryObjById() {
        ...
        post(params, "/pl/iopool/inpool/elec/eInPoolApplyMain/func_findInPoolBatchById").then(res => {
          if (res) {
            let retCode = res.data.retCode;
            if (retCode === "000000") {
      ...
      handleClose() {
        ...
        this.addOrEditForm.saveBrchName = "";
        this.hadBatchSave = false;
      ...
      //查询签约信息
      queryAgreementNo() {
        this.signParams.agreementUrl = "/pl/iopool/inpool/elec/eInPoolApplyMain/func_pagingBillAgreement";
      ...
      formSave() {
        this.$refs["addOrEditForm"].validate(valid => {
          if (valid) {
            ...
            let url = this.batchParams.batchType === "add" ? "/pl/iopool/inpool/elec/eInPoolApplyMain/func_addInPoolBatch" : "/pl/iopool/inpool/elec/eInPoolApplyMain/func_editInPoolBatch";
            if (this.hadBatchSave) {
              url = "/pl/iopool/inpool/elec/eInPoolApplyMain/func_editInPoolBatch";
            }
      ...
      jumpToPickBillWin() {
        ...
        this.ioPoolPickBillParams.billOriginArray = this.addOrEditForm.billOriginArray;
        this.ioPoolPickBillParams.pickBillType = "inPool";
        this.ioPoolPickBillParams.pickBillUrl = "/pl/iopool/inpool/elec/eInPoolApplyMain/func_pagingQueryInPoolUnsignBills";
      ...
      handleDeleteDetail() {
        ...
        }, "/pl/iopool/inpool/elec/eInPoolApplyMain/func_deleteInPoolBill").then(res => {
          if (res) {
            let msg = res.data.retMsg;
            let retCode = res.data.retCode;
            if (retCode === "000000") {
      ...
      getRiskBillNo(params) {
        post(params, "/pl/iopool/inpool/elec/eInPoolApplyMain/func_getRiskBillNo").then(res => {
          if (res) {
            let retCode = res.data.retCode;
      ...
      handleSubmit(params, isRiskCkeck) {
        post(params, "/pl/iopool/inpool/elec/eInPoolApplyMain/func_submitFlow").then(res => {
          if (res) {
            let retCode = res.data.retCode;
            if (retCode === "000000") {
              this.$msgTip.success(this);
      ...
      handleCancelApplyBatch() {
        ...
        // }
        post({ batchId: this.batchId }, "/pl/iopool/inpool/elec/eInPoolApplyMain/func_cancelFlow").then(res => {
          if (res) {
            let retCode = res.data.retCode;
            if (retCode === "000000") {
              this.$msgTip.success(this);
