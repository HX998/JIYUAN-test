...
<template>
  <div class="layout">
      ...
      <h-col span="24">
        ...
        <div>
          ...
          <h-datagrid :columns="columns"
                      highlightRow
                      url="/pl/iopool/outpool/elec/eOutPoolEndorseMain/func_pagingQueryOutPoolBatchList"
                      :bindForm="formItem"
                      :onCurrentChange="handleCurrentChange"
                      :onCurrentChangeCancel="handleCurrentChangeCancel"
                      ref="datagrid">
            <div slot="toolbar" class="pull-left">
              <h-button type="primary" @click="outPoolEndorse()" :disabled="formItem.isCommit === 'hasCommit'">{{$t("m.i.pl.enDo")}}
              </h-button>
              <h-button type="primary" @click="endorseCancel()" :disabled="formItem.isCommit === 'noCommit'">{{$t("m.i.pl.returnEnDo")}}
              </h-button>
            </div>
          </h-datagrid>
        </div>
    ...
    <e-out-pool-endorse-bill :billManagerVue="billManagerVue" :outPoolBillParams="outPoolBillParams"
                         @returnEndorseMain="returnEndorseMain"></e-out-pool-endorse-bill>
    <!-- 查询客户信息弹窗 -->
...
<script>
  ...
  export default {
    name: "eOutPoolEndorseMain",
    components: {
      EOutPoolEndorseBill: () => import( /* webpackChunkName: "pl/iopool/outPool/elec/eOutPoolEndorseBill" */ '@/views/bizViews/pl/iopool/outPool/elec/eOutPoolEndorseBill')
    },
    data() {
      let columns= [
            {
    ...
    mounted() {
      this.getDictListByGroups("DraftTypeCode,GetWay,BillOrigin").then(res => {
    ...
    methods: {
      ...
      noCommitButton() {
        ...
        this.formItem.funcNo = "PIPL040206";
        this.handleSearch();
      },
      //已背书
      hasCommitButton() {
        this.formItem.isCommit = "hasCommit";
        this.formItem.funcNo = "PIPL040208";
        this.handleSearch();
      },
      //表单查询
      handleSearch() {
      ...
      outPoolEndorse() {
        ...
        this.outPoolBillParams.batchId = this.currentSelectRow.id;
        this.assembleParams();
      },
      //出池撤销背书
      endorseCancel() {
      ...
      openOutPoolEditWin(batchId) {
        this.outPoolBillParams.batchId = batchId;
        this.assembleParams();
      },
      assembleParams() {
        this.outPoolBillParams.dictMap = this.dictMap;
        this.outPoolBillParams.funcNo = this.formItem.funcNo;
        this.outPoolBillParams.billClass = this.formItem.billClass;
        this.outPoolBillParams.isCommit = this.formItem.isCommit;
        this.outPoolBillParams.accountType = "outPool";
        this.outPoolBillParams.batchInfoUrl = "/pl/iopool/outpool/elec/eOutPoolEndorseMain/func_queryBatchInfo";
        this.mainVue = false;
