      ...
      <h-col span="24">
        <div>
          ...
          <h-datagrid
            :columns="columns"
            highlightRow
            url="/pl/iopool/outpool/elec/eOutPoolApplyMain/func_pagingQueryOutPoolBatchList"
            :bindForm="formItem"
            :onCurrentChange="handleCurrentChange"
            :onCurrentChangeCancel="handleCurrentChangeCancel"
            ref="datagrid">
            <div slot="toolbar" class="pull-left">
              <h-button type="primary" @click="outPoolOpenWin('add')" :disabled="formItem.isCommit === 'hasCommit'">
                {{$t("m.i.common.add")}}
              </h-button>
              <h-button type="primary" @click="outPoolOpenWin('modify')"
                        :disabled="formItem.isCommit === 'hasCommit'">{{$t("m.i.common.modify")}}
              </h-button>
              <h-button type="primary" @click="outPoolDelete()" :disabled="formItem.isCommit === 'hasCommit'">
                {{$t("m.i.common.delete")}}
              </h-button>
              <h-button type="primary" @click="outPoolCancel()" :disabled="formItem.isCommit === 'noCommit'">{{$t("m.i.common.cancelCheckAuth")}}
              </h-button>
            </div>
          </h-datagrid>
        </div>
        ...
        <!-- 新增修改弹窗 -->
        <e-out-pool-apply-add :outPoolAddOrEditWin="outPoolAddOrEditWin" :batchParams="batchParams"
                              @outPoolAddOrEditWinClose="outPoolAddOrEditWinClose"></e-out-pool-apply-add>
      </h-col>
    </h-row>
  </div>
</template>
...
<script>
import {formatNumber, post,getSingleParamValuesByKeys} from "@/api/bizApi/commonUtil";

export default {
    name: "eOutPoolApplyMain",
    components: {
      EOutPoolApplyAdd: () => import(/* webpackChunkName:  "pl/iopool/common/outPoolApplyAdd"*/ '@/views/bizViews/pl/iopool/common/eOutPoolApplyAdd')
    },
    data() {
      return {
        formItem: {
    ...
    mounted() {
      this.getDictListByGroups("CDMedia,DraftTypeCode,Yon,GetWay,BillOrigin").then(res => {
      ...
      getSingleParamValuesByKeys("pl.iopool.is_ebank_pick_bill").then(res => {
        this.showEbankPickBill = res["pl.iopool.is_ebank_pick_bill"];
      });
    },
    methods: {
      clearVal(type){
      ...
      noCommitButton() {
        ...
        this.formItem.funcNo = "PIPL040202";
        this.handleSearch();
      },
      //已提交
      hasCommitButton() {
        this.formItem.isCommit = "hasCommit";
        this.formItem.funcNo = "PIPL040203";
        this.handleSearch();
      },
      //表单查询
      handleSearch(pageNo) {
      ...
      outPoolOpenWin(str) {
        if (str === "modify") {
          ...
          this.batchParams.outPoolBatchId = this.currentSelectRow.id;
        } else {
          this.batchParams.outPoolBatchId = "";
        }
        this.assembleParams(str);
      },
      //关闭新增、修改弹窗
      outPoolAddOrEditWinClose() {
        this.outPoolAddOrEditWin = false;
      ...
      handleDel() {
        ...
        post({ id: id }, "/pl/iopool/outpool/elec/eOutPoolApplyMain/func_deleteOutPoolBatch").then(res => {
          if (res) {
      ...
      openOutPoolEditWin(batchId) {
        this.batchParams.outPoolBatchId = batchId;
        this.assembleParams("modify");
      },
      assembleParams(optType) {
        this.outPoolAddOrEditWin = true;
        this.$nextTick(() => {
