      ...
      <h-col span="24">
        <div>
          ...
          <h-datagrid
            :columns="columns"
            highlightRow
            url="/pl/iopool/outpool/paper/pOutPoolApplyMain/func_pagingQueryOutPoolBatchList"
            :bindForm="formItem"
        ...
        <!-- 新增修改弹窗 -->
        <out-pool-apply-add :outPoolAddOrEditWin="outPoolAddOrEditWin" :batchParams="batchParams"
                              @outPoolAddOrEditWinClose="outPoolAddOrEditWinClose"></out-pool-apply-add>
      </h-col>
    </h-row>
  </div>
</template>
...
<script>
  ...
  export default {
    name: "pOutPoolApplyMain",
    components: {
      OutPoolApplyAdd: () => import( /* webpackChunkName: "pl/iopool/common/outPoolApplyAdd" */ '@/views/bizViews/pl/iopool/common/outPoolApplyAdd')
    },
    data() {
      return {
        formItem: {
    ...
    mounted() {
      this.getDictListByGroups("CDMedia,DraftTypeCode,Yon,GetWay").then(res => {
    ...
    methods: {
      ...
      handleDel() {
        ...
        post({ id: id }, "/pl/iopool/outpool/paper/pOutPoolApplyMain/func_deleteOutPoolBatch").then(res => {
          if (res) {
