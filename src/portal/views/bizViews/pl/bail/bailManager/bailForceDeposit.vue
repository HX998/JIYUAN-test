...
<template>
  <div class="layout">
      ...
      <h-col span="24">
        <div>
          ...
          <h-datagrid
            :columns="columns"
            highlightRow
            url="/pl/bail/bailManager/bailForceDeposit/func_pagingQueryCustCredit"
            :bindForm="formItem"
    ...
    <h-msg-box v-model="addOrEditWin" width="800" :mask-closable="false" @on-close="handleWinClose"
      ...
      <div>
        <h-form :model="addOrEditForm" :label-width="115" ref="addOrEditForm" cols="2" class="h-form-search">
          ...
          <h-form-item>
            <h-button type="primary" :disabled="!addOrEditForm.billBailAcctNo" @click="syncBailBalance()">{{$t("m.i.pl.syncBailBalance")}}</h-button>
          </h-form-item>
          ...
          <h-form-item :label="$t('m.i.pl.changeAmt')" prop="changeAmt" required :validRules="changeAmtRule">
            <h-typefield type="money" v-model="addOrEditForm.changeAmt" :maxlength="20" bigTips divided placeholder=""></h-typefield>
          </h-form-item>
          <h-form-item :label="$t('m.i.pl.freezedAmt')" prop="freezedAmt" required>
            <h-typefield type="money" v-model="addOrEditForm.freezedAmt" :maxlength="20" readonly bigTips
                         divided placeholder=""></h-typefield>
          </h-form-item>
        </h-form>
      </div>
    ...
    <show-pool-credit-info :pool-credit-info-win="poolCreditInfoWin" :poolCreditInfoParams="poolCreditInfoParams"
                           @poolCreditInfoChange="poolCreditInfoChange" @poolCreditInfoWinClose="poolCreditInfoWinClose"></show-pool-credit-info>
  ...
  export default {
    ...
    methods: {
      validChangeAmt(rule, val, callback) {
        let maxChangeAmt =  Number(this.addOrEditForm.bailBalance);
        if (Number(this.addOrEditForm.changeAmt) > Number(maxChangeAmt)) {
          callback(new Error("转账金额不能大于保证金余额"));
        }
      ...
      poolCreditInfoChange(info){
        ...
        this.addOrEditForm.freezedAmt = info.freezedAmt;
      ...
      queryCreditInfo(params) {
        post(params, "/pl/bail/bailManager/bailForceDeposit/func_queryCustCreditInfo").then(res => {
          if (res) {
            let retCode = res.data.retCode;
            if (retCode === "000000") {
              ...
              this.addOrEditForm.freezedAmt = res.data.retData.freezedAmt;
            } else {
              this.$msgTip.error(this, { info: res.data.retMsg });
            }
          } else {
      ...
      submitForm() {
        this.$refs["addOrEditForm"].validate(valid => {
          if (valid) {
            ...
            this.addOrEditForm.freezedAmt = this.addOrEditForm.freezedAmt.toString().replace(/,/g, "");
            this.submitFlag = true;
            let url = "/pl/bail/bailManager/bailForceDeposit/func_bailToSettle";
        ...
        post(params, "/pl/bail/bailManager/bailDeposit/func_pagingQueryCustCreditInfo").then(res => {
          if (res) {
            ...
            if (retCode === "000000") {
              ...
              this.addOrEditForm.freezedAmt = list.freezedAmt;
            } else {
              this.$msgTip.error(this, { info: res.data.retMsg });
            }
          } else {
      ...
      queryDetail() {
        if (this.currentSelectRow != null && this.currentSelectRow.length !== 0) {
          ...
          this.logDetailParams.batchUrl = "/pl/bail/bailManager/bailForceDeposit/func_pagingQueryCustCreditInfo";
          this.logDetailParams.isQueryLog = false;
          this.logDetailParams.showSubBillBailAcctNo = this.isSubAcctWorks==='1';   //是否显示保证金子账号
          this.mainVue = false;
