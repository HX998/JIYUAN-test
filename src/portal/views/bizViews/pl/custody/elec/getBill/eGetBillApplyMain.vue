      ...
      <h-col span="24">
        <div>
          ...
          <h-datagrid
            :columns="columns"
            highlightRow
            url="/pl/custody/elec/getBill/eGetBillApplyMain/func_pageQueryGetBatchList"
            :bindForm="formItem"
        ...
        <!-- 新增修改弹窗 -->
        <e-get-bill-apply-add :get-bill-add-or-edit-win="getBillAddOrEditWin" :batch-params="batchParams"
                            @getBillAddOrEditWinClose="getBillAddOrEditWinClose"></e-get-bill-apply-add>
...
<script>
  ...
  export default {
    name: "eGetBillApplyMain",
    components: {
      eGetBillApplyAdd : () => import(/* webpackChunkName: "pl/custody/elec/getBill/eGetBillApplyAdd" */ '@/views/bizViews/pl/custody/elec/getBill/eGetBillApplyAdd')
    },
    data() {
      return {
        formItem : {
    ...
    mounted() {
      this.getDictListByGroups("DraftTypeCode,GetWay,Yon,BillOrigin").then(res => {
    ...
    methods: {
      ...
      handleDel() {
        post({id: this.currentSelectRow.id},'/pl/custody/elec/getBill/eGetBillApplyMain/func_deleteGetBatch').then(res => {
          if(res){
