      ...
      <h-col span="24">
        ...
        <div v-if="mainVue" key="1">
          <div class="h-form-search-box">
            ...
            <h-panel class="clearfix">
              <h-form :model="formItem" :label-width="90" ref="formItem" cols="4" class="h-form-search">
                ...
                <bill-no :label="$t('m.i.billInfo.billPackageNo')" prop="trackSearchBillNo" v-model="formItem.trackSearchBillNo"></bill-no>
          ...
          <h-datagrid :columns="columns"
                      highlightRow
                      url="/pl/custody/elec/getBill/eGetBillTrackSearch/func_pageQueryGetBatchTrack"
                      :bindForm="formItem"
        ...
        <get-bill-batch-info-detail :is-show="billManagerVue" :params="batchInfoParams" ref="getBillBatchInfoDetail"
                                    batchInfoUrl="/pl/custody/elec/getBill/eGetBillTrackSearch/func_pageQueryGetBatchList"></get-bill-batch-info-detail>
        <!-- 数据展示表格 -->
...
<script>
  ...
  export default {
    ...
    mounted() {
      getDictListByGroups("DraftTypeCode,CheckType,GetWay,BillOrigin").then(res => {
    ...
    methods: {
      ...
      assembleBillParams(vocherData) {
        ...
        for (let i = 0; i < vocherData.length; i++) {
          let printData = {};
          printData.billOrigin = !!vocherData[i].billOrigin ? this.getDictValueFromMap(this.dictMap, "BillOrigin", vocherData[i].billOrigin) : "";
        ...
        let headHtml = `<div style="text-align:center;" ref="headHtml">
          ...
          <div style="text-align:left;font-size:14px;font-weight:bold;">客户名称：${this.$refs.getBillBatchInfoDetail.viewDetailForm.custName}</div>
          <div style="text-align:left;font-size:14px;font-weight:bold;">客户账号：${this.$refs.getBillBatchInfoDetail.viewDetailForm.custAcctNo}</div>
        </div>`;
