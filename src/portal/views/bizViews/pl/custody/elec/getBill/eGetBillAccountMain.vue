      ...
      <h-col span="24">
        ...
        <div v-if="mainVue" key="1">
          ...
          <h-datagrid :columns="columns"
                      highlightRow
                      url="/pl/custody/elec/getBill/eGetBillAccountMain/func_pageQueryGetBatchList"
                      :bindForm="formItem"
        ...
        <get-bill-batch-info-detail :is-show="billManagerVue" :params="batchInfoParams" ref="getBillBatchInfoDetail"
                                    batchInfoUrl="/pl/custody/elec/getBill/eGetBillAccountMain/func_pageQueryGetBatchList"></get-bill-batch-info-detail>
        <!-- 数据展示表格 -->
        <div v-if="billManagerVue" key="2">
          <h-datagrid :columns="billManagerColumns"
                      url="/pl/custody/elec/getBill/eGetBillAccountMain/func_pageQueryGetBillList"
                      :auto-load="false"
                      :row-select="true"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //获取分录信息
      getEntryInfo(str) {
        this.acctRecordUrl = "/pl/custody/elec/getBill/eGetBillAccountMain/func_getGetBillAcctRecord";
        if(str === "batch") {
      ...
      acctSubmitSure(info) {
        ...
        post(param, "/pl/custody/elec/getBill/eGetBillAccountMain/func_getBillAccount").then(res => {
      ...
      batchBack() {
        this.$hMsgBox.confirm({
          ...
          content: "确定批次退回吗?",
          onOk: () => {
            post({batchId: this.batchId, operType: "1"}, "/pl/custody/elec/getBill/eGetBillAccountMain/func_getBillAccountBack").then(res => {
              if(res) {
                let msg = res.data.retMsg;
                let retCode = res.data.retCode;
                if (retCode === "000000") {
      ...
      selectBack() {
        ...
        this.$hMsgBox.confirm({
          ...
          content: "确定退回吗?",
          onOk: () => {
            post({batchId: this.batchId, operType: "0", ids: this.acctIds}, "/pl/custody/elec/getBill/eGetBillAccountMain/func_getBillAccountBack").then(res => {
              if(res) {
                let msg = res.data.retMsg;
                let retCode = res.data.retCode;
                if (retCode === "000000") {
