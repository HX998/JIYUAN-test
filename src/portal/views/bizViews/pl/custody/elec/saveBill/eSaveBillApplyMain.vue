      ...
      <h-col span="24">
        <div>
          ...
          <h-datagrid
            :columns="columns"
            highlightRow
            url="/pl/custody/elec/saveBill/eSaveBillApplyMain/func_pageQuerySaveBatchList"
            :bindForm="formItem"
        ...
        <!-- 新增修改弹窗 -->
        <e-save-bill-apply-add :saveBillAddOrEditWin="saveBillAddOrEditWin" :batchParams="batchParams"
                             @saveBillAddOrEditWinClose="saveBillAddOrEditWinClose"></e-save-bill-apply-add>
...
<script>
  ...
  export default {
    name: "eSaveBillApplyMain",
    components: {
      ESaveBillApplyAdd: () => import(/* webpackChunkName: "pl/custody/elec/saveBill/eSaveBillApplyAdd" */ '@/views/bizViews/pl/custody/elec/saveBill/eSaveBillApplyAdd')
    },
    data() {
      return {
        formItem : {
    ...
    methods: {
      ...
      handleDel() {
        post({id: this.currentSelectRow.id},'/pl/custody/elec/saveBill/eSaveBillApplyMain/func_deleteSaveBatch').then(res => {
          if(res){
