      ...
      <h-col span="24">
        ...
        <div v-if="mainVue" key="1">
          ...
          <h-datagrid :columns="columns"
                      highlightRow
                      url="/pl/custody/elec/saveBill/eSaveBillAccountMain/func_pageQuerySaveBatchList"
                      :bindForm="formItem"
        ...
        <save-bill-batch-info-detail  :is-show="billManagerVue" :params="batchInfoParams" ref="saveBillBatchInfoDetail"
                                      batchInfoUrl="/pl/custody/elec/saveBill/eSaveBillAccountMain/func_pageQuerySaveBatchList"></save-bill-batch-info-detail>
        <!-- 数据展示表格 -->
        <div v-if="billManagerVue" key="2">
          <h-datagrid :columns="billManagerColumns"
                      url="/pl/custody/elec/saveBill/eSaveBillAccountMain/func_pageQuerySaveBillList"
                      :auto-load="false"
                      :row-select="true"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //获取分录信息
      getEntryInfo(str) {
        this.acctRecordUrl = "/pl/custody/elec/saveBill/eSaveBillAccountMain/func_getSaveBillAcctRecord";
        if(str === "batch") {
      ...
      acctSubmitSure(info) {
        ...
        post(param, "/pl/custody/elec/saveBill/eSaveBillAccountMain/func_saveBillAccount").then(res => {
      ...
      batchBack() {
        this.$hMsgBox.confirm({
          ...
          content: "确定批次退回吗?",
          onOk: () => {
            post({batchId: this.batchId, operType: "1"}, "/pl/custody/elec/saveBill/eSaveBillAccountMain/func_saveBillAccountBack").then(res => {
              if(res) {
                let msg = res.data.retMsg;
                let retCode = res.data.retCode;
                if (retCode === "000000") {
      ...
      selectBack() {
        ...
        this.$hMsgBox.confirm({
          ...
          content: "确定退回吗?",
          onOk: () => {
            post({batchId: this.batchId, operType: "0", ids: this.acctIds}, "/pl/custody/elec/saveBill/eSaveBillAccountMain/func_saveBillAccountBack").then(res => {
              if(res) {
                let msg = res.data.retMsg;
                let retCode = res.data.retCode;
                if (retCode === "000000") {
