      ...
      <h-col span="24">
        ...
        <div v-if="mainVue" key="1">
          ...
          <h-datagrid :columns="columns"
                      highlightRow
                      url="/pl/custody/elec/saveBill/eSaveBillCancelAccountMain/func_pageQuerySaveBatchList"
                      :bindForm="formItem"
        ...
        <save-bill-batch-info-detail  :is-show="billManagerVue" :params="batchInfoParams" ref="saveBillBatchInfoDetail"
                                      batchInfoUrl="/pl/custody/elec/saveBill/eSaveBillCancelAccountMain/func_pageQuerySaveBatchList"></save-bill-batch-info-detail>
        <!-- 数据展示表格 -->
        <div v-if="billManagerVue" key="2">
          <h-datagrid :columns="billManagerColumns"
                      url="/pl/custody/elec/saveBill/eSaveBillCancelAccountMain/func_pageQueryCancelAcctFlow"
                      :auto-load="false"
                      :row-select="true"
        ...
        <!-- 查看票据明细弹窗 -->
        <e-save-bill-show-bill-detail :bill-detail-win="billDetailWin" :batch-id="this.batchId" :batch-params="this.batchParams"
                                    :acctFlowIds="this.infoIds" bill-class="ME02" @billDetailWinClose="billDetailWinClose"
                                    billDetailUrl="/pl/custody/elec/saveBill/eSaveBillCancelAccountMain/func_pageQuerySaveBillList"></e-save-bill-show-bill-detail>
        <!--显示流水明细界面-->
...
<script>
  ...
  export default {
    name: "eSaveBillCancelAccountMain",
    components: {
      AcctFlowDetail: () => import(/* webpackChunkName: "pl/custody/common/acctFlowDetail" */ '@/views/bizViews/pl/custody/common/acctFlowDetail'),
      ESaveBillShowBillDetail: () => import(/* webpackChunkName: "pl/custody/common/saveBillShowBillDetail" */ '@/views/bizViews/pl/custody/common/eSaveBillShowBillDetail'),
    },
    data() {
      let columns = [
        {
    ...
    methods: {
      ...
      batchCancelAccount() {
        this.$hMsgBox.confirm({
          ...
          content: "确定批次撤销记账吗?",
          onOk: () => {
            post({batchId: this.batchId, operType: "1"}, "/pl/custody/elec/saveBill/eSaveBillCancelAccountMain/func_cancelBillAccount").then(res => {
              if(res) {
      ...
      selectCancelAccount() {
        ...
        this.$hMsgBox.confirm({
          ...
          content: "确定撤销记账吗?",
          onOk: () => {
            post({batchId: this.batchId, operType: "0", acctFlowIds: ids}, "/pl/custody/elec/saveBill/eSaveBillCancelAccountMain/func_cancelBillAccount").then(res => {
              if(res) {
                let msg = res.data.retMsg;
                let retCode = res.data.retCode;
                if (retCode === "000000") {
