<template>
  <div>
    <h-msg-box v-model="tempGetBillAddOrEditWin" :maximize="true" @on-close="handleClose"
      ...
      <div>
        <h-form :model="addOrEditForm" :label-width="115" ref="addOrEditForm" cols="3" class="h-form-search">
          <div>
            <h-form-item prop="agreementNo" :label="$t('m.i.pl.agreementNo')" required>
              <h-input v-model="addOrEditForm.agreementNo" placeholder=""
            ...
            <show-branch v-model="addOrEditForm.saveBrchNo" :label="$t('m.i.pl.saveBrchName')" prop="saveBrchNo" required title="机构名称"
                         :brchNo.sync="addOrEditForm.saveBrchNo" :brchName.sync="addOrEditForm.saveBrchName" :showIcon="this.saveBrchIcon"
        ...
        <h-datagrid
          :columns="columns"
          :auto-load="false"
          url="/pl/custody/paper/getBill/getBillApplyMain/func_pageQueryGetBillList"
          :bindForm="formItem"
          :rowSelect="true"
    ...
    <!-- 挑票 -->
    <get-bill-pick :save-pick-bill-win="savePickBillWin" @savePickBillWinClose="savePickBillWinClose"
                   @savePickBillChange="savePickBillChange" :bill-class="this.billClass"  :bill-type="this.batchParams.billType"
                   :data-source="1"
                   :batch-id="this.batchParams.batchId" :custAcctNo="this.batchParams.custAcctNo" :acct-brch-no="this.addOrEditForm.saveBrchNo"></get-bill-pick>

    <!-- 挑票（网银发起） -->
    <get-bill-pick :save-pick-bill-win="savePickBillWinWithEbank" @savePickBillWinClose="savePickBillWinWithEbankClose"
                     ...
                     :data-source="2"
                     :custAcctNo="this.batchParams.custAcctNo" :acct-brch-no="this.addOrEditForm.saveBrchNo"></get-bill-pick>
...
<script>
  ...
  export default {
    name: "getBillApplyAdd",
    components: {
      GetBillPick: () => import(/* webpackChunkName: "pl/custody/common/getBillPick" */ '@/views/bizViews/pl/custody/common/getBillPick')
    },
    data() {
      return {
    ...
    methods: {
      ...
      queryObjById() {
        ...
        post(params, "/pl/custody/paper/getBill/getBillApplyMain/func_pageQueryGetBatchList").then(res => {
          if (res) {
      ...
      //查询签约信息
      queryAgreementNo() {
        this.signParams.agreementUrl = "/pl/custody/paper/getBill/getBillApplyMain/func_pagingBillAgreement";
      ...
      formSave() {
        this.$refs["addOrEditForm"].validate(valid => {
          ...
          if (valid) {
            ...
            let url = this.batchParams.batchType === "add" ? "/pl/custody/paper/getBill/getBillApplyMain/func_addGetBatch" : "/pl/custody/paper/getBill/getBillApplyMain/func_updateGetBatch";
            if (this.hadBatchSave) {
              url = "/pl/custody/paper/getBill/getBillApplyMain/func_updateGetBatch";
            }
      ...
      savePickBillChange(info,dataSource) {
        ...
        post({batchId: this.batchId ,dataSource: dataSource, pickBillDtoList: pickDtoList},'/pl/custody/paper/getBill/getBillApplyMain/func_addGetBillInfo').then(res => {
          if(res){
            let retCode = res.data.retCode;
            if(retCode==="000000"){
              this.$msgTip.success(this);
      ...
      batchDelete() {
        ...
        this.$hMsgBox.confirm({
          ...
          onOk: () => {
            ...
            post({batchId: this.batchId, billOperType: "1",flowStatusList: flowStatusList }, "/pl/custody/paper/getBill/getBillApplyMain/func_deleteGetBillInfo").then(res => {
              if (res) {
                let msg = res.data.retMsg;
                let retCode = res.data.retCode;
                if (retCode === "000000") {
      ...
      selectDelete() {
        ...
        this.$hMsgBox.confirm({
          ...
          onOk: () => {
            ...
            post({batchId: this.batchId, billOperType: "0", ids: ids }, "/pl/custody/paper/getBill/getBillApplyMain/func_deleteGetBillInfo").then(res => {
              if (res) {
                let msg = res.data.retMsg;
                let retCode = res.data.retCode;
                if (retCode === "000000") {
      ...
      handleSubmit(params) {
        post(params, "/pl/custody/paper/getBill/getBillApplyMain/func_submitGetBillFlow").then(res => {
          if (res) {
            let retCode = res.data.retCode;
            if (retCode === "000000") {
              this.$msgTip.success(this);
      ...
      handleCancelApplyBatch() {
        post({ batchId: this.batchId, billOperType: '1' }, "/pl/custody/paper/getBill/getBillApplyMain/func_cancelGetBillFlow").then(res => {
          if (res) {
            let retCode = res.data.retCode;
            if (retCode === "000000") {
              this.$msgTip.success(this);
      ...
      exportBillExcel(){
        ...
        this.param = "getBillListExportInfo";
