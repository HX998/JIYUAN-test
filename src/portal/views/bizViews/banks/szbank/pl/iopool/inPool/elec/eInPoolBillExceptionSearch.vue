      ...
      <h-col span="24">
        <div>
          <div class="h-form-search-box">
            ...
            <h-panel class="clearfix" :class="{'h-form-overhd':!ifShowMore}">
              <h-form :model="formItem" :label-width="91" ref="formItem" cols="4" class="h-form-search">
                <show-branch v-model="formItem.transBrchNoList" label="入池机构名" prop="transBrchNoList" required
                             title="机构名称" :brchNo.sync="formItem.transBrchNoList"
                             :brchName.sync="formItem.transBrchNameList">
                </show-branch>
                ...
                <h-form-item :label="$t('m.i.pl.inPoolDt')" prop="minInPoolDt">
                  <h-date-picker :value="inPoolDt" type="daterange" showFormat :editable=false
                                 @on-change="handleInPoolDtChange" placeholder=""></h-date-picker>
                </h-form-item>
                ...
                <h-form-item :label="$t('m.i.billInfo.billPackageMoney')" :prop="isMinBillMoney"
                ...
                <h-form-item prop="custName" label="入池申请人名称" class="h-form-long-label">
                  <h-input v-model="formItem.custName" placeholder="" :maxlength="125"></h-input>
                </h-form-item>
          ...
          <h-datagrid
            :columns="columns"
            highlightRow
            url="/banks/szBank/inPool/inPoolBillExceptionSearch/func_queryInPoolInfoExceptionResult"
            :bindForm="formItem"
            :onSelectChange="handleSelectClick"
            ref="datagrid">
            <div slot="toolbar" class="pull-left">
              <h-button type="primary" @click="exceptionResult">{{ $t("m.i.ce.discInfoExceptionResult") }}
              </h-button>
            </div>
          </h-datagrid>
        </div>
...
export default {
  name: "eInPoolBillExceptionSearch",
  components: {
  ...
  data() {
    return {
      ...
      formItem: {
        ...
        transBrchNameList: this.$store.getters.userInfo.brchName,
        billNo: "",
        reverseBillNo: "",
        billOrigin: "",
        minInPoolDt: "",
  ...
  methods: {
    ...
    handleSearch() {
      this.$refs["formItem"].validate(valid => {
        if (valid) {
          ...
          this.formItem.minInPoolDt = this.formItem.minInPoolDt.replace(/-/g, "");
          this.formItem.maxInPoolDt = this.formItem.maxInPoolDt.replace(/-/g, "");
    ...
    resetSearch() {
      ...
      this.formItem.minInPoolDt = "";
      this.formItem.maxInPoolDt = "";
    ...
    handleInPoolDtChange(arr) {
      if (arr && arr.length == 2) {
        this.formItem.minInPoolDt = arr[0].replace(/-/g, "");
        this.formItem.maxInPoolDt = arr[1].replace(/-/g, "");
        this.inPoolDt = [arr[0], arr[1]];
      } else {
        this.formItem.minInPoolDt = "";
        this.formItem.maxInPoolDt = "";
        this.inPoolDt = [];
    ...
    exceptionResult() {
      ...
      if (list != null && list.length > 0) {
        ...
        post({listIds: discIds.join(",")}, "/banks/szBank/inPool/inPoolBillExceptionSearch/func_exceptionBillProcess").then(res => {
          if (res) {
