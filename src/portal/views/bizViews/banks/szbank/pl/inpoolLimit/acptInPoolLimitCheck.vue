      ...
      <h-col span="24">
        <div>
          ...
          <h-datagrid
            ...
            :has-select="false"
            highlightRow
            url="/banks/szbank/pl/inpoolLimit/acptInPoolLimitCheck/func_pagingQueryCheckAcptInPoolLimitInfos"
            :bindForm="formItem"
            ref="datagrid">
            <div slot="toolbar" class="pull-left">
              <h-button type="primary" @click="handleComfirm('pass')" :disabled="operDisabled">{{ $t('m.i.common.pass') }}</h-button>
              <h-button type="primary" @click="handleComfirm('reject')" :disabled="operDisabled">{{ $t('m.i.common.reject') }}</h-button>
            </div>
          </h-datagrid>
        </div>
      </h-col>
...
export default {
  name:"acptInPoolLimitCheck",
  data(){
    return {
      formItem:{
        acptTopBankNo:"",
        acptTopBankNameLike:"",
        operStatusList:["01","04","07"]
      },
      ifShowMore:false,
      ...
      columns:[
        ...
        {
          title: "入池限额(元)",
          key:"checkInpoolLimitAmtStr",
          hiddenCol:false,
          render: (h, params) => {
            let inpoolLimitAmt = formatNumber(params.row.checkInpoolLimitAmtStr, 2, ',');
            return h("span", {
              domProps: {
      ...
      searchUrl:"/banks/szbank/pl/inpoolLimit/acptInPoolLimitCheck/func_queryAcptInpoolLimitUsedInfo",
      operDisabled:false,
    }
  },
  components:{
  ...
  methods:{
    ...
    handlePassOrRej(oper) {
      ...
      let url = '';
      if (oper == 'pass') {
        url = '/banks/szbank/pl/inpoolLimit/acptInPoolLimitCheck/func_reviewAcptInPoolLimitInfoPass';
      }
      if (oper == 'reject') {
        url = '/banks/szbank/pl/inpoolLimit/acptInPoolLimitCheck/func_reviewAcptInPoolLimitInfoReject';
      }
