...
<script>
  ...
  export default {
    ...
    data() {
      ...
      let nextColumns = [
        ...
        },
        {
          ...
          hiddenCol: false,
          render: (h, params) => {
            let dictValue = this.getDictValueFromMap(this.dictMap, "SmsNoticeSignType", params.row.reserve1);
            return h("span", {
              domProps: {
                title: dictValue
              }
    ...
    mounted() {
      this.getDictListByGroups("signBusiType,custAcctType,custAcctKind,bailType,depositStyle,SmsNoticeSignType").then(res => {
    ...
    methods: {
      ...
      handleSignOut() {
        let idList = [];
        idList.push(this.selectData.id);
      ...
      printVoucher(idList){
        ...
        let params = {templateName: "SZBANK003", params: JSON.stringify(param),listIds: idList,saveFlag:1};
      ...
      tempShowExcelTemplateWinOpen() {
        if (this.formItem.signStatus === "2") {
          this.$msgTip.info(this, { info: "未签约数据暂不支持导出" });
          return;
        }
        this.param = "szbankElecSignBusiDataInfo";
      ...
      exportList(field, exportType) {
        let exportUrl = window.LOCAL_CONFIG.API_HOME + "/bm/sign/func_exportElecSignBusiListExcel";
        let options = {
          params: {}
        };
        if (this.$refs.datagrid.selectIds.length !== 0) {
