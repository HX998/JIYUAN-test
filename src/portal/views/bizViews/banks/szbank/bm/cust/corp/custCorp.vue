        ...
        <div>
          ...
          <h-datagrid
            ...
            <div slot="toolbar" class="pull-left" v-if="this.isShowToolbar">
              <h-button type="primary" @click="handleAddForm('coll')"
                        v-if="authObj.bizSetRoleAdd && (this.btnAuth.synch === undefined ? true : this.btnAuth.synch.isShow)"
                        :disabled="this.deletDisabled">{{ $t("m.i.common.synch") }}
              </h-button>
              <h-button type="primary" @click="handleAddForm('modify')"
                        v-if="authObj.bizSetRoleAdd && (this.btnAuth.modify === undefined ? true : this.btnAuth.modify.isShow)"
                        :disabled="this.isLogOn">{{ $t("m.i.common.modify") }}
              </h-button>
              <h-button type="primary" @click="handleShowAcct()"
                        v-if="authObj.bizSetRoleDelete && (this.btnAuth.view === undefined ? true : this.btnAuth.view.isShow)"
                        :disabled="this.isLogOn">{{ $t("m.i.bm.viewAccount") }}
              </h-button>
              <h-button type="primary" @click="handleComfirm('logoff')"
                        v-if="authObj.bizSetRoleDelete && (this.btnAuth.loginOut === undefined ? true : this.btnAuth.loginOut.isShow)"
                        :disabled="this.isLogOn">{{ $t("m.i.common.loginOut") }}
              </h-button>
              <h-button type="primary" @click="handleComfirm('logon')"
                        v-if="authObj.bizSetRoleDelete && (this.btnAuth.cacelOut === undefined ? true : this.btnAuth.cacelOut.isShow)"
                        :disabled="this.isLogOff">{{ $t("m.i.bm.cacelOut") }}
              </h-button>
              <h-button type="primary" @click="handleShowGroup()"
                        v-if="authObj.bizSetRoleDelete && (this.btnAuth.group === undefined ? true : this.btnAuth.group.isShow)"
                        :disabled="this.isGroupOff">{{ $t("m.i.bm.dissolutionGroup") }}
              </h-button>
              <h-button type="primary" @click="tempShowExcelTemplateWinOpen()"
                        :disabled="this.ifDisabledExport">{{$t("m.i.common.export")}}</h-button>
              <!--              客户号归并-->
              <h-button type="primary" @click="handleMergeCustNoForm()" v-if="CustMergeIsShow">
                {{ $t("m.i.bm.custNoMerge") }}
              </h-button>
            </div>
          </h-datagrid>
        </div>
    ...
    <h-msg-box v-model="addOrEditWin" width="800" class="h-form-search-layer" :maximize="true" :mask-closable="false"
      ...
      <h-panel>
        <h-form :model="addForm" :label-width="120" ref="addForm" cols="2" class="h-form-search">
          ...
          <h-form-item :label="$t('m.i.common.industryCategory')" prop="industryCategory" >
          ...
          <h-form-item :label="$t('m.i.common.corpScale')" prop="corpScale" >
          ...
          <h-form-item :label="$t('m.i.common.isArc')" prop="isArc" >
          ...
          <h-form-item :label="$t('m.i.common.isGreenCorp')" prop="isGreenCorp" >
          ...
          <h-form-item :label="$t('m.i.common.area')" prop="area" >
          ...
          <h-form-item :label="$t('m.i.bm.custAddr')" prop="custAddr" >
          ...
          <!--主要经营地址-->
          <h-form-item :label="$t('m.i.bm.mainBusiAddr')" prop="mainBusiAddr">
          ...
          <h-form-item :label="$t('m.i.bm.corpLegalName')" prop="corpLegalName">
            <h-input v-if="type==='view'" :value="addForm.corpLegalName" class="input-boder-0"
                     :title="addForm.corpLegalName" :maxlength=200 :lengthByByte="false" readonly></h-input>
            <h-input v-else-if="type==='coll'" v-model="addForm.corpLegalName" :title="addForm.corpLegalName"
                     :maxlength=200 :lengthByByte="false"></h-input>
            <h-input v-else v-model="addForm.corpLegalName" :maxlength=200 :lengthByByte="false"></h-input>
          </h-form-item>
...
export default {
  ...
  methods: {
      ...
      post({id: id}, "/bm/cust/corp/custCorp/func_queryCustCorpById").then(res => {
        ...
        if (res) {
          ...
          if (retCode === "000000") {
            ...
            this.addForm.provinceCode5 = res.data.retData.provinceCode5;
            this.addForm.cityCode5 = res.data.retData.cityCode5;
            this.addForm.areaCode5 = res.data.retData.areaCode5;
            this.addForm.streetCode3 = res.data.retData.streetCode3;
            this.addForm.workAddr1 = res.data.retData.workAddr1;
          } else {
            // this.submitMsg = "查询失败";
            this.$msgTip.error(this, {info: res.data.retMsg});
          }
    ...
    submitForm() {
      if (this.addForm.socCode.length !== 18) {
      this.$msgTip.error(this, {info: "社会信用代码长度不合法，请检查！"});
        return;
      }
