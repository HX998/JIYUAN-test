      ...
      <h-col span="24">
        <div>
          ...
          <h-datagrid :columns="columns"
                      ...
                      :row-select="true"
                      :has-select="false"
                      url="/banks/szbank/ce/acpt/innerBillPayment/guarntrPaymentAccountMain/func_pageQueryGuarntrPaymentAccountList"
                      :bindForm="formItem"
                      ref="datagrid">
            <div slot="toolbar" class="pull-left">
              <h-button type="primary" @click="paymentAccount">{{ $t('m.i.ce.account') }}</h-button>
            </div>
          </h-datagrid>
        </div>
...
export default {
  ...
  methods: {
    ...
    //获取分录信息
    getEntryInfo() {
      this.acctRecordUrl = "/banks/szbank/ce/acpt/innerBillPayment/guarntrPaymentAccountMain/func_generateAcctRecord";
    ...
    acctSubmitSure(info) {
      ...
      post({
        code: this.acptIds,
      }, "/banks/szbank/ce/acpt/innerBillPayment/guarntrPaymentAccountMain/func_acct").then(res => {
