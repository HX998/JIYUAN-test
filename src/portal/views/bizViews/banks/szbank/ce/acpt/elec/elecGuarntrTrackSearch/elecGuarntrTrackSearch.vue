      ...
      <h-col span="24">
        <div>
              ...
              <h-form :model="formItem" :label-width="90" ref="formItem" cols="4" class="h-form-search">
                ...
                <h-form-item prop="acptStatus" :label="$t('m.i.ce.acptStatus')">
                  <h-select v-model="formItem.acptStatus" placeholder="" showTitle>
                    <h-option v-for="item in acptStatusList" :value="item.key" :key="item.key">{{
                        item.value
                      }}
                    </h-option>
                  </h-select>
                ...
                <h-form-item prop="deductStatus" :label="$t('m.i.ce.deductStatus')">
                  <h-select v-model="formItem.deductStatus" placeholder="" showTitle>
                    <h-option v-for="item in deductStatusList" :value="item.key" :key="item.key">{{
                        item.value
                      }}
                    </h-option>
                  </h-select>
                ...
                <!--                  <h-date-picker type="date" v-model="formItem.payRgstDt" placeholder=""-->
                <!--                                 @on-change="handlePayRgstDtChange" :editable=false></h-date-picker>-->
                <!--                </h-form-item>-->

                <h-form-item :label="$t('m.i.billInfo.duePayDt')" prop="paymentDate">
                  <h-date-picker type="daterange" autoPlacement placeholder="" v-model="formItem.paymentDate"
                                 format="yyyy-MM-dd" showFormat @on-change="handlePaymentDtChange"></h-date-picker>
                </h-form-item>
                ...
                <!--                  </h-select>-->
                <!--                </h-form-item>-->
                <h-form-item :label="$t('m.i.ce.guarntrAcctStatus')" prop="acptAcctFlag"
                             v-if="isShowAcctStatus === '1'">
                  <h-select v-model="formItem.acptAcctFlag" placeholder="">
                    <h-option v-for="item in acctStatusList" :value="item.key" :key="item.key">{{
                        item.value
                      }}
                    </h-option>
                  </h-select>
                ...
                <h-form-item :label="$t('m.i.ce.paymentAcctStatus')" prop="paymentAcctFlag" class="h-form-long-label"
                  ...
                  <h-select v-model="formItem.paymentAcctFlag" placeholder="">
                    <h-option v-for="item in payAcctStatusList" :value="item.key" :key="item.key">{{
                        item.value
                      }}
                    </h-option>
                  </h-select>
          ...
          <h-datagrid :columns="columns"
                      highlightRow
                      url="banks/szbank/ce/acpt/search/elecGuarntrTrackSearch/func_trackList"
...
export default {
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          hiddenCol: false,
          render: (h, params) => {
            let list = getDictValueFromMap(this.dictMap, "CorpAcptStatus", params.row.acptStatus);
            return h("span",
              {
                domProps: {
                  title: list
        ...
        {
          title: this.$t("m.i.ce.guarntrAcctStatus"),
          key: 'acptAcctStatus',
          hiddenCol: false,
          render: (h, params) => {
            let list = this.acctStatusList[params.row.acptAcctStatus].value;
            return h("span",
              {
                domProps: {
                  title: list
        ...
        },
        {
          ...
          hiddenCol: false,
          render: (h, params) => {
            let list = this.acctStatusList[params.row.payAcctStatus].value;
            return h("span",
              {
                domProps: {
                  title: list
  ...
  mounted() {
    ...
    getDictListByGroups("CorpAcptStatus,Yon,AcctFlag,DeductStatusType,BillOrigin,DraftTypeCode").then(res => {
      this.acptStatusList = res.get("CorpAcptStatus");
    ...
    this.acctStatusList.push(item4);

    this.payAcctStatusList = this.acctStatusList.slice(0, 3)
    this.getParams();
  },
  methods: {
    ...
    getList() {
      ...
      let url = "banks/szbank/ce/acpt/search/elecGuarntrTrackSearch/func_trackList";
    ...
    //清单导出
    exportData() {
      this.param = 'elecAcptGuarntrTrackSearchExport';
    ...
    exportList(field, exportType) {
      ...
      let url = window.LOCAL_CONFIG.API_HOME + "/banks/szbank/ce/acpt/search/elecGuarntrTrackSearch/func_exportDataForEAcptGuarntrTrackSearch";
      if (exportType === "1") {
        let params = null;
        if (this.$refs.datagrid.selectIds.length != 0) {
          params = {
    ...
    getParams() {
      if (this.$route.path === "banks/szbank/ce/acpt/search/elecGuarntrTrackSearch") {
