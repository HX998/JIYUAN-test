    ...
    <div class="text-wrap mb-10">
      ...
      <div class="text-body">
        <h-row :gutter="8">
          <h-col span="8">
            <h-row>
              <h-col span="9" class="label">{{$t('m.i.ce.contractNo')}}：</h-col>
              <h-col span="15" class="val">{{ viewDetailForm.acptProtocalNo }}</h-col>
            </h-row>
          </h-col>
          <h-col span="8">
            <h-row>
              <h-col span="9" class="label">{{$t('m.i.ce.warteeCustNo')}}：</h-col>
              <h-col span="15" class="val">{{ viewDetailForm.drwrCustNo }}</h-col>
            </h-row>
          </h-col>
          <h-col span="8">
            <h-row>
              <h-col span="9" class="label">{{$t('m.i.ce.warteeName')}}：</h-col>
        ...
        <h-row :gutter="8">
          <h-col span="8">
            <h-row>
              <h-col span="9" class="label">{{$t('m.i.ce.warteeAcctNo')}}：</h-col>
              <h-col span="15" class="val">{{ viewDetailForm.drwrAcctNo }}</h-col>
            </h-row>
          </h-col>
          <h-col span="8">
            <h-row>
              <h-col span="9" class="label">{{$t('m.i.ce.guarntrApplyDt')}}：</h-col>
              <h-col span="15" class="val">{{ viewDetailForm.guarntrDt == null ? "" : this.$moment(viewDetailForm.guarntrDt,
...
<script>
  ...
  export default {
    name: "eGuarntrAcptBatchInfoDetail",
    data() {
      return {
        tempIsShow: false,
        dictMap: new Map(),
          ...
          if(res) {
            ...
            if (retCode == "000000") {
              if(res.data.retData !== null) {
                ...
                this.viewDetailForm.guarntrDt = res.data.retData.guarntrDt;
                ...
                this.viewDetailForm.guarntrNo = res.data.retData.guarntrNo;
