...
export default {
          ...
          render: (h, params) => {
            return h('div', [
              h('a', {
                ...
                on: {
                  click: () => {
                    this.batchNoMain(params.row.id,params.row.ownedBrchNo,params.row.batchNo)
                  }
                }
              }, params.row.batchNo)
            ]);
    ...
    methods: {
      ...
      billManager() {
        ...
        this.batchNo = this.currentSelectRow.batchNo;
      ...
      //承兑协议编号超链接
      batchNoMain(str,ownedBrchNo,batchNo) {
        this.batchId = str;
        this.billManagerFormItem.batchId = this.batchId;
      ...
      handleBillPrintConfirm(vocherData) {
        ...
        for (let j = 0; j < vocherData.length; j++) {
          ...
          printData.batchId=this.batchId;
          printData.listId= vocherData[j].id;
          printData.busiType="acpt";
          printData.flowNo = vocherData[j].flowNo;
          printData.acptProtocalNo = vocherData[j].acptProtocalNo;
          printData.batchNo = this.batchNo;
          ...
          printData.printObj = JSON.stringify(printData);
      ...
      billPrintSuccessConfirm() {
        post({
          ...
          if (res) {
            ...
            let retCode = res.data.retCode;
            if (retCode == "000000") {
              let param = {listIds: this.acptIds};
              let params = {templateName: "BE030405", params: JSON.stringify(param),saveFlag:1,batchId: this.batchId,batchNo:this.batchNo};
              this.$print.vouchTemplatePrint(this, {
                ...
                errorCallback: () => {
                  this.$msgTip.error(this, { info: "银承签发凭证打印失败,请重新尝试" });
                  this.$refs.acptBatchInfoDetail.getBatchInfo();
                  this.$refs.billManagerDatagrid.dataChange(this.$refs.billManagerDatagrid.$refs.gridPage.currentPage);
                  this.$refs.billManagerDatagrid.selects = [];
                  this.printBillNos = "";
