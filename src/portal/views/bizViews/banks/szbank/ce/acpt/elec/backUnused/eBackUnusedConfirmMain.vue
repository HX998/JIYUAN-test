...
export default {
    ...
    methods: {
      ...
      hasCommitButton() {
        ...
        this.formItem.applTellerNoNotEqual = this.$store.getters.userInfo.userNo;
        this.formItem.auditModeKey = 'ce.acpt.drwgback.is_check_exclude';
        ...
        if(this.$refs.datagrid.selects.length === 0) {
          ...
          this.$hMsgBox.confirm({
            ...
            onOk: () => {
              ...
              post({transIds: ids, billOperType: '1'}, "/ce/acpt/elec/backUnused/eBackUnusedConfirmMain/func_elecDrwgBackSubmit").then(res => {
                if (res) {
                  let retCode = res.data.retCode;
                  if (retCode === "000000") {
                    this.$msgTip.success(this);
