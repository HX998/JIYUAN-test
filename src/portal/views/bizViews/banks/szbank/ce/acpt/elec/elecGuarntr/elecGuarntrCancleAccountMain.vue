      ...
      <h-col span="24">
        ...
        <div v-if="mainVue" key="1">
          ...
          <h-datagrid :columns="columns"
                      url="banks/szbank/ce/acpt/account/elecGuarntrCancleAccountMain/func_queryPageAcptGUarntrCancelAccountBatch"
                      :bindForm="formItem"
                      highlightRow
        ...
        <e-guarntr-acpt-batch-info-detail :is-show="billManagerVue" :params="batchInfoParams"
                                          :batchUrl="'/banks/szbank/ce/acpt/account/elecGuarntrCancleAccountMain/func_getAcptGuarntrBatchByQueryDto'"
        ...
        <div v-if="billManagerVue" key="2">
          <h-datagrid :columns="billManagerColumns"
                      url="/banks/szbank/ce/acpt/account/elecGuarntrCancleAccountMain/func_queryPageAcctFlow"
                      :auto-load="false"
                      highlightRow
                      :row-select="true"
                      :has-select="false"
            ...
            <div slot="toolbar" class="pull-left">
              <h-button type="primary" @click="cancelAccountDetail('batch')">{{ $t('m.i.ce.batchCancelAccount') }}
              </h-button>
        ...
        <!--显示流水明细界面-->
        <h-msg-box v-model="acctFlowDetailWin" width="1000" :mask-closable="false" @on-close="acctFlowDetailClose"
                   top="10"
        ...
        <!-- 查看票据明细弹窗 -->
        <show-bill-detail :bill-detail-win="billDetailWin" :batch-id="this.batchId" :acctFlowIds="this.infoIds"
                          bill-class="ME02" @billDetailWinClose="billDetailWinClose" :licenseFlag="licenseFlag"
                          :ownedBrchNo="ownedBrchNo"
                          listUrl="banks/szbank/ce/acpt/account/elecGuarntrCancleAccountMain/func_queryPageAcptGuarntrBill"></show-bill-detail>
      </h-col>
    </h-row>
    <cancel-acct-print :showCancelAcctPrintWin="showCancelAcctPrintWin" :frontFlowNos="frontFlowNo"
                       @showCancelAcctPrintWinClose="showCancelAcctPrintWinClose"></cancel-acct-print>
...
export default {
  name: "elecGuarntrCancleAccountMain",
  components: {
    ShowBillDetail: () => import(/* webpackChunkName: "banks/szbank/ce/ce/acpt/common/showBillDetail" */`@/views/bizViews/banks/szbank/ce/acpt/common/showBillDetail`),
    ...
    batchCancelAccount() {
      this.$hMsgBox.confirm({
        ...
        onOk: () => {
          ...
          }, "/banks/szbank/ce/acpt/account/elecGuarntrCancleAccountMain/func_cancelAccount").then(res => {
            if (res) {
              let msg = res.data.retMsg
              let retCode = res.data.retCode
              if (retCode == "000000") {
