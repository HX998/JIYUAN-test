      ...
      <h-col span="24">
        ...
        <div v-if="mainVue" key="1">
          ...
          <h-datagrid :columns="columns"
                      highlightRow
                      url="/ce/acpt/paper/account/cancelAcptAccountMain/func_queryPageAcptCancelAccountBatch"
                      :bindForm="formItem"
                      :on-current-change="handleRowClick"
                      :on-current-change-cancel="handleRowClickCancel"
                      ref="datagrid">
            <div slot="toolbar" class="pull-left">
              <h-button type="primary" @click="cancelAccountMain()">{{$t('m.i.ce.cancelAccount')}}</h-button>
            </div>
          </h-datagrid>
        </div>
        <!-- 子页面 -->
        <acpt-batch-info-detail :is-show="billManagerVue" :params="batchInfoParams"
                                :batchUrl = "'/ce/acpt/paper/account/cancelAcptAccountMain/func_getAcptBatchByQueryDto'"
                                ref="acptBatchInfoDetail"></acpt-batch-info-detail>
        <!-- 数据展示表格 -->
        <div v-if="billManagerVue" key="2">
          <h-datagrid :columns="billManagerColumns"
                      url="/ce/acpt/paper/account/cancelAcptAccountMain/func_queryPageAcctFlow"
                      :row-select="true"
                      :has-select="false"
                      :auto-load="false"
            ...
            <div slot="toolbar" class="pull-left">
              <h-button type="primary" @click="batchCancelAccount" >{{$t('m.i.ce.batchCancelAccount')}}</h-button>
        ...
        <!-- 查看票据明细弹窗 -->
        <show-bill-detail :bill-detail-win="billDetailWin" :batch-id="this.batchId" :acctFlowIds="this.infoIds" bill-class="ME01" @billDetailWinClose="billDetailWinClose" listUrl="/ce/acpt/paper/account/cancelAcptAccountMain/func_queryPageAcptBill"></show-bill-detail>
      </h-col>
    </h-row>
...
<script>
  import { post, on, off, getDictListByGroups, getDictValueFromMap, formatNumber,getSingleParamValuesByKeys } from "@/api/bizApi/commonUtil";
  import {deepClone} from "@/utils/index";

  export default {
    name: "cancelAcptAccountMain",
    components: {
      ...
      batchCancelAccount() {
        this.$hMsgBox.confirm({
          ...
          onOk: () => {
            ...
            post({batchId: this.batchId, billOperType: "1"}, "/ce/acpt/paper/account/cancelAcptAccountMain/func_cancelAccount").then(res => {
              if(res) {
      ...
      selectCancelAccount() {
        ...
        this.$hMsgBox.confirm({
          ...
          onOk: () => {
            ...
            post({batchId: this.batchId, billOperType: "0", ids: ids}, "/ce/acpt/paper/account/cancelAcptAccountMain/func_cancelAccount").then(res => {
              if(res) {
                let msg = res.data.retMsg
                let retCode = res.data.retCode
                if (retCode == "000000") {
