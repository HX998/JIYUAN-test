    ...
    <h-row name="flex" class="layout-menu-wrapper">
      <h-col span="24">
        ...
        <!--        <h-form-item :label="$t('m.i.common.brchName')" prop="acptBrchNoList">
                ...
                <show-branch v-model="formItem.acptBrchNameList" :label="$t('m.i.common.brchName')" prop="acptBrchNameList" isNeedChecked
                ...
                <h-form-item :label="$t('m.i.billInfo.acptDt')" prop="minAcptDt">
                  <h-date-picker :value="acptDt" type="daterange" autoPlacement  showFormat :editable=false @on-change="handleAcptDtChange" placeholder=""></h-date-picker>
                </h-form-item>
                ...
                <h-form-item :label="$t('m.i.billInfo.payDt')" prop="paymentDtRange">
                  <h-date-picker :value="paymentDtRange" type="daterange" autoPlacement  showFormat :editable=false @on-change="handlePaymentDtChange" placeholder=""></h-date-picker>
                </h-form-item>
                ...
                <h-form-item :label="$t('m.i.pe.payRgstDt')" prop="payRgstDtRange">
                  <h-date-picker :value="payRgstDtRange" type="daterange" autoPlacement  showFormat :editable=false @on-change="handlepayRgstDtRange" placeholder=""></h-date-picker>
                </h-form-item>
...
export default {
    ...
    data() {
      let batchColumns = [
        ...
        {
          title: this.$t("m.i.pe.acptBrchAreaName"),
          key: "acptBrchAreaName",
          hiddenCol: false,
          sortable: true
        },
        {
      ...
      return {
        ...
        billColumns: [
          ...
          {
            title: this.$t("m.i.ce.commitFeeAmtUnit"),
            key: "commitFee",
            hiddenCol: false,
            sortable: true,
            render: (h, params) => {
              let commitFeeAmt = formatNumber(params.row.commitFee, 2, ",");
              return h("span", {
                domProps: {
                  title: commitFeeAmt
                }
          ...
          // {
          //   title: this.$t("m.i.pe.receiptType"),
          //   key: "receiptType",
          //   hiddenCol: false,
          //   sortable: true
          // },
          {
            title: this.$t("m.i.pe.receiptAmt"),
            key: "reserve2",
            hiddenCol: false,
            sortable: true,
            render: (h, params) => {
              let reserve2 = formatNumber(params.row.reserve2, 2, ",");
              return h("span", {
                domProps: {
                  title: reserve2
                }
    ...
    mounted() {
      this.getOccurData();
      this.getDictListByGroups("AcptType,AcptStatus,DeductStatusType,LabelTypeClass,CDMedia,Yon,AcptRgstStatus,AcptCpesRgstType,InOutFlagCode,BillOrigin,GuarntrType").then(res => {
