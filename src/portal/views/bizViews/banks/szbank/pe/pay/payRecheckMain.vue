      ...
      <h-col span="24">
        <div>
          ...
          <h-datagrid :columns="columns"
            ...
            <div slot="toolbar" class="pull-left">
              ...
              <h-button type="primary" @click="backToRePay" :disabled="this.isDisabledPayReBack">{{$t("m.i.pe.backToRePay")}}</h-button>
            </div>
          </h-datagrid>
        </div>
        ...
        <h-msg-box v-model="inputRecheckWin" width="800" @on-close="inputRecheckWinClose" class="h-form-search-layer"
          ...
          <div>
            <h-form :model="inputRecheckForm" :label-width="115" ref="inputRecheckForm" cols="2" class="h-form-search"
              ...
              <h-form-item prop="pyeeBankNo" :label="$t('m.i.billInfo.pyeeBankNo')" required :validRules="bankNoRule">
                <h-input v-model="inputRecheckForm.pyeeBankNo" placeholder="" :maxlength="12"  :readonly="!isShowBackButton"></h-input>
              </h-form-item>
              <h-form-item prop="payAmt" :label="$t('m.i.pe.payAmt')" required >
                <h-typefield v-model="inputRecheckForm.payAmt" type="money" bigTips placeholder="" :readonly="!isShowBackButton"></h-typefield>
              </h-form-item>
          ...
          <div style="text-align: center">
            <h-button type="primary" @click="recheckNoPassAndBack" size="large" v-if=this.isShowBackButton>{{$t("m.i.pe.recheckNoPassAndBack")}}</h-button>
  ...
  export default {
    ...
    methods: {
      ...
      handleCurrentChange(currentRow) {
        ...
        if (this.currentSelectRow.prodNo !=='01010211') {
          this.isDisabledPayReBack = true;
        } else {
          this.isDisabledPayReBack = false;
        }
      },
        ...
        post({ id: id }, "/pe/pay/payRecheckMain/func_queryCheckPayInfoById").then(res => {
          if (res) {
            ...
            if (retCode === "000000") {
              ...
              if (res.data.retData.prodNo === "01010211") {
            ...
            post(this.inputRecheckForm, url).then(res => {
              if (res) {
                ...
                if (retCode === "000000") {
                  ...
                  this.printVoucher(this.inputRecheckForm);
      ...
      backToRePay() {
        ...
        let id = this.currentSelectRow.id;
        let url = "/banks/szbank/ce/acpt/paper/paymentDue/cancelPaymentAccountMain/func_cancelAcct";
        let content = "确认退回重解付吗？";
    ...
    //复核不通过作废提醒
      ...
      printVoucher(recheckForm){
        let param = {listIds: recheckForm.id};
        let params = {templateName: "BE031808", params: JSON.stringify(param),listIds: recheckForm.id,saveFlag:1,batchNo:recheckForm.batchNo};
