<template>
  <div class="app-login" v-if="!loginCas">
    ...
    <div class="login-header">
      <span class="login-header-title">{{this.productName}}</span>
      <span class="login-header-version">{{this.productVersion}}</span>
    </div>
      ...
      post({loading: false}, "/sm/auth/login/queryVersion").then(res => {
        if (res) {
          ...
          if (retCode === "000000") {
            ...
            this.productVersion = obj.productVersion ? obj.productVersion : this.$t("m.i.topBar.version");
            if (typeof (bankName) === "string" && bankName !== "" && bankName !== "default") {
              import(`@/views/bizViews/banks/${bankName}/style/style.scss`).catch(err => {});
              import(/* webpackChunkName: "banks/locale/[request]"  */`@/views/bizViews/banks/${bankName}/locale/lang/zh-CN.js`).then((info) => {
