<template>
  <div id="stockBillMoneyReport">
    <div class="h-form-search-box">
      <h-panel class="clearfix" :class="{'h-form-overhd':!ifShowMore}">
        <h-form :model="formItem" :label-width="90" ref="formItem" cols="4" class="h-form-search">
          <h-form-item :label="$t('m.i.pc.reportDt')" prop="reportDt">
            <h-date-picker :value="formItem.reportDt" type="date" placeholder="" readonly :editable=false
                           :showFormat=true @on-change="handleDateChange"></h-date-picker>
          </h-form-item>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        formItem: {
          ...
          reportCnName: "",
          brchNo: "",
          reportName: "stockBillMoneyReport",
    ...
    created() {
      this.formItem.reportDt = window.sessionStorage.getItem("workDate");
    },
    methods: {
      createReport() {
        let reportDt = this.formItem.reportDt.replace(/-/g, "");
        let params = { creatorNo: window.sessionStorage.getItem("userName"), reportDt: reportDt };
        reportFunc.createReport(this, "/pc/report/stockBillMoneyReport/func_sendQueryReport", params);
      },
      ...
      downReport(id, status) {
        reportFunc.downReport(this, {
          status: status,
          downReportUrl: "/pc/report/stockBillMoneyReport/func_stockReport",
          urlSuffix: "reportId=" + id + "&userNo=" + window.sessionStorage.getItem("userName"),
          downReportTitle: "库存票据结构(金额)报表"
        });
      }
    }
  };
