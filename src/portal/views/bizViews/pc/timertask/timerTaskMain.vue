      ...
      <h-col span="24">
        <div>
          ...
          <h-datagrid :columns="columns"
                      highlight-row
                      url="/pc/timertask/timerTaskMain/func_queryTimerTaskInfo"
                      :bindForm="formItem"
      ...
      <div>
        <h-form :model="addForm" :label-width="120" ref="addForm" cols="2" class="h-form-search">
          ...
          <h-form-item :label="$t('m.i.pc.timingFlag')" prop="timingFlag" :required="isRequired">
            <h-select v-model="addForm.timingFlag" :placeholder="'是否开启定时开关'" readonly>
              <h-option v-for="item in isList" :value="item.key" :key="item.key">{{ item.value }}</h-option>
            </h-select>
          </h-form-item>
          <h-form-item :label="$t('m.i.pc.cronExpression')" prop="cronExpression" :required="isRequired">
            <h-input v-model="addForm.cronExpression"></h-input>
          </h-form-item>
          <h-form-item :label="$t('m.i.pc.repeatFlag')" prop="repeatFlag" :required="isRequired">
            <h-select v-model="addForm.repeatFlag" :placeholder="'当日是否允许重复执行'" readonly>
              <h-option v-for="item in isList" :value="item.key" :key="item.key">{{ item.value }}</h-option>
            </h-select>
          </h-form-item>
...
<script>
  ...
  export default {
      ...
      return {
        columns: [
          ...
          },
          {
            ...
            hiddenCol: false,
            render: (h, params) => {
              let dictValue = getDictValueFromMap(this.dictMap, "TimerTaskProcessStatus", params.row.processStatus);
              return h("span", { domProps: { title: dictValue } }, dictValue);
            }
          }
        ],
    ...
    methods: {
      ...
      submitSeqNoEdit() {
        this.$refs["seqNoForm"].validate(valid => {
          if (valid) {
            ...
            let url = "/pc/timertask/timerTaskMain/func_updateTimerTaskInfo";
      ...
      addOrEditFormSave() {
        this.$refs["addForm"].validate(valid => {
          if (valid) {
            let url = this.type == "add" ? "/pc/timertask/timerTaskMain/func_addTimerTaskInfo" : "/pc/timertask/timerTaskMain/func_updateTimerTaskInfo";
      ...
      handleDel() {
        let list = this.currentSelectRow;
        post({ id: list.id }, "/pc/timertask/timerTaskMain/func_deleteTimerTaskInfo").then(res => {
          if (res) {
            let retCode = res.data.retCode;
            if (retCode == "000000") {
              this.$msgTip.success(this);
      ...
      handleExec() {
        ...
        if (list != null && list.length !== 0) {
          ...
          post(this.currentSelectRow, "/pc/timertask/timerTaskMain/func_runImmediatelyTimerTask").then(res => {
            if (res) {
              let retCode = res.data.retCode;
              if (retCode == "000000") {
    ...
    mounted() {
      getDictListByGroups("Yon,TimerTaskProcessStatus").then(res => {
        this.isList = res.get("Yon");
        this.statusList = res.get("TimerTaskProcessStatus");
