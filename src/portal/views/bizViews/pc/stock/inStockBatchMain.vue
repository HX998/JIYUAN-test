<template>
  ...
  <div class="layout">
    <h-row name="flex" class="layout-menu-wrapper">
      <h-col span="24">
        <div v-if="inStockBatchMain" key="1">
          <div>
            <!--查询表单-->
            <div class="h-form-search-box">
              ...
              <h-datagrid
                ...
                highlight-row
                :bindForm="formItem"
                url="/pc/stock/inStockBatchMain/func_queryStockBatchInfoByPage"
        ...
        <div v-if="inStockAccountBillMain" key="2">
          <!--查询表单-->
          <div class="h-form-search-box">
        ...
        <stock-batch-info-detail :is-show="inStockAccountBillMain" :params="this.params"
                                  :dictMap="dictMap"
                                 ref="stockBatchInfoDetail"></stock-batch-info-detail>
        <!-- 数据展示表格 -->
        <div v-if="inStockAccountBillMain" key="3">
          <h-datagrid
            :columns="billColumns"
            highlight-row
            :auto-load="false"
            ...
            <div slot="toolbar">
              <h-dropdown placement="bottom-start" trigger="click" @on-click="inStockSubmit">
                <h-button type="primary">入库提交<h-icon name="unfold"></h-icon></h-button>
              ...
              <h-dropdown placement="bottom-start" trigger="click" @on-click="accountBackList">
                <h-button type="primary">入库退回<h-icon name="unfold"></h-icon></h-button>
                <h-dropdown-menu slot="list"  >
  ...
  export default {
    ...
    data() {
      ...
      return {
        ...
        dictMap: new Map(),
        inStockBatchMain : true,
        inStockAccountBillMain : false,
    ...
    methods: {
      ...
      handleForm() {
        ...
        this.inStockBatchMain = false;
        this.inStockAccountBillMain = true;
        this.$nextTick(() => {
      ...
      returnBack() {
        ...
        this.ownedBrchNo="";
        this.resetSearch();
        this.inStockAccountBillMain = false;
        this.inStockBatchMain = true;
        this.$nextTick(() => {
      ...
      acctSubmitSure(info) {
        ...
        post(param, "/pc/stock/inStockBatchMain/func_submitAccount").then(res => {
      ...
      accountSelectBatchBack(){
        ...
        this.$hMsgBox.confirm({
          ...
          content: "确定选择记账退回吗?",
          onOk: () => {
            post({batchId:this.stockBatchId , ids:ids ,pageOperType:"0",busiType: this.formItem.busiType,licenseFlag:this.licenseFlag,ownedBrchNo:this.ownedBrchNo}, "/pc/stock/inStockBatchMain/func_stockBack").then(res => {
              if(res) {
                let msg = res.data.retMsg;
                let retCode = res.data.retCode;
                if (retCode === "000000") {
      ...
      handleAccountBatchBack() {
        post({ batchId:this.stockBatchId ,pageOperType:"1",busiType:this.formItem.busiType,licenseFlag:this.licenseFlag,ownedBrchNo:this.ownedBrchNo}, "/pc/stock/inStockBatchMain/func_stockBack").then(res => {
          if (res) {
            let msg = res.data.retMsg;
            let retCode = res.data.retCode;
            if (retCode === "000000") {
      ...
      /*直接调用入库方法*/
      inStock(params){
        post(params, "/pc/stock/inStockBatchMain/func_inStock").then(res=> {
          let msg = res.data.retMsg;
          let retCode = res.data.retCode;
          if (retCode === "000000") {
            this.$msgTip.success(this);
      ...
      acctAuthorize(){
        let param = {};
        if(this.acctType === "batch") {
          param = {batchId: this.stockBatchId, pageOperType: "1",busiType:"1",licenseFlag:this.licenseFlag,ownedBrchNo:this.ownedBrchNo};
        }else if (this.acctType === "select") {
          param = {batchId: this.stockBatchId, pageOperType: "0", ids: this.stockBillIds,busiType:"1",licenseFlag:this.licenseFlag,ownedBrchNo:this.ownedBrchNo};
        }
        this.acctSubmitFlag = true;
        post(param, "/pc/stock/inStockBatchMain/func_addAcctAuthorize").then(res => {
          this.acctSubmitFlag = false;
          if(res) {
            let msg = res.data.retMsg;
            let retCode = res.data.retCode;
