<!--查看菜单-->
<template>
  <h-msg-box v-model="tempShowLicenseMenuWin" :mask-closable="false" @on-close="handleClose" class="h-form-search-layer"
             :maximize="true" @on-maximize="getTreeHeight($event, 'showLicenseMenuMsg')" ref="showLicenseMenuMsg">
    <p slot="header">
      <span>{{title}}</span>
    </p>
...
<script>
  ...
  export default {
    ...
    computed: {
      tempShowLicenseMenuWin: {
        get() {
          return this.showLicenseMenuWin;
        },
        set() {
        }
      }
    ...
    methods: {
      ...
      //获取机构角色对应菜单
      getBrchMenuTreeList() {
        let params = this.roleType === "1" ? { licenseBrchNo: this.licenseBrchNo,licensedBrchNo: this.licensedBrchNo } : { userNo: this.userNo };
        let url = this.roleType === "1" ? "/sm/auth/licenseBrch/focusOper/func_queryLicenseAuth" : "/sm/auth/branchUser/queryUserAuthority";
      ...
      handleClose() {
        this.$emit("showLicenseMenuWinClose", "");
      },
