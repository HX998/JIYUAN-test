<!--机构查询-->
<template>
  <h-msg-box v-model="tempShowBranchWin" :mask-closable="false" @on-close="handleClose" class="h-form-search-layer"
             :z-index=1200 :maximize="true" @on-maximize="getTreeHeight($event, 'showBranchMsg')" ref="showBranchMsg">
    <p slot="header">
      <span>{{title}}</span>
    </p>
    <div class="h-tree-search">
      ...
      <div class="h-tree-content" style="position:relative;" :style="{height: mTreeHeight + 'px'}">
        <h-tree v-if="ifcheck" :data="branchList" ref='branchTree' class='h-tree-self' selectToCheck show-checkbox
                :checkStrictly="checkStrictly" :onlyUpdateDown="onlyUpdateDown" :multiple="multiple" notDeepCopy @on-toggle-expand="toggleExpand"></h-tree>
  ...
  export default {
    ...
    props: {
      ...
      treeType:{
        type: String,
        default() {
          return "1";//1:查询所有机构树  ，2：查询子机构树 ，3:查询父机构树
        }
      },
      brchNoQry:{
        tyep: String,
    ...
    methods: {
      ...
      getBranchTreeList() {
        ...
        let url = this.url;
        if(this.treeType === "3"){
          url = "/sm/auth/branch/showBranch/func_queryParentBranchTree";
          params.brchNo = this.brchNoQry;
        }
      ...
      submitForm() {
        let selectedNodes = [];
        if (this.ifcheck) {
          selectedNodes = this.$refs["branchTree"].getCheckedNodes();
        } else {
          selectedNodes = this.$refs["branchTree"].getSelectedNodes();
        }
