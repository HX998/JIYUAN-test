    ...
    <div class="h-tree-search">
      ...
      <div class="h-tree-content" style="position:relative;" :style="{height: mTreeHeight + 'px'}">
        <h-tree v-if="ifcheck" :data="branchList" ref='branchTree' class='h-tree-self' selectToCheck show-checkbox
                :checkStrictly="checkStrictly" :multiple="multiple" notDeepCopy @on-toggle-expand="toggleExpand"></h-tree>
  ...
  export default {
    ...
    props: {
      ...
      url: {
        type: String,
        default() {
          return "sm/auth/licenseBrch/focusOper/func_queryLicenseBrchTree";
        }
      },
      authPath:{
        type: String,
    ...
    methods: {
      ...
      //获取机构
      getBranchTreeList() {
        let params = {authPath:this.authPath};
        let url = this.url;
