  ...
  export default {
    ...
    methods: {
      ...
      getPermissionTreeList() {
        let params = window.sessionStorage.getItem("roleRight");
        let json = JSON.parse(params);
        post({ "authorityType": "4", "roleId": this.roleId }, "/sm/auth/authority/queryAssignedRoleAuths").then(res => {
          if (res && res.data) {
      ...
      renderSearchTree(h, { root, node, data }) {
        ...
        // 保存首次数据
        if (this.firstRender) {
          this.treeOldArray = deepClone(root);
    ...
    mounted() {
      // let params = JSON.parse(window.sessionStorage.getItem("roleRight"));
