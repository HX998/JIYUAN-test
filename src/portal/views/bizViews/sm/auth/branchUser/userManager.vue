    ...
    <h-msg-box v-model="addOrEditWin" width="800" :maximize="true" @on-close="handleWinClose">
      ...
      <!--    <h-form-item :label="$t('m.i.common.brchNo')" prop="brchNo" :required="isRequired">
          ...
          <show-branch v-model="addForm.brchNo"  :label="$t('m.i.common.brchNo')"  showValue prop="brchNo" :required="isRequired"
                       :filterable="(type==='add'||type==='syncUser'||(type==='modify' && modityBrch))&&isCurBrchNo"   :showCheckBox="false"  :brchNo.sync="addForm.brchNo" :brchName.sync="addForm.brchName"   :msgBoxWin ="addOrEditWin"
      ...
      handleAddForm(str) {
        ...
        if (this.type == "modify" || this.type == "view") {
          ...
          if (this.type == "modify") {
            ...
            // 是否可修改机构
            this.modityBrch = this.btnAuth.modifyBrch === undefined ? false : this.btnAuth.modifyBrch.isShow;
            if (this.modityBrch) {
              this.showIcon=true;
              this.brchNoReadonly=false;
              this.isCurBrchNo=true;
