          ...
          <h-datagrid
            ...
            <div slot="toolbar" class="pull-left" v-if="this.isShowToolbar">
              ...
              <h-button type="primary" @click="handleAddForm('add')"
                        v-if="authObj.bizSetRoleAdd && (this.btnAuth.add === undefined ? true : this.btnAuth.add.isShow)"
                        :disabled="this.deletDisabled">
                {{ $t("m.i.common.add") }}
              </h-button>
    ...
    <h-msg-box v-model="addOrEditWin" width="800" class="h-form-search-layer" :maximize="true" :mask-closable="false"
      ...
      <h-panel>
        <h-form :model="addForm" :label-width="120" ref="addForm" cols="2" class="h-form-search">
          ...
          <!--是否短信提醒-->
          <h-form-item :label="$t('是否短信提醒')" prop="isMsgNotice">
            <h-select v-if="type==='view'" :value="addForm.isMsgNotice" class="input-boder-0" :title="addForm.isMsgNotice"
                      placeholder="" :readonly="addForm.phone===''">
              <h-option v-for="item in isList" :value="item.key" :key="item.key">{{ item.value }}</h-option>
            </h-select>
            <h-select v-else-if="type==='coll'" v-model="addForm.isMsgNotice" :title="addForm.isMsgNotice" placeholder=""
                      :readonly="addForm.phone===null || addForm.phone===''" >
              <h-option v-for="item in isList" :value="item.key" :key="item.key">{{ item.value }}</h-option>
            </h-select>
            <h-select v-else v-model="addForm.isMsgNotice" :maxlength=60 placeholder=""
                      :readonly="addForm.phone===null || addForm.phone===''">
              <h-option v-for="item in isList" :value="item.key" :key="item.key">{{ item.value }}</h-option>
            </h-select>
          </h-form-item>
...
export default {
  ...
  data() {
    let columns = [
      ...
      // 是否发送短信
      {
        ...
        hiddenCol: false,
        render: (h, params) => {
          let dictValue = this.getDictValueFromMap(this.dictMap, "Yon", params.row.isMsgNotice);
          return h("span", {domProps: {title: dictValue}}, dictValue);
        },
      },
  ...
  methods: {
    ...
    addFormResetWithOutCollType() {
      ...
      this.addForm.custPartnerType = "RC01";
      this.isUpdateMainGroupCus = false;
      this.addForm.isMsgNotice = "0";// 是否发送短信
    },
      ...
      post({id: id}, "/bm/cust/corp/custCorp/func_queryCustCorpById").then(res => {
        ...
        if (res) {
          ...
          if (retCode === "000000") {
            ...
            this.addForm.isMsgNotice = res.data.retData.isMsgNotice === null ? "0" : res.data.retData.isMsgNotice;// 是否发送短信
          } else {
            // this.submitMsg = "查询失败";
            this.$msgTip.error(this, {info: res.data.retMsg});
          }
  ...
  mounted() {
    ...
    this.getDictListByGroups("CorpScale,Yon,partnerType,Province,Industry,CustCollType,CertificateCodeType,CpesCorpNature,CertType").then(res => {
