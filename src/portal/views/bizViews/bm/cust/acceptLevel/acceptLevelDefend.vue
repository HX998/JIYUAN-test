      ...
      <h-col span="24">
        <div>
            ...
            <h-panel class="clearfix" :class="{'h-form-overhd':!ifShowMore}">
              <h-form :model="formItem" :label-width="90" ref="formItem" cols="4" class="h-form-search">
                <h-form-item :label= "$t('m.i.pc.custType')"  prop="custType">
                  <h-select v-model="formItem.custType">
          ...
          <h-datagrid
              :columns="columns"
              highlight-row
              url="/bm/cust/pageQueryAcceptLevelDefendList"
              :bindForm="formItem"
              :hasSelect="false"
              rowSelect
              ref="datagrid">
      ...
      <h-panel>
        <h-form :model="addForm" :label-width="115" ref="addForm" cols="" class="h-form-search">
          ...
          <div class="h-search-form-row">
            <h-form-item :label="$t('m.i.pc.custType')" prop="custType" :required="this.isCustTypeRequired">
              <h-select v-if="type==='modify'" v-model="addForm.custType" readonly>
                <h-option v-for="item in typeList" :value="item.key" :key="item.key">{{ item.value }}</h-option>
              </h-select>
              <h-select v-else v-model="addForm.custType">
                <h-option v-for="item in typeList" :value="item.key" :key="item.key">{{ item.value }}</h-option>
              </h-select>
            </h-form-item>
          </div>
          <div class="h-search-form-row">
            <h-form-item :label="$t('m.i.common.acptBankLevel')" prop="acceptLevelName" :required="this.isRequired">
              <h-input v-model="addForm.acceptLevelName" :maxlength="32"></h-input>
            </h-form-item>
          </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      queryObjById(id) {
        post({ id: id }, this.reqUrl + "/queryAcceptLevelById").then(res => {
          ...
          if (res) {
            ...
            if (retCode === "000000") {
              ...
              this.addForm.acceptLevel = res.data.retData.acceptLevel;
              this.addForm.acceptLevelName = res.data.retData.acceptLevelName;
            } else {
      ...
      addFormReset() {
        ...
        this.addForm.custType = "";
        this.addForm.acceptLevelName = "";
        if (this.type === "modify" && this.isAllowContinue()) {
          this.queryObjById(this.$refs.datagrid.selectIds[0]);
        }
      },
      ...
      submitForm() {
        ...
        this.$refs["addForm"].validate(valid => {
          if (valid) {
            let url = this.type === "modify" ? this.reqUrl + "/updateAcceptLevel" : this.reqUrl + "/insertAcceptLevel";
      ...
      //提交删除请求
      handledel() {
        let url = this.reqUrl + "/deleteAcceptLevel";
    ...
    mounted() {
      this.getDictListByGroups("CreditCustType").then(res => {
        this.typeList = res.get("CreditCustType");
