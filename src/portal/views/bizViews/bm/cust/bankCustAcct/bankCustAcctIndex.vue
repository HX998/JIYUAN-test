            ...
            <h-panel class="clearfix" :class="{'h-form-overhd':!ifShowMore}">
              <h-form :model="queryFormItem" :label-width="90" ref="queryFormItem" cols="4" class="h-form-search">
                <!--<h-form-item :label="$t('m.i.common.brchName')" prop="openBrchName">
                  <h-input v-model="queryFormItem.openBrchName" readonly :maxlength="60"
                           icon="android-search" @on-click="queryRcvBrchName()" clearable @on-clear="clearRcvBrchName"></h-input>
                </h-form-item>-->
                <show-cpes-branch v-model="queryFormItem.openBrchCode" :label="$t('m.i.common.brchName')"
                                  :brchCode.sync="queryFormItem.openBrchCode"  :cpesBrchName.sync="queryFormItem.openBrchName"
                ...
                <h-form-item :label="$t('m.i.bm.isOtherBank')" prop="isOtherBank">
                  <h-select v-model="queryFormItem.isOtherBank" placeholder="">
                ...
                <h-form-item :label="$t('m.i.common.bailAcctType')" prop="bailAcctType">
                  <h-select v-model="queryFormItem.bailAcctType" placeholder="">
                ...
                <h-form-item :label="$t('m.i.common.acctNo')" prop="acctNo">
                  <h-input v-model="queryFormItem.acctNo" :maxlength="32"></h-input>
                </h-form-item>
                <h-form-item :label="$t('m.i.common.acctName')" prop="acctName">
                  <h-input v-model="queryFormItem.acctName"></h-input>
                </h-form-item>
                <h-form-item :label="$t('m.i.common.openBankNo')" prop="openBankNo">
                  <h-input v-model="queryFormItem.openBankNo" :maxlength="12"></h-input>
                </h-form-item>
                <h-form-item :label="$t('m.i.common.openBankName')" prop="openBankName">
                  <h-input v-model="queryFormItem.openBankName"></h-input>
                </h-form-item>
                <h-form-item :label="$t('m.i.billInfo.flowStatus')" prop="flowStatus">
                  <h-select v-model="queryFormItem.flowStatus" placeholder="">
                    <h-option value="FS01">已录入</h-option>
                    <h-option value="FS02">待复核</h-option>
                    <h-option value="FS06">复核退回</h-option>
                    <h-option value="FS08">复核通过</h-option>
                  </h-select>
                </h-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      formSearchReset() {
        ...
        this.queryFormItem.openBrchNo = "";
        this.queryFormItem.openBrchCode = "";
        this.queryFormItem.openBrchName = "";
      },
      //查复机构名称
    /*  queryRcvBrchName() {
      ...
      //根据弹框所选数据进行赋值
      /*brchCodeChange(info) {
        this.queryFormItem.openBrchNo = info.brchNo;
        this.queryFormItem.openBrchCode = info.brchCode;
        this.queryFormItem.openBrchName = info.brchFullNameZh;
        this.showCpesBranchWin = false;
      }*/
    },
    mounted() {
      this.getDictListByGroups("CustFlowStatus,custAcctKind").then(res => {
        this.flowStatusList = res.get("CustFlowStatus");
