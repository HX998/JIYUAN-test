        ...
        if (val === true) {
          ...
          getSingleParamValuesByKeys("bm.cust.is_sub_acct_works").then(res => {
            ...
            for(let i = 0; i < this.columns.length; i++ ){
              if(this.columns[i].key == 'subAcctSrlNo'){
                this.columns[i].hiddenCol = this.isSubAcctWorks != '1';
              }
            }
            this.ifDatagrid = true;
            if (this.selectType !== "coll") {
              this.$nextTick(() => {
    ...
    methods: {
      ...
      getCustAcctList() {
        ...
        post({
          ...
          "subAcctSrlNo": this.formItem.subAcctSrlNo
        }, url).then(res => {
            if (res) {
