...
<template>
  <div class="layout">
          ...
          <h-row>
            <h-col span="24">
              <h-datagrid
                :columns="columns"
                highlightRow
                url="/bm/valetBusiness/valetSign/func_querySignTransInfo"
                :bindForm="formItem"
                :param-id="paramId"
                :hasSelect="false"
                :rowSelect="true"
                ...
                <div slot="toolbar" class="pull-left">
                  <h-button type="primary" @click="valetSign">{{$t('m.i.bm.sign')}}</h-button>
                  <h-button type="primary" @click="valetRefuse">{{$t('m.i.common.refuse')}}</h-button>
                </div>
              </h-datagrid>
            </h-col>
          </h-row>
    ...
    <!--拒绝弹出窗-->
    <h-msg-box v-model="refuseWin" width="400" :mask-closable="false" @on-close="handleWinClose"
      ...
      <div>
        <h-form :model="refuseForm" :label-width="115" ref="refuseForm" cols="1" class="h-form-search">
          <h-form-item :label="$t('m.i.bm.dshnrCd')" prop="dshnrCd" required>
            <h-select v-model="refuseForm.dshnrCd" placeholder="" @on-change="dshnrCdChange()">
              <h-option v-for="item in dshnrCdList" :value="item.key" :key="item.key">{{ item.value }}</h-option>
            </h-select>
          </h-form-item>
          <h-form-item :label="$t('m.i.bm.dshnrRsn')" prop="dshnrRsn" :required="isRequired" class="h-form-height-auto">
            <h-input type="textarea" :rows=3 v-model="refuseForm.dshnrRsn" :canResize="false"
...
<script>
  ...
  export default {
    ...
    mounted() {
      this.getDictListByGroups("DraftTypeCode,EcdsDishonorCode,DishonorCode,EcdsBillStatusCode,BillStatusCode,BillOrigin,ValetSignTransNo").then(res => {
        this.billTypeList = res.get("DraftTypeCode");
        this.ecdsDshnrCdList = res.get("EcdsDishonorCode");
        this.cpesDshnrCdList = res.get("DishonorCode");
        this.billOriginList = res.get("BillOrigin");
        this.transNoList = res.get("ValetSignTransNo");
    ...
    methods: {
      ...
      valetSign() {
        ...
        let content = "确定进行代客签收吗？", url = "/bm/valetBusiness/valetSign/func_valetSignBill", params = {};
        if (list != null && list.length !== 0) {
          params = this.initParams("SU00");
      ...
      valetRefuse() {
        let list = this.$refs.datagrid.selects;
        let content = "确定进行代客拒绝吗？", url = "/bm/valetBusiness/valetSign/func_valetSignBill", params = {};
        if (list != null && list.length !== 0) {
          if (this.formItem.transNoList === "NES.011.20.00P" || this.formItem.transNoList === "NES.011.20.01P" || this.formItem.transNoList === "NES.011.20.03P") {
          // if (this.formItem.transNoList === "NES.006.20.00P") {
            //代客付款拒绝
            if(list.length > 1){
              this.$msgTip.info(this, { info: this.$t("m.i.common.onlyChooseOneData") });
              return;
            }
            if (list[0].billOrigin == "CS01" && list[0].inOutFlag == "1"){
              this.dshnrCdList = this.ecdsDshnrCdList;
            }else{
              this.dshnrCdList = this.cpesDshnrCdList;
            }
            this.refuseWin = true;
            this.$nextTick(() => {
            ...
            this.$nextTick(() => {
              if (this.$refs.refuseForm != null) {
                this.$refs.refuseForm.resetFields();
              }
            });
          }
          params = this.initParams("SU01");
          this.handleValet(content, params, url);
        } else {
          this.$msgTip.info(this, { info: this.$t("m.i.common.chooseAtLeastOneRecord") });
        }
      ...
      /**
       * 组装后台所需参数
       * @param signFlag  签收标识，SU00-同意签收，SU01-拒绝签收
       */
      initParams(signFlag) {
        let list = this.$refs.datagrid.selects;
        let requestDto = {};
        ...
        for (let i = 0, len = list.length; i < len; i++) {
          ...
          reqInfo.signFlag = signFlag;
          billInfo=list[i];
          reqInfo.transName = list[i].transName;
          reqInfo.rcrsAmt = list[i].transAmt;
          ...
          if (signFlag === "SU01") {
            reqInfo.dshnrCd = this.refuseForm.dshnrCd;
            reqInfo.dshnrRsn = this.refuseForm.dshnrRsn;
          }
      ...
      dshnrCdChange() {
        if(this.refuseForm.dshnrCd == "DC09" || this.refuseForm.dshnrCd == "CP06") {
      ...
      submitForm() {
        this.$refs["refuseForm"].validate(valid => {
          if (valid) {
            ...
            let params = this.initParams("SU01");
            let url = "/bm/valetBusiness/valetSign/func_valetSignBill";
            post(params, url).then(res => {
