      ...
      <h-col span="24">
        <div>
          ...
          <h-datagrid :columns="columns"
                      highlight-row
                      url="/bm/valuate/assetValuate/func_queryCheckDayAssetValuateDetail"
                      :bindForm="formItem"
                      :onCurrentChange="onCurrentChange"
                      :onCurrentChangeCancel="onCurrentChangeCancle"
                      ref="datagrid">
            <div slot="toolbar" class="pull-left">
              <h-button type="primary" @click="assetValuatePassOrRefuse('pass')">{{$t("m.i.common.pass")}}</h-button>
              <h-button type="primary" @click="assetValuatePassOrRefuse('refuse')">{{$t("m.i.common.refuse")}}</h-button>
            </div>
          </h-datagrid>
        </div>
      </h-col>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      assetValuatePassOrRefuse(optType) {
        if (this.currentSelectRow) {
          let content = optType === "pass" ? "确定复核通过吗?" : "确定拒绝待复核数据吗？";
          let params = {
            dayDetailId: this.currentSelectRow.dayDetailId,
            checkResult: optType === "pass" ? "1" : "0"
          }
          this.$hMsgBox.confirm({
            title: this.$t("m.i.common.confirm"),
            content: content,
            onOk: () => {
              this.handlePassOrRefuse("bm/valuate/assetValuate/func_checkChangeAssetValuateInfo", params);
            }
          });
        } else {
