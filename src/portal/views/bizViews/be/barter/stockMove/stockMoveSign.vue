      ...
      <h-col span="24">
        ...
        <!--        <h-form-item :label="$t('m.i.be.moveOutBrchName')" prop="moveOutBrchName" class="h-form-long-label">
                ...
                <h-form-item prop="moveOutDtRange" :label="$t('m.i.be.moveApplDt')">
                  <h-date-picker :value="moveOutDtRange" type="daterange" showFormat :editable=false placeholder="" @on-change="handleMoveOutDtChange"></h-date-picker>
                </h-form-item>
          ...
          <!--数据展示表格-->
              <h-datagrid
                :columns="columns"
                highlight-row
                url="/be/barter/stockmove/stockMoveSign/func_queryStockMoveSignBill"
                :bindForm="formItem"
          ...
          <!--点击库存移入应答弹出窗-->
          <h-msg-box v-model="agreeWin" width="400" @on-close="agreeWinClose" class="h-form-search-layer" :maximize="true">
            <p slot="header">
    ...
    methods:{
      ...
      agreeClickSubmit(str){
        if("close"==str){
          ...
          let url = '/be/barter/stockmove/stockMoveSign/func_saveStockMoveSignAndSend';
          post(this.agreeForm,url).then(res => {
            if(res){
              ...
              if(return_code==="000000"){
                ...
                if (res.data.retCode === "0BE320204109") {
                  this.$hMsgBox.confirm({
                    title:"确认要继续同意移入吗?",
                    content:res.data.retMsg,
                    onOk: () => {
                      this.agreeForm.flag = '1'
                      post(this.agreeForm, url).then(res => {
                        if (res) {
