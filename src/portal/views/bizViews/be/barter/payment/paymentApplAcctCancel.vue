      ...
      <h-col span="24">
        <div>
          ...
          <h-datagrid
              :columns="columns"
              highlight-row
              url="/be/barter/payment/paymentApplCancelAcct/func_queryPaymentConfirmCancelAcctPage"
              :bindForm="formItem"
              :hasSelect="hasSelect"
              rowSelect
              ref="datagrid">
            <div slot="toolbar" class="pull-left">
              <h-button type="primary" @on-click="cancelAcct()">{{$t("m.i.be.cancelAcct")}}</h-button>
            </div>
          </h-datagrid>
        </div>
      </h-col>
  ...
  export default {
    ...
    methods: {
      ...
      cancelAcct(){
        ...
        if(flag){
          post({ ids: ids ,ownedBrchNo:ownedBrchNo,licenseFlag:this.licenseFlag }, "/be/barter/payment/paymentApplCancelAcct/func_paymentConfirmApplyApplyAcctCancel").then(res => {
            if (res) {
