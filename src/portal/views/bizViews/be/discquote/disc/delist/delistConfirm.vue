<template>
  <div class="layout">
      ...
      <h-col span="24">
        <div>
          ...
          <h-datagrid
              :columns="columns"
              highlightRow
              url="/be/discquote/disc/delist/delistConfirm/func_getDiscDealDelistingList"
              :bindForm="formItem"
              :onCurrentChange="handleCurrentChange"
              :onCurrentChangeCancel="handleCurrentChangeCancel"
              ref="datagrid">
            <div slot="toolbar" class="pull-left">
              <h-button type="primary" @click="quoteApply">{{$t("m.i.be.delistDeal")}}</h-button>
    ...
    <listing-detail :listingDetailWin="listingDetailWin" :detailParams="detailParams"
                    @listingDetailWinClose="listingDetailWinClose" showType="confirm"
  ...
  export default {
    ...
    data() {
      return {
        ...
        billsUrl: "/be/discquote/disc/delist/delistConfirm/func_getListingBillList",
        batchUrl: "/be/discquote/disc/delist/delistConfirm/func_getDelistingBatch",
        exportUrl: "/be/discquote/disc/delist/delistConfirm/func_downloadDiscDelistBill",
        filePathUrl: "/be/discquote/disc/delist/delistConfirm/"
      };
    },
    mounted() {
      ...
      quoteApply() {
        if (this.currentSelectRow) {
          this.$hMsgBox.confirm({
            ...
            content: "确定要成交吗？",
            onOk: () => {
              let params = { batchId: this.currentSelectRow.id, operType: "1" };
              let url = "/be/discquote/disc/delist/delistConfirm/func_dealDestingOper";
      ...
      returnApply() {
        if (this.currentSelectRow) {
          this.$hMsgBox.confirm({
            ...
            content: "确定要退回吗？",
            onOk: () => {
              let params = { batchId: this.currentSelectRow.id, operType: "0" };
              let url = "/be/discquote/disc/delist/delistConfirm/func_dealDestingOper";
              this.sendPost(params, url);
            }
          });
