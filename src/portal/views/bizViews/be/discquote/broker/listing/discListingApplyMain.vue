      ...
      <h-col span="24">
        <div>
          ...
          <h-datagrid
            :columns="columns"
            highlightRow
            url="/be/discquote/broker/listing/discListingApplyMain/func_queryDiscListingBatchApplyPage"
            :bindForm="formItem"
            :onCurrentChange="handleCurrentChange"
            :onCurrentChangeCancel="handleCurrentChangeCancel"
            ref="datagrid">
            <div slot="toolbar" class="pull-left">
              <h-button type="primary" @click="addOrEditListingBatch('add')">{{$t("m.i.common.add")}}</h-button>
              <h-button type="primary" @click="addOrEditListingBatch('modify')">{{$t("m.i.common.modify")}}</h-button>
              <h-button type="primary" @click="deleteListingBatch">{{$t("m.i.common.delete")}}</h-button>
            </div>
          </h-datagrid>
        </div>
      </h-col>
    </h-row>
    <disc-listing-apply-add :addOrEditListingWin="addOrEditListingWin" :batchParams="batchParams"
                            @addOrEditListingWinClose="addOrEditListingWinClose" ref="discListingApplyAdd"></disc-listing-apply-add>
...
<script>
  ...
  export default {
    ...
    components: {
      ...
      DiscListingApplyAdd: () => import(/* webpackChunkName: "be/discquote/broker/listing/discListingApplyAdd" */`@/views/bizViews/be/discquote/broker/listing/discListingApplyAdd`),
    ...
    data() {
      return {
        ...
        dataGridUrl: "/be/discquote/broker/listing/discListingApplyMain/func_queryDiscListingBillPage",
        exportUrl: "/be/discquote/broker/listing/discListingApplyMain/func_exportDiscListingBill",
        batchUrl: "/be/discquote/broker/listing/discListingApplyMain/func_editDiscListing",
        transDataGridUrl: "/be/discquote/broker/listing/discListingApplyMain/func_queryBranchClassAndBranchCodePage",
        filePathUrl: "/be/discquote/broker/listing/discListingApplyMain/",
        custInfoUrl: "/be/discquote/broker/listing/discListingApplyMain/"
      };
    },
    mounted() {
    ...
    methods: {
      ...
      assembleParams(optType) {
        ...
        this.addOrEditListingWin = true;
      },
      //关闭新增、修改弹窗
      addOrEditListingWinClose() {
        this.addOrEditListingWin = false;
        this.currentSelectRow = null;
        ...
        if (this.currentSelectRow != null && this.currentSelectRow.length !== 0) {
          this.$hMsgBox.confirm({
            ...
            onOk: () => {
              ...
              let url="/be/discquote/broker/listing/discListingApplyMain/func_deleteDiscListingBatch";
