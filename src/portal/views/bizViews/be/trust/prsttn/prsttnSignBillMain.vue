...
<!--                <show-cpes-branch  :label="$t('m.i.billInfo.prsttnName')" class="h-form-long-label" v-if="mode==='2'"
          ...
          <h-datagrid
            ...
            <div slot="toolbar" class="pull-left">
              <h-button type="primary" @on-click="sendPrsttnSign('SU00')" v-if="authObj.sendPrsttnSignAppBtn">{{$t('m.i.be.agree')}}
              </h-button>
              <h-button type="primary" @on-click="sendPrsttnSign('SU01')" v-if="authObj.sendPrsttnSignRefBtn">{{$t('m.i.be.refuse')}}
              </h-button>
            </div>
          </h-datagrid>
        </div>
    ...
    <!-- 提示付款拒绝弹窗 -->
    <h-msg-box v-model="refuseWin" width="400" @on-close="refuseWinClose" class="h-form-search-layer" :maximize="true"
               :mask-closable="false">
      <p slot="header">
        <span>提示付款拒绝</span>
      </p>
      <div>
        <h-form :model="refuseFormItem" :label-width="115" ref="refuseFormItem" cols="1" class="h-form-search">
          <h-form-item prop="prsttnRefuseReasonCode" :label="$t('m.i.be.prsttnRefuseReasonCode')" required>
          ...
          <h-form-item prop="prsttnRefuseReason" :label="$t('m.i.be.prsttnRefuseReason')" class="h-form-height-auto"
            ...
            <h-input v-model="refuseFormItem.prsttnRefuseReason" placeholder="" type="textarea"
                     :autosize="{minRows: 2, maxRows: 4}" :maxlength="150" :readonly = "otherRefuse" :lengthByByte="false"></h-input>
          </h-form-item>
...
export default {
      ...
      return {
        ...
        autmentColumns:[
          ...
          },
          {
            ...
            hiddenCol: false,
            render: (h, params) => {
              let name = this.getDictValueFromMap(this.dictMap, "BarterTrustStatus", params.row.billStatus).replace("应答方：", "");
              return h("span", name);
            }
          },
          {
          ...
          {
            title: this.$t('m.i.be.promNoteNo'),
            key: "promNoteNo",
            hiddenCol: false
          },
          {
    ...
    created(){
      ...
      getConnectMode().then(res=>{
        this.mode = res;
        // if (this.mode==="2") {
        //   this.answerUrl="/be/trust/prsttn/prsttnSignBillMain/func_savePrsttnSignBill";
        //   this.queryBillUrl="/be/trust/prsttn/prsttnSignBillMain/func_queryPaymentApplyNeedResp";
        //   this.formItem.order="APPL_DT,id asc";
        // }else {
          this.answerUrl = "/be/trust/prsttn/prsttnSignBillMain/func_ceSavePrsttnSignBill";
          this.queryBillUrl = "/be/trust/prsttn/prsttnSignBillMain/func_ceQueryPaymentApplyNeedResp";
    ...
    mounted() {
      this.getDictListByGroups("CDMedia,DraftTypeCode,RiskStatus,BarterTrustStatus,DishonorCodeSubset,BillOrigin").then(res => {
        this.BillClassList = res.get("CDMedia");
        this.RiskStatusList = res.get("RiskStatus");
        this.refuseCodeList = res.get("DishonorCodeSubset");
