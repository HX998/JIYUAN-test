  ...
  <div class="layout">
      ...
      <h-col span="24">
        <div>
          ...
          <h-datagrid
            :columns="columns"
            highlight-row
            :url="url + '/unImpawnSign/func_queryUnImpawnSignCanSend'"
            :bindForm="formItem"
    ...
    <h-msg-box v-model="signWin" width="1000" height="400" class="h-form-table-layer"
      ...
      <h-datagrid :columns="unImpawnBill.billColumns"
                  ...
                  ref="datagridbill">
        <div slot="toolbar" class="pull-left">
          <h-dropdown v-show="type!=='view'" trigger="click" @on-click="sign">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 查询批次明细
      queryBatchById(id) {
        post({ id: id + "" }, "/pb/trust/unImpawn/unImpawnApply/func_getUnImpawnBatchById").then(res => {
          if (res) {
            ...
            if (retCode === "000000") {
              ...
              this.$msgTip.error(this, { info: "查询批次信息失败" + res.data.retMsg });
              this.signWinClose();
            }
          } else {
      ...
      queryBillByBatchId(id) {
        ...
        post(obj, "/pb/trust/unImpawn//unImpawnSign/func_queryUnImpawnSignSendBill").then(res => {
          if (res) {
            let retCode = res.data.retCode;
            if (retCode === "000000") {
              this.billList = res.data.retData;
            } else {
              this.$msgTip.error(this, { info: "查询票据明细失败" + res.data.retMsg });
              this.signWinClose();
            }
          } else {
            this.$msgTip.error(this, { info: this.$t("m.i.common.netError") });
      ...
      // 查询票据清单
      queryBillList(obj) {
        post(obj, this.url + "/unImpawnSign/func_queryUnImpawnSignSendBill").then(res => {
          let retCode = res.data.retCode;
          if (retCode === "000000") {
            this.billList = res.data.retData;
          }
      ...
      sign(obj) {
        ...
        if (obj === "selectSign" && this.currentSelectBillList.length === 0) {
          ...
          this.$hMsgBox.confirm({
            ...
            onOk: () => {
              post(this.addForm, this.url + "/unImpawnSign/func_sendUnImpawnSignBatch").then(res => {
                if (res) {
                  let retCode = res.data.retCode;
                  if (retCode === "000000") {
      ...
      back(obj) {
        ...
        if (obj === "selectBack" && this.currentSelectBillList.length === 0) {
          ...
          this.$hMsgBox.confirm({
            ...
            onOk: () => {
              post(this.addForm, this.url + "/unImpawnSign/func_backImpawnSignBatch").then(res => {
                if (res) {
                  let retCode = res.data.retCode;
                  if (retCode === "000000") {
