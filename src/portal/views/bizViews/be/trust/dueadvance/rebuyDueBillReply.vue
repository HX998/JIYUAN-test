      ...
      <h-col span="24">
        <div>
          ...
          <h-datagrid
            :columns="columns"
            highlightRow
            url="/be/trust/dueadvance/rebuyDueBillReply/func_queryRebuyDueBillReply"
            :bindForm="formItem"
            rowSelect
            ref="datagrid">
            <div slot="toolbar" class="pull-left">
              <h-button type="primary" @click="rebuyDueBillApplyAgree()">{{$t("m.i.common.agree")}}</h-button>
  ...
  export default {
    ...
    data() {
      return {
        ...
        acctRecord : "/be/trust/dueadvance/rebuyDueBillReply/func_queryRebuyDueBillAcctRecord",
        accountUrl : "/be/trust/dueadvance/rebuyDueBillReply/func_rebuyDueBillAgreeApply",
      };
    },
    mounted() {
    ...
    methods: {
      ...
      rebuyDueBillApplyAgree(){
        let ids = this.$refs.datagrid.selectIds;
        if (ids != null && ids.length !== 0) {
          this.$hMsgBox.confirm({
            title: "同意信息",
            content: "请确认，同意将所选的票据转垫款？",
            onOk: () => {
              this.account(ids);
              // let params={ id: this.currentSelectRow.id };
              // let url="/be/trust/dueadvance/rebuyDueBillReply/func_rebuyDueBillAgreeApply";
      ...
      rebuyDueBillApplyRefuse(){
        ...
        if (ids != null && ids.length !== 0) {
          this.$hMsgBox.confirm({
            title: "拒绝信息",
            content: "请确认，拒绝将所选的票据转垫款？",
            onOk: () => {
              let params={ ids: ids };
              let url="/be/trust/dueadvance/rebuyDueBillReply/func_rebuyDueBillCancelApply";
              this.sendPost(params,url);
            }
          });
        } else {
