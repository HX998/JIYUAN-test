        ...
        <div slot="toolbar" class="pull-left">
          <h-dropdown trigger="click" @on-click="account">
            ...
            <h-dropdown-menu slot="list">
              <h-dropdown-item name="selectAccout">{{$t('m.i.be.selectAccount')}}</h-dropdown-item>
              <h-dropdown-item name="allAccout">{{$t('m.i.be.batchAccount')}}</h-dropdown-item>
            </h-dropdown-menu>
          </h-dropdown>
        </div>
      </h-datagrid>
  ...
  export default {
    ...
    data() {
      return {
        ...
        authPath: this.$route.path,
       // brchNoWin: false,
        acctRecord: "/bp/guaranty/apply/func_getAcctRecordBatch",
        param: {},
        addOrEditWin: false,
        billList: {
          list: [],
    ...
    methods: {
      ...
      queryBillList(obj) {
        if (this.isRollBack) {
          post(obj, "bp/guaranty/apply/func_queryBatchBillByPage").then(res => {
      ...
      showAcctWin() {
        if (!this.checkOnlyOne(this.currentSelectRow)) {
          ...
          this.$nextTick(() => {
            let pageNo = this.$refs.datagridbill.pageInfo.pageNo;
            this.$refs.datagridbill.dataChange(pageNo);
        });
      },
      ...
      queryObjById(id) {
        this.batchId = id;
        post({ batchId: id + "", queryType: 4 }, "bp/guaranty/apply/func_getGuarntrBatchById").then(res => {
      ...
      applyAccount(info) {
        ...
        }, "/bp/guaranty/apply/func_guarantyAcctByBatch").then(res => {
          if (res) {
            let retCode = res.data.retCode;
            ...
            if (retCode == "000000") {
              ...
              this.queryBillList(obj);
              this.$msgTip.info(this, { info: "记账成功" });
