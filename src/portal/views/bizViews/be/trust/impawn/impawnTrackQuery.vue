<template>
  <div class="layout">
    <h-row name="flex" class="layout-menu-wrapper">
      <h-col span="24">
        <div>
            ...
            <h-panel class="clearfix" :class="{'h-form-overhd':!ifShowMore}">
              <h-form :model="formItem" :label-width="90" ref="formItem" cols="4" class="h-form-search">
                <h-form-item :label="$t('m.i.be.pledgeType')" prop="applSignFlag">
                  <h-select v-model="formItem.applSignFlag" placeholder="" @on-change="showBrch" :clearable="false" showTitle>
                ...
                <h-form-item :label="$t('m.i.be.branchType')" prop="brchType">
                  <h-select v-model="formItem.brchType" placeholder="" showTitle>
                ...
                <h-form-item :label="$t('m.i.billInfo.dueDt')" prop="dueDt">
                  <h-date-picker type="daterange" autoPlacement placeholder="" :value="dueDt" format="yyyy-MM-dd" showFormat
                                 @on-change="dueDtChange"></h-date-picker>
                </h-form-item>
                <h-form-item :label="$t('m.i.be.pledgeDt')" prop="applDt">
                  <h-date-picker type="daterange" autoPlacement placeholder="" :value="applDt" format="yyyy-MM-dd"
                                 showFormat @on-change="applDtChange"></h-date-picker>
                </h-form-item>
                <h-form-item :label="$t('m.i.be.pledgeRespFlag')" prop="respFlag">
          ...
          <h-datagrid
            :columns="columns"
            url="pb/trust/impawn/impawnTrackQuery/func_impawnTrackQuery"
            :bindForm="formItem"
            :hasSelect=false
            :rowSelect=true
            ref="datagrid">
    ...
    <h-msg-box v-model="impawnTrackQueryWin" width="1000" height="400" class="h-form-table-layer"
               @on-close="impawnTrackQueryWinClose" @on-maximize="onMaximize" :maximize="true" :mask-closable="false"
               :footerHide="true">
      <p slot="header">
        <span>质押跟踪查询明细</span>
      </p>
  ...
  export default {
    ...
    data() {
      return {
        ...
        columns: [
          ...
          {
            title: this.$t('m.i.be.pledgeDt'),
            key: "impawnDt",
            sortable: true,
            hiddenCol: false,
            render: (h, params) => {
              let applDt = this.$moment(params.row.impawnDt, "YYYYMMDD").format("YYYY-MM-DD");
          ...
          {
            title: this.$t('m.i.be.pledgeRespFlag'),
            key: "respFlag",
            hiddenCol: false,
            render: (h, params) => {
          ...
          // {
          //   title: this.$t('m.i.be.isRelieve'),
          //   key: "isRelieve",
          //   hiddenCol: false,
          //   render: (h, params) => {
          //     let list = this.getDictValueFromMap(this.dictMap, "Yon", params.row.isRelieve);
          //     return h("span",
          //       {
          //         domProps: {
          //           title: list
    ...
    methods: {
      ...
      tempShowExcelTemplateWinOpen() {
        this.param = "impawnTrackQueryReportInfo";
      ...
      exportList(field, exportType) {
        ...
        if (exportType === "1") {
          let url = window.LOCAL_CONFIG.API_HOME + "/pb/trust/impawn/impawnTrackQuery/func_exportExcel";
          let params = null;
          ...
          if (this.$refs.datagrid.selectIds.length != 0) {
            ...
            params = {
              ...
              isRelieve:this.formItem.isRelieve,
      ...
      queryBillByBatchId(id) {
        ...
        post(obj, "pb/trust/impawn/impawnBatchSign/func_queryImpawnBillByBillId").then(res => {
          if (res) {
      ...
      getParams(){
        if (this.$route.path === "/pb/trust/impawn/impawnTrackQuery") {
          if (this.$route.query.dueDt) {
            let dueDt = this.$moment(this.$route.query.dueDt, "YYYY-MM-DD").format("YYYY-MM-DD");
            this.dueDt = [dueDt, dueDt];
            this.formItem.billClass = this.$route.query.billClass;
            this.formItem.isRelieve = this.$route.query.isRelieve;
            this.formItem.applSignFlag = this.$route.query.applSignFlag;
            this.$nextTick(() => {
              this.formSearch();
            });
          }
    ...
    mounted() {
      this.getDictListByGroups("DraftTypeCode,CDMedia,AcctFlag,BarterTrustBillStatus,Yon,BillOrigin").then(res => {
