  ...
  <div class="layout">
        ...
        <div>
          ...
          <h-datagrid
            ...
            <div slot="toolbar" class="pull-left">
              <h-button type="primary" @click="showRollBackWin()" v-if="authObj.bizSetRoleDelete">{{$t('m.i.be.revokeAppl')}}
              </h-button>
            </div>
          </h-datagrid>
        </div>
    ...
    <h-msg-box v-model="addOrEditWin" width="1000" height="400" class="h-form-table-layer"
      ...
      <h-panel>
        <h-form :model="addForm" :label-width="120" ref="addForm" cols="2" class="h-form-search">
          <h-form-item :label="$t('m.i.billInfo.billType')" prop="billType" :required="isRequired">
            <h-select v-if="type=='view'" readonly :value="addForm.billType" :title="addForm.billType" showTitle>
          ...
          <h-form-item :label="$t('m.i.billInfo.billClass')" prop="billClass" :required="isRequired">
            <h-select v-if="type=='view'" readonly :value="addForm.billClass" :title="addForm.billClass" showTitle>
          ...
          <h-form-item :label="$t('m.i.be.pawneeName')" prop="custName" :required="isRequired">
            <h-input v-if="type=='view'" readonly :value="addForm.custName" :title="addForm.custName"></h-input>
          ...
          <h-form-item :label="$t('m.i.be.pawneeBrchCode')" prop="custBrchCode" :required="isRequired">
            <h-input v-if="type=='view'" readonly :value="addForm.custBrchCode" :title="addForm.custBrchCode"></h-input>
          ...
          <h-form-item :label="$t('m.i.common.applDt')" prop="impawnDt" :required="isRequired">
            <h-date-picker v-if="type=='view'" readonly :value="addForm.impawnDt" :title="addForm.impawnDt"
          ...
          <h-form-item :label="$t('m.i.be.pledgorBrchName')" prop="applBrchName" :required="isRequired">
            <h-input v-if="type=='view'" readonly :value="addForm.applBrchName" :title="addForm.applBrchName"></h-input>
      ...
      <h-datagrid
        ...
        ref="datagridbill">
        <div slot="toolbar" class="pull-left">
          <h-dropdown trigger="click" @on-click="applyCancel" v-if="authObj.bizSetRoleAdd">
            <h-button type="primary">{{$t('m.i.be.revokeAppl')}}
              <h-icon name="unfold"></h-icon>
            </h-button>
            <h-dropdown-menu slot="list">
              <h-dropdown-item name="selectCancel">选择撤回</h-dropdown-item>
              <h-dropdown-item name="allCancel">批次撤回</h-dropdown-item>
            </h-dropdown-menu>
          </h-dropdown>
        </div>
      </h-datagrid>
  ...
  export default {
    ...
    methods: {
      ...
      queryObjById(id,funcNo) {
        let batchId = id;
        this.batchId = id;
        post({ batchId: id + "",funcNo:funcNo }, "pb/trust/impawn/impawnApply/func_queryImpawnApplyBill").then(res => {
      ...
      applyCancel(obj) {
        ...
        if (obj == "selectCancel" && this.currentSelectBillList.length == 0) {
          this.$msgTip.info(this, { info: this.$t("m.i.common.chooseOneData") });
        } else {
          if (obj === "selectCancel") {
          ...
          this.$hMsgBox.confirm({
            ...
            onOk: () => {
              post(this.addForm, "/pb/trust/impawn/impawnApplyRollBack/func_cancleImpawnApplyBillApply").then(res => {
                if (res) {
                  let retCode = res.data.retCode;
                  if (retCode == "000000") {
                    ...
                    this.$refs.datagrid.dataChange(1);
                    this.$msgTip.info(this, { info: this.$t("m.i.common.rollBackSuccess") });
                    this.currentSelectBillList = [];
                    this.$refs.datagridbill.selectIds = [];
                    this.$refs.datagridbill.selects = [];
                    this.addOrEditWin = false;
                  } else {
                    this.$msgTip.info(this, { info: this.$t("m.i.common.rollBackFailed") });
                    this.$refs.datagrid.dataChange(1);
                  }
                } else {
                  this.$msgTip.error(this, { info: this.$t("m.i.common.netError") });
                }
                this.addForm.operType = null;
                this.addForm.impawnIds = null;
              });
            }
          });
