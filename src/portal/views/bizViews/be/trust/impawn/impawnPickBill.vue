    ...
    <div :class="{'h-form-overhd':!ifShowMore}">
      <h-form :model="formItem" :label-width="115" ref="formItem" cols="4" class="h-form-search">
        ...
        <h-form-item :label="$t('m.i.billInfo.remitDt')" prop="remitDt">
          <h-date-picker type="daterange" :value="remitDt" format="yyyy-MM-dd" showFormat
                         @on-change="remitDtChange"></h-date-picker>
        </h-form-item>
        <!--汇票到票日期-->
        <h-form-item :label="$t('m.i.billInfo.dueDt')" prop="dueDt">
          <h-date-picker type="daterange" :value="dueDt" :options="options1" format="yyyy-MM-dd"  showFormat
  ...
  export default {
    ...
    data() {
      ...
      return {
        ...
        datagridUrl: "/pb/trust/impawn/impawnApply/func_queryPendingImpawnApplyBill",
        ifShowMore: false,
        currentSelectList: [],
    ...
    watch: {
      pickBillWin(val) {
        if (val === true) {
          this.datagridUrl = "pb/trust/impawn/impawnApply/func_queryPendingImpawnApplyBill";
          this.$nextTick(() => {
            let  columns = [];
