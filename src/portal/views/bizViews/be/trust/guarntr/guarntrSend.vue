  ...
  <div class="layout">
        ...
        <div>
               ...
               <!-- <h-form-item :label="$t('m.i.common.ownedBrchNo')" prop="ownedBrchNos" v-if="this.licenseFlag">
                ...
                <h-form-item :label="$t('m.i.common.applDt')" prop="applDate">
                  <h-date-picker type="daterange" autoPlacement placeholder="" :value="applDate" :showFormat="true"
                                 format="yyyy-MM-dd" @on-change="handleApplDtChange"></h-date-picker>
                </h-form-item>
          ...
          <h-datagrid
            ...
            <div slot="toolbar" class="pull-left">
              <h-button type="primary" @click="billManage()">{{$t('m.i.be.confirmationSign')}}</h-button>
            </div>
          </h-datagrid>
        </div>
      </h-col>
    ...
    <h-msg-box v-model="viewWin" width="1000" height="400" class="h-form-table-layer"
        ...
        <h-form :model="addForm" :label-width="120" ref="addForm" cols="2" class="h-form-search">
          <h-form-item :label="$t('m.i.billInfo.billType')" prop="billType" :required="isRequired">
            <h-select v-if="type=='view'" :disabled=true :value="addForm.billType" class="input-boder-0"
                      :title="addForm.billType" showTitle>
              <h-option v-for="item in billTypeList" :value="item.key" :key="item.key">{{item.value}}</h-option>
            </h-select>
          ...
          <h-form-item :label="$t('m.i.billInfo.billClass')" prop="billClass" :required="isRequired">
            <h-select v-if="type=='view'" :disabled=true :value="addForm.billClass" class="input-boder-0"
                      :title="addForm.billClass" showTitle>
              <h-option v-for="item in cdMediaList" :value="item.key" :key="item.key">{{item.value}}</h-option>
            </h-select>
            <h-select v-else v-model="addForm.billClass" showTitle>
              <h-option v-for="item in cdMediaList" :value="item.key" :key="item.key">{{item.value}}</h-option>
            </h-select>
          </h-form-item>
      ...
      <h-datagrid
        ...
        ref="datagridbill">
        <div slot="toolbar" class="pull-left">
          <h-dropdown placement="bottom-start" trigger="click" @on-click="sendGuarntr">
            <h-button type="primary" v-if="authObj.bizSetRoleORight">{{$t('m.i.be.sign')}}
              <h-icon name="unfold"></h-icon>
            </h-button>
            <h-dropdown-menu slot="list">
              <h-dropdown-item name="choice">选择签收</h-dropdown-item>
              <h-dropdown-item name="all">批次签收</h-dropdown-item>
            </h-dropdown-menu>
          </h-dropdown>
          <h-dropdown placement="bottom-start" trigger="click" @on-click="resetGuarntr">
            <h-button type="primary" v-if="authObj.bizSetRoleORight">{{$t('m.i.be.returnBack')}}
              <h-icon name="unfold"></h-icon>
            </h-button>
            <h-dropdown-menu slot="list">
              <h-dropdown-item name="choice">选择退回</h-dropdown-item>
              <h-dropdown-item name="all">批次退回</h-dropdown-item>
            </h-dropdown-menu>
          </h-dropdown>
        </div>
      </h-datagrid>
    ...
    methods: {
      ...
      sendGuarntr(str) {
        ...
        this.$hMsgBox.confirm({
          ...
          content: this.$t("确认要签收吗") + "?",
          onOk: () => {
            this.send(guarntrBillIds, guarntrBatchId);
          }
        });
      },
      /**确认签收**/
      send(guarntrBillIds, guarntrBatchId) {
        post({
          listIds: guarntrBillIds,
          funcNo:"PIPB010203",
          signFlag:"SU00",
          batchId: guarntrBatchId
        }, "bp/guaranty/sign/func_sendSignMsg").then(res => {
          if (res) {
            let retCode = res.data.retCode;
            if (retCode == "000000") {
              this.$msgTip.success(this, { info: "签收成功" });
              this.$refs.datagridbill.dataChange(this.$refs.datagridbill.pageInfo.pageNo);
              this.$refs.datagrid.dataChange(this.$refs.datagrid.pageInfo.pageNo);
              this.dataTmpSelectReset();
              this.viewWin = false;
            } else {
              this.$msgTip.error(this, { info: "签收失败：" + res.data.retMsg });
              this.$refs.datagridbill.dataChange(this.$refs.datagridbill.pageInfo.pageNo);
              this.dataTmpSelectReset();
            }
          } else {
      ...
      resetGuarntr(str) {
        ...
        this.$hMsgBox.confirm({
          ...
          onOk: () => {
            this.reset(guarntrBillIds, guarntrBatchId);
          }
        });
      },
      /**确认退回**/
      reset(guarntrBillIds, guarntrBatchId) {
        post({
          listIds: guarntrBillIds,
          funcNo:"PIPB010203",
          batchId: guarntrBatchId
