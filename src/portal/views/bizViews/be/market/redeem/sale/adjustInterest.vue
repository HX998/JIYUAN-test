  ...
  <h-msg-box v-model="tempAdjustInterestWin" width="800" :mask-closable="false" @on-close="handleClose"
    ...
    <div>
      <h-form :model="adjustForm" :label-width="115" ref="adjustForm" cols="2" class="h-form-search">
        ...
        <h-form-item :label="$t('m.i.be.interestAdjustmentType')" prop="interestAdjustType" :required="isRequired">
          <h-select v-model="adjustForm.interestAdjustType" showTitle>
          <h-option v-for="item in adjustParams.adjustTypeList" :value="item.key" :key="item.key">{{item.value }}</h-option>
          </h-select>
        </h-form-item>
        <h-form-item :label="$t('m.i.be.adjustmentInterest')" prop="adjustInterest" :required="isRequired" :validRules="interestRule">
          <h-typefield v-model="adjustForm.adjustInterest" placeholder="正数,支持2位小数" :maxlength="20"></h-typefield>
        </h-form-item>
      </h-form>
    </div>
    <div slot="footer">
    ...
    methods: {
      ...
      submitForm() {
        this.$refs["adjustForm"].validate(valid => {
          if (valid) {
            post(this.adjustForm, "/be/market/redeem/sale/saleMatureApplyMain/func_adjustment").then(res => {
              if (res) {
