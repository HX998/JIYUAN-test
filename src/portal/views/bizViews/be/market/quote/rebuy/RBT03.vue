  ...
  <div class ="out-side">
    <div>
      <h-form-item :label="$t('m.i.be.firstSettleDt')" v-if="formItem.busiType=='BT03'"  prop="firstSettleDt" required class="h-form-three">
        <h-date-picker readonly v-model="formItem.firstSettleDt" format="yyyy-MM-dd" placeholder="首期结算日" showFormat :editable=false></h-date-picker>
      </h-form-item>
      <h-form-item :label="$t('m.i.be.repoTerm')" v-if="formItem.busiType=='BT03'" prop="tenorDays" required class="h-form-three" :validRules="tenorDaysRule">
        <h-input v-model="formItem.tenorDays" placeholder="" @on-blur="changeSettleDate"></h-input>
      </h-form-item>
      <h-form-item :label="$t('m.i.be.dueSettleDt')" v-if="formItem.busiType=='BT03'"  prop="dueSettleDt" required class="h-form-three">
        <h-date-picker v-model="formItem.dueSettleDt" format="yyyy-MM-dd" showFormat @on-change='dueDtChange'></h-date-picker>
      </h-form-item>
      <h-form-item :label="$t('m.i.be.firstTransRate')" v-if="formItem.busiType=='BT03'"  prop="ratePct" required  class="h-form-three h-form-long-label" :validRules="ratePctRule">
        <h-input v-model="formItem.ratePct" placeholder="利率范围0~100，且小数位不超过四位" ></h-input>
      </h-form-item>
      <h-form-item :label="$t('m.i.be.dueTransRate')" v-if="formItem.busiType=='BT03'" prop="dueRatePct" required class="h-form-three h-form-long-label" :validRules="ratePctRule">
