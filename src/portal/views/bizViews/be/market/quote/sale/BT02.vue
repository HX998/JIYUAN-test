  ...
  <div class ="out-side">
    <div>
      <h-form-item :label="$t('m.i.be.firstSettleDt')" v-if="formItem.busiType=='BT02'" prop="firstSettleDt" required class="h-form-three">
        <h-date-picker  v-model="formItem.firstSettleDt" format="yyyy-MM-dd" placeholder="" showFormat @on-change='firstDtChange' :options="options2"></h-date-picker>
      </h-form-item>
      <h-form-item :label="$t('m.i.be.repoTerm')" v-if="formItem.busiType=='BT02'" prop="tenorDays" :required="!(formItem.isTemStorage&&formItem.isTemStorage=='1'&&!formItem.quoteNo)" class="h-form-three" :validRules="tenorDaysRule">
        <h-input v-model="formItem.tenorDays" placeholder="" @on-blur="changeSettleDate"></h-input>
      </h-form-item>
      <h-form-item :label="$t('m.i.be.dueSettleDt')" v-if="formItem.busiType=='BT02'" prop="dueSettleDt" :required="!(formItem.isTemStorage&&formItem.isTemStorage=='1'&&!formItem.quoteNo)" class="h-form-three">
        <h-date-picker v-model="formItem.dueSettleDt" format="yyyy-MM-dd" placeholder="" showFormat @on-change='dueDtChange' :options="optionsDueDt"></h-date-picker>
      </h-form-item>
      <h-form-item :label="$t('m.i.be.repurchaseRate')" v-if="formItem.busiType=='BT02'" prop="ratePct" :required="!(formItem.isTemStorage&&formItem.isTemStorage=='1'&&!formItem.quoteNo)"  class="h-form-three" :validRules="ratePctRule">
      ...
      <h-form-item :label="$t('m.i.be.billSelectType')" v-if="formItem.busiType=='BT02'" prop="selectType" required  class="h-form-three">
        <h-select v-if="formItem.quoteNo" v-model="formItem.selectType" placeholder="" readonly showTitle>
          <h-option v-for="item in dictMap['CdSelectMode']" :value="item.key" :key="item.key">{{item.value }}</h-option>
        </h-select>
        <h-select v-else v-model="formItem.selectType" placeholder="" showTitle>
          <h-option v-for="item in dictMap['CdSelectMode']" :value="item.key" :key="item.key">{{item.value }}</h-option>
        </h-select>
      </h-form-item>
