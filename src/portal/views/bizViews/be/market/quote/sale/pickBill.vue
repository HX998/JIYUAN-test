      ...
      <div :class="{'h-form-overhd':!ifShowMore}">
        <h-form :model="pickBillForm" :label-width="90" ref="pickBillForm" cols="4" class="h-form-search">
          <h-form-item :label="$t('m.i.billInfo.billType')">
            <h-select v-model="pickBillForm.billType" placeholder="请选择" readonly showTitle>
              <h-option v-for="item in curDictMap['billTypeList']" :value="item.key" :key="item.key">{{item.value }}
              </h-option>
            </h-select>
          </h-form-item>
          <h-form-item :label="$t('m.i.billInfo.billClass')">
            <h-select v-model="pickBillForm.billClass" placeholder="请选择" readonly showTitle>
              <h-option v-for="item in curDictMap['billClassList']" :value="item.key" :key="item.key">{{item.value }}
              </h-option>
            </h-select>
          </h-form-item>
          ...
          <h-form-item prop="minDueDt" class="h-form-long-label" :label="$t('m.i.billInfo.minDueDate')"
                       v-if="pickBillFlag">
            <h-date-picker :value="pickBillForm.minDueDt" format="yyyy-MM-dd" type="date" autoPlacement showFormat
                           :editable=false @on-change="handleDueDateChange"></h-date-picker>
          </h-form-item>
          <h-form-item prop="maxDueDt" class="h-form-long-label" :label="$t('m.i.billInfo.maxDueDate')"
                       v-if="pickBillFlag">
            <h-date-picker :value="pickBillForm.maxDueDt" format="yyyy-MM-dd" type="date" autoPlacement showFormat
                           :editable=false @on-change="handleMaxDueDateChange"></h-date-picker>
          </h-form-item>
          ...
          <h-form-item prop="minBuyDt" class="h-form-long-label" :label="$t('m.i.be.minBuyDt')">
            <h-date-picker :value="pickBillForm.minBuyDt" format="yyyy-MM-dd" type="date" autoPlacement showFormat
                           :editable=false @on-change="handleBuyDateChange"></h-date-picker>
          </h-form-item>
          <h-form-item prop="maxBuyDt" class="h-form-long-label" :label="$t('m.i.be.maxBuyDt')">
            <h-date-picker :value="pickBillForm.maxBuyDt" format="yyyy-MM-dd" type="date" autoPlacement showFormat
                           :editable=false @on-change="handleMaxBuyDateChange"></h-date-picker>
          </h-form-item>
          ...
          <h-form-item :label="$t('m.i.billInfo.acptTopBankLevel')" prop="acptTopBankLevels" v-if="pickBillFlag">
            <h-select v-model="pickBillForm.acptTopBankLevels" multiple placeholder="请选择" showTitle>
              <h-option v-for="item in curDictMap['bankLevelList']" :value="item.key" :key="item.key">{{item.value }}
              </h-option>
            </h-select>
          </h-form-item>
          ...
          <h-form-item :label="$t('m.i.be.residualTerm')" prop="billTerm" v-if="pickBillFlag">
            <h-select v-model="pickBillForm.billTerm" placeholder="请选择" showTitle>
              <h-option v-for="item in curDictMap['TenorCodeList']" :value="item.key" :key="item.key">{{item.value }}
              </h-option>
            </h-select>
          </h-form-item>
          <h-form-item :label="$t('m.i.billInfo.acptBankType')" prop="acptBankTypes" v-if="pickBillFlag">
            <h-select v-model="pickBillForm.acptBankTypes" placeholder="请选择" multiple showTitle>
              <h-option v-for="item in curDictMap['bankTypeList']" :value="item.key" :key="item.key">{{item.value }}
              </h-option>
            </h-select>
          </h-form-item>
          ...
          <h-form-item :label="$t('m.i.billInfo.creditMainBankType')" prop="creditBranchClasses" v-if="pickBillFlag">
            <h-select v-model="pickBillForm.creditBranchClasses" placeholder="请选择" multiple showTitle>
              <h-option v-for="item in curDictMap['CreditMajorList']" :value="item.key" :key="item.key">{{item.value
                }}
              </h-option>
            </h-select>
          </h-form-item>
          <h-form-item :label="$t('m.i.billInfo.creditMainBankName')" prop="creditBranchName" class="h-form-long-label"
                       v-if="pickBillFlag">
          ...
          <h-form-item :label="$t('m.i.billInfo.creditMainBrchCode')" class="h-form-long-label" prop="creditBrchCode"
                       v-if="pickBillFlag">
            <h-input v-model="pickBillForm.creditBrchCode" @on-click="showChooseBrchWin('creditBrchCode')" icon="search"
                     clearable @on-clear="clearVal('creditBrchCode')" readonly placeholder=""></h-input>
          </h-form-item>
          ...
          <h-form-item :label="$t('m.i.billInfo.businessSource')" prop="orgnlProdNos" v-if="pickBillFlag">
            <h-select v-model="pickBillForm.orgnlProdNos" placeholder="请选择" multiple showTitle>
              <h-option v-for="item in curDictMap['billSourceList']" :value="item.key" :key="item.key">{{item.value }}
              </h-option>
            </h-select>
          </h-form-item>
        ...
        <h-datagrid :columns="pickColumns"
          ...
          <div slot="toolbar" class="pull-left">
            ...
            <h-button type="primary" @on-click="tempShowExcelTemplateWinOpen()" v-if="pickBillFlag">
              {{$t('m.i.common.exportDetail')}}
            </h-button>
          </div>
        </h-datagrid>
      </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        paramId: "id",
        //明细挑票地址
        pickBillUrl: "/be/market/quote/quoteManager/func_pagingQueryHldrBillInfo",
        //批次挑票地址
        pickBillBatchUrl: "/be/market/quote/quoteManager/func_pagingQueryHldrBillCollect",
        pickUrl: "/be/market/quote/quoteManager/func_pagingQueryHldrBillInfo",
        pickBillForm: {
          billType: '',
          billClass: '',
          batchNo: '',
    ...
    watch: {
      pickBillWin(val) {
        if (val) {
          if (this.rootUrl) {
            //明细挑票地址
            this.pickBillUrl = this.rootUrl + "/func_pagingQueryHldrBillInfo";
            //批次挑票地址
            this.pickBillBatchUrl = this.rootUrl + "/func_pagingQueryHldrBillCollect";
            this.pickUrl = this.pickBillUrl;
          } else {
            //明细挑票地址
            this.pickBillUrl = "/be/market/quote/quoteManager/func_pagingQueryHldrBillInfo";
            //批次挑票地址
            this.pickBillBatchUrl = "/be/market/quote/quoteManager/func_pagingQueryHldrBillCollect";
            this.pickUrl = this.pickBillUrl;
          }
          this.paramId = "id";
    ...
    methods: {
      memoryInit() {
        ...
        if (this.rootUrl) {
          url = this.rootUrl + "/func_findQuoteMemoryByBrchCode";
        }
          ...
          if (res) {
            ...
            if (retCode == "000000") {
              ...
              if (obj) {
                ...
                if (obj.maxDueDt != null) {
                  this.pickBillForm.maxDueDt = this.$moment(obj.maxDueDt, "YYYYMMDD").format("YYYY-MM-DD");
                }
                if (obj.minDueDt != null) {
                  this.pickBillForm.minDueDt = this.$moment(obj.minDueDt, "YYYYMMDD").format("YYYY-MM-DD");
                }
              }
              this.$nextTick(() => {
      ...
      handlePickBillSearch() {
        ...
        if (this.pickBillFlag) {
          if (this.isMemory == "hasMemory") {
            ...
            if (this.rootUrl) {
              url = this.rootUrl + "/func_addQuoteMemory";
            }
            ...
            if (this.rootUrl) {
              url = this.rootUrl + "/func_deleteQuoteMemoryByBrchCode";
            }
            post({code: this.custBrchCode}, url).then(res => {
              if (res) {
                let retCode = res.data.retCode
      ...
      addBill(list) {
        ...
        if (pickBillFlag) {
          ...
          let url = "/be/market/quote/quoteManager/func_pickBillStock";
          if (this.rootUrl) {
            url = this.rootUrl + "/func_pickBillStock";
          }
          ...
          let batchId = this.batchId;
          let saleBrchCode = list[0].saleBrchCode;
          let data = {
            batchId: batchId,
            batchNo: batchNo,
            buyDt: buyDt,
            transFromName: transFromName,
            prodNo: prodNo,
            billClass: billClass,
            billType: billType,
            buyRatePct: buyRatePct,
            isLock: this.isLock,
            isTemStorage: this.isTemStorage,
            saleBrchCode: saleBrchCode
          };
          let url = "/be/market/quote/quoteManager/func_batchPickBillStock";
          if (this.rootUrl) {
            url = this.rootUrl + "/func_batchPickBillStock";
          }
      ...
      exportList(field, exportType) {
        ...
        let url = window.LOCAL_CONFIG.API_HOME + "/be/market/quote/quoteManager/func_exportSaleHldrBillExcel";
        if (this.rootUrl) {
          url = window.LOCAL_CONFIG.API_HOME + this.rootUrl + "/func_exportSaleHldrBillExcel";
        }
        ...
        if (exportType === "1") {
          ...
          if (this.$refs.pickBillDatagrid.selectIds.length != 0) {
            ...
            input6.type = 'text'
            input6.name = 'batchId';
            input6.value = this.pickBillForm.batchId;
