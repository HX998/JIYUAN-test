    ...
    <h-msg-box maximize v-model="tempImageWin" width="1000" @on-close="handleClose">
      ...
      <div v-show="false" ref="saleBatch">
        <ul class="list-total-info clearfix">
          <li>
            <label class="label">{{$t('m.i.be.counterpartyFullName')}}：</label><span class="val">{{viewDatailForm.buyBrchName}}</span>
          </li>
          <li>
            <label class="label">{{$t('m.i.billInfo.billType')}}：</label><span class="val">{{format("DraftTypeCode",viewDatailForm.billType)}}</span>
          </li>
          <li>
            <label class="label">{{$t('m.i.common.batchNo')}}：</label><span
            class="val">{{viewDatailForm.batchNo}}</span>
          </li>
          <li>
            <label class="label">{{$t('m.i.billInfo.billClass')}}：</label><span class="val">{{format("CDMedia",viewDatailForm.billClass)}}</span>
          </li>
          <li>
            <label class="label">{{$t("m.i.be.firstTransRate")}}：</label><span
            class="val">{{viewDatailForm.ratePct}}</span>
          </li>
          <li>
            <label class="label">{{$t("m.i.be.dueTradRate")}}：</label><span
            class="val">{{viewDatailForm.dueRatePct}}</span>
          </li>
          <li>
      ...
      <div ref="saleBill" v-show="false" class="list-table-box">
        ...
        <div class="list-page">
          <span>当前第<em><font tdata="PageNO">##</font></em>页</span><span>共<em><font
          tdata="PageCount">##</font></em>页</span>
        </div>
      </div>
    </h-msg-box>
  </div>
          ...
          if (res && res.data) {
            ...
            if (retCode == "000000") {
              ...
              this.$nextTick(() => {
                ...
                let options = {bodyTop: "37%", headHeight: "25%", bodyWidth: "90%",bodyHtmlHeight:this.$refs.saleBatch.offsetHeight,bodyHtmlWidth:this.$refs.saleBatch.offsetWidth,headHtmlHeight:this.$refs.saleBill.offsetHeight};
