<template>
  <!-- 客户账号信息查询 -->
  <h-msg-box v-model="tempShowBillDetailWin" width="1000" class="h-form-table-layer" :maximize="true"
...
<script>
  ...
  export default {
    ...
    computed: {
      tempShowBillDetailWin: {
        get() {
          return this.showBillDetailWin;
        },
        set() {
        }
      }
    ...
    methods: {
      ...
      doubleHandleClick(arr) {
        this.currentSelectRow = arr;
        this.$emit("billDetailSelectSubmit", this.currentSelectRow);
      },
      //查询
      formSearch() {
        this.currentSelectRow = [];
      ...
      submitForm() {
        ...
        // 选择客户
        this.$emit("billDetailSelectSubmit", this.currentSelectRow[0]);
      },
      handleClose() {
        this.$emit("showBillDetailWinClose", "");
      }
    }
  };
</script>
