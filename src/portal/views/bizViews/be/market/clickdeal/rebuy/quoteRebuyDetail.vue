<template>
  <div>
        ...
        <h-row :gutter="16">
          ...
          <h-col span="6">
            <h-row>
              ...
              <h-col span="15" class="val">{{viewDatailForm.saleBrchCode}}</h-col>
            </h-row>
          </h-col>
          <h-col span="6">
            <h-row>
              <h-col span="9" class="label">{{$t("m.i.be.custProductNo")}}：</h-col>
              <h-col span="15" class="val">{{viewDatailForm.saleProductName}}</h-col>
            </h-row>
          </h-col>
        </h-row>
        <br>
        <h-row :gutter="16">
          ...
          <h-col span="6">
            <h-row>
              ...
              <h-col span="15" class="val">{{viewDatailForm.saleTraderNo}}</h-col>
            </h-row>
          </h-col>
          <h-col span="6">
            <h-row>
          ...
          <h-row :gutter="16">
            ...
            <h-col span="6">
              <h-row>
                ...
                <h-col span="15" class="val">{{formatAmt(viewDatailForm.totalAmt)}}</h-col>
              </h-row>
            </h-col>
            <h-col span="6">
              <h-row>
            ...
            <h-col span="6">
              <h-row>
                ...
                <h-col span="15" class="val">{{formatAmt(viewDatailForm.firstPayInterest)}}</h-col>
              </h-row>
            </h-col>
          </h-row>
          <br>
          <h-row :gutter="16">
            <h-col span="6">
              <h-row>
                ...
                <h-col span="15" class="val">{{formatAmt(viewDatailForm.firstSettleAmt)}}</h-col>
              </h-row>
            </h-col>
            <h-col span="6">
              <h-row>
            ...
            <h-col span="6">
              <h-row>
                ...
                <h-col span="15" class="val">{{formatRate(viewDatailForm.yieldRatePct)}}</h-col>
              </h-row>
            </h-col>
          </h-row>
      </div>
      <h-datagrid :columns="billsColumns" :autoLoad="autoLoad" :url="billsUrl" :bindForm="billFormItem"
                  ref="billsDatagrid">
        <div slot="toolbar" class="pull-left">
          <h-button type="primary" @click="exportTable">{{$t("m.i.common.exportExcel")}}</h-button>
        </div>
      </h-datagrid>
      <div slot="footer">
      </div>
    ...
    <common-excel-download @handleSubmit="exportList" @showExcelTemplateWinClose="showExcelTemplateWinClose"
                           ...
                           :rows="rows"></common-excel-download>
  </div>
</template>
<script>
  ...
  export default {
    ...
    data() {
      return {
        batchUrl: "/be/market/clickdeal/rebuy/rebuyReply/func_getRcvRebuyBatchSub",
        billsUrl: "/be/market/clickdeal/rebuy/rebuyReply/func_getClickDealRebuyBillList",
        billsColumns: [
          {
            type: "selection",
            width: 50,
          ...
          },
          {
            ...
            sortable: true,
            render: (h, params) => {
              if (this.viewDatailForm.clearStatus !== "R20") {
    ...
    methods: {
      ...
      exportTable() {
        ...
        if (this.$refs.billsDatagrid.selectIds.length !== 0) {
          this.rows = this.$refs.billsDatagrid.selectIds.length;
        } else {
          this.rows = this.$refs.billsDatagrid.total;
        }
      ...
      exportList(field, exportType) {
        ...
        let url = window.LOCAL_CONFIG.API_HOME + "/be/market/clickdeal/rebuy/rebuyReply/func_downloadRebuyInfo";
        if (this.rootUrl) {
          url = window.LOCAL_CONFIG.API_HOME + this.rootUrl + "/func_downloadRebuyInfo";
        }
        if (exportType === "1") {
          let params = null;
          params = {
            ids: this.$refs.billsDatagrid.selectIds,
            batchId: this.subBatchId,
            pageSize: this.$refs.billsDatagrid.selectIds.length,
            field: field,
          ...
          input1.type = "text";
          input1.name = "ids";
          input1.value = this.$refs.billsDatagrid.selectIds;
    ...
    watch: {
      rebuyDetailWin(i) {
        if (i == true) {
          if (this.rootUrl) {
            this.batchUrl = this.rootUrl + "/func_getRcvRebuyBatchSub";
            this.billsUrl = this.rootUrl + "/func_getClickDealRebuyBillList";
          } else {
            this.batchUrl = "/be/market/clickdeal/rebuy/rebuyReply/func_getRcvRebuyBatchSub";
            this.billsUrl = "/be/market/clickdeal/rebuy/rebuyReply/func_getClickDealRebuyBillList";
          }
          this.subBatchId = this.batchId;
          post({batchId:null,subBatchId: this.subBatchId}, this.batchUrl).then(res => {
          ...
          this.$nextTick(() => {
            this.billFormItem.subBatchId = this.batchId;
