<template>
  <div class="layout">
      ...
      <h-col span="24">
        <div>
          ...
          <h-datagrid :columns="supplColumns"
                      url="/be/market/rediscount/rediscSupplRegistApply/func_queryRediscountSupplInfo"
                      :bindForm="formItem"
                      :row-select="true"
                      :has-select="false"
                      ref="datagrid">
    ...
    <h-msg-box v-model="addOrEditWin" width="600" @on-close="addOrEditWinClose" class="h-form-search-layer"
      ...
      <p slot="header">
        <span v-if="addOrEditType==='add'">新增再贴现补充登记信息</span>
        <span v-if="addOrEditType==='modify'">修改再贴现补充登记信息</span>
      </p>
      <div>
        <h-form :model="addOrEditForm" :label-width="115" ref="addOrEditForm" cols="1" class="h-form-search">
          <h-form-item :label="$t('m.i.be.brchType')" prop="brchType" required>
            <h-select v-model="addOrEditForm.brchType" :clearable="false" placeholder="" @on-change="changeBrchType"
                      :readonly="isBrchTypeReadOnly">
              <h-option value="1">贴现行</h-option>
              <h-option value="2">出票人开户行</h-option>
            </h-select>
          </h-form-item>
          <h-form-item :label="$t('m.i.be.msgType')" prop="msgType" required>
            <h-select v-model="addOrEditForm.msgType" :clearable="false" placeholder="" @on-change="changeMsgType"
          ...
          <bill-range :formItem="addOrEditForm" :rangeProps="['billRangeStart','billRangeEnd']"
                      ...
                      v-if="addOrEditForm.brchType==='1'&&(addOrEditForm.billOrigin==='CS02'||addOrEditForm.billOrigin==='CS03')">
          </bill-range>
         <!-- <h-form-item prop="stdAmt" :label="$t('m.i.billInfo.stdAmt')" v-if="addOrEditForm.stdAmt">
            <h-typefield v-model="addOrEditForm.stdAmt" :maxlength="20" placeholder="" bigTips readonly></h-typefield>
          </h-form-item>-->
          <!-- 子票区间 -->
          ...
          <common-select :label="$t('m.i.billInfo.billOrigin')" prop="billOrigin"
                         v-model="addOrEditForm.billOrigin" :dictList="billOriginList" required readonly
                         v-if="addOrEditForm.billClass==='ME02'"></common-select>
          ...
          <h-form-item prop="corpScale" :label="$t('m.i.common.corpScale')" required>
            <h-select v-model="addOrEditForm.corpScale" placeholder="" :readonly="isReadOnly||corpScaleReadOnly">
            <h-option v-for="item in corpScaleList" :value="item.key" :key="item.key">{{ item.value }}</h-option>
            </h-select>
          </h-form-item>
          <h-form-item prop="industryCategory" :label="$t('m.i.common.industryCategory')" required>
            <h-select v-model="addOrEditForm.industryCategory" placeholder=""
                      :readonly="isReadOnly||industryCategoryReadOnly">
            <h-option v-for="item in industryCategoryList" :value="item.key" :key="item.key">{{ item.value }}
              </h-option>
            </h-select>
          </h-form-item>
          <h-form-item :label="$t('m.i.common.isArc')" prop="isArc" required>
            <h-select v-model="addOrEditForm.isArc" :maxlength=60 placeholder="" :readonly="isReadOnly||isArcReadOnly">
            <h-option v-for="item in isList" :value="item.key" :key="item.key">{{ item.value }}</h-option>
            </h-select>
          </h-form-item>
          <h-form-item :label="$t('m.i.common.isGreenCorp')" prop="isGreenCorp" required>
            <h-select v-model="addOrEditForm.isGreenCorp" :maxlength=60 placeholder=""
                      :readonly="isReadOnly||isGreenCorpReadOnly">
            <h-option v-for="item in isList" :value="item.key" :key="item.key">{{ item.value }}</h-option>
            </h-select>
          </h-form-item>
          <h-form-item :label="$t('m.i.common.isPrivateCorp')" prop="isPrivateCorp" required>
...
import {post, getDictListByGroups, getDictValueFromMap, formatNumber, getConnectMode,accDiv} from "@/api/bizApi/commonUtil";
  ...
  export default {
      ...
      return {
        ...
        supplColumns: [
          ...
          },
          {
            ...
            sortable: true,
            render: (h, params) => {
              let registerBillMoney = formatNumber(params.row.registerBillMoney, 2, ',');
              return h("span", {
                domProps: {
        ...
        fileParams: {
          ...
          columns: [
            ...
            },
            {
              ...
              render: (h, params) => {
                let result = "";
                if (params.row.msgType === "1") {
                  result = "贴现申请人";
                } else if (params.row.msgType === "2") {
                  result = "出票人";
                }
                return h("span",
                  {
    ...
    mounted() {
      getDictListByGroups("DraftTypeCode,CDMedia,DraftTypeCode,Yon,RediscSupplStatus,CorpScale,Industry,AcptRgstStatus,BillOrigin").then(res => {
    ...
    methods: {
      ...
      singleAdd(type) {
        ...
        if (this.addOrEditType === "modify") {
          ...
          let url = "/be/market/rediscount/rediscSupplRegistApply/func_getModifyRediscountSupplInfoById";
            ...
            if (res && res.data) {
              ...
              if (retCode === "000000") {
                this.$nextTick(() => {
                  ...
                  this.addOrEditForm.brchType = obj.brchType;
                  this.addOrEditForm.msgType = obj.msgType;
                  this.addOrEditForm.billNo = obj.billNo
                  this.addOrEditForm.billClass = obj.billClass;
                  this.addOrEditForm.billType = obj.billType;
                  this.addOrEditForm.billRangeStart = obj.billRangeStart;
                  this.addOrEditForm.billRangeEnd = obj.billRangeEnd;
                  this.addOrEditForm.stdAmt = obj.stdAmt;
                  ...
                  if (this.addOrEditForm.billClass === "ME01") {
                    ...
                    if (this.mode === '2' || this.mode === '6') {
                      ...
                      this.isPrivateCorpReadOnly = this.isCorpReadOnly(this.addOrEditForm.isPrivateCorp);
                    } else {
                    ...
                    this.isMsgTypeReadOnly = true;
                    this.isReadOnly = false;
                    this.isBrchTypeReadOnly = true;
                  }
                  this.addOrEditForm.billMoney = obj.billMoney;
                  this.addOrEditForm.originBillRangeStart = obj.originBillRangeStart;
                  this.addOrEditForm.originBillRangeEnd = obj.originBillRangeEnd;
                  this.addOrEditForm.originBillMoney = obj.originBillMoney;
                  this.addOrEditForm.registerBillRangeStart = obj.registerBillRangeStart;
                  this.addOrEditForm.registerBillRangeEnd = obj.registerBillRangeEnd;
                  this.addOrEditForm.registerBillMoney = obj.registerBillMoney;
                  this.addOrEditForm.isAcpt = obj.isAcpt;
                  this.addOrEditForm.dueDt = this.$moment(obj.dueDt.toString(), "YYYY-MM-DD").format('YYYY-MM-DD');
                  this.addOrEditForm.remitDt = this.$moment(obj.remitDt.toString(), "YYYY-MM-DD").format('YYYY-MM-DD');
                  this.addOrEditForm.corpScale = obj.corpScale;
                  this.addOrEditForm.industryCategory = obj.industryCategory;
                  this.addOrEditForm.isArc = obj.isArc;
                  this.addOrEditForm.isGreenCorp = obj.isGreenCorp;
                  this.addOrEditForm.isPrivateCorp = obj.isPrivateCorp;
        ...
        this.$refs["addOrEditForm"].validate(valid => {
          if (valid) {
            let url = this.addOrEditType === "add" ?
              ...
              "/be/market/rediscount/rediscSupplRegistApply/func_updateRediscountSupplInfo";
      ...
      changeRegisterBillMoney() {
        if (this.addOrEditForm.registerBillMoney) {
          ...
          if (this.addOrEditForm.stdAmt && this.addOrEditForm.stdAmt !== 0
            ...
            this.addOrEditForm.registerBillRangeEnd = accDiv(this.addOrEditForm.registerBillMoney, this.addOrEditForm.stdAmt) + this.addOrEditForm.registerBillRangeStart - 1;
          }
        }
      },
        ...
        post({
          ...
          if (res) {
            ...
            if (retCode === "000000") {
              ...
              this.addOrEditForm.isPrivateCorp = res.data.retData.isPrivateCorp;
              if (this.addOrEditForm.billClass === "ME01") {
                if (this.mode ==='2'||this.mode ==='6'){
                  this.isReadOnly = false;
                  this.corpScaleReadOnly = this.isCorpReadOnly(this.addOrEditForm.corpScale);
