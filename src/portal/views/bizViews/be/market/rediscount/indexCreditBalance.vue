        ...
        <div>
              ...
              <h-form ref="formItem" :model="formItem" :label-width="90" class="h-form-search" cols="4">
                ...
                <!--  <h-form-item :label="$t('m.i.common.brchName')" prop="brchName">
                  ...
                  <h-form-item :label="$t('m.i.common.activeFlag')" prop="activeFlag">
                    <h-select  v-model="formItem.activeFlag" placeholder="" :clearable="false">
          ...
          <!--数据展示表格-->
              <h-datagrid
                :columns="columns"
                highlight-row
                url="/be/market/rediscount/indexCreditBalance/func_queryCreditBalance"
                :bindForm="formItem"
    ...
    data(){
      return {
        ...
        columns : [
          ...
          {
            title: this.$t('m.i.be.lastCreditBalance'),
            key: 'lastCreditBalanceAmtStr',
            hiddenCol: false,
            // width:160,
            render: (h, params) => {
              return h('div', [
                h('span', formatNumber(params.row.lastCreditBalanceAmtStr, 2, ','))
              ])
            },
          },
          {
            title: this.$t('m.i.be.creditBalance'),
            key: 'creditBalanceAmtStr',
            hiddenCol: false,
            render: (h, params) => {
              return h('div', [
                h('span', formatNumber(params.row.creditBalanceAmtStr, 2, ','))
              ]);
            },
          },
          {
            title: this.$t('m.i.be.creditSumBalance'),
            key: 'creditSumBalanceAmtStr',
            hiddenCol: false,
            render: (h, params) => {
              return h('div', [
                h('span', formatNumber(params.row.creditSumBalanceAmtStr, 2, ','))
              ]);
            },
          },
          {
