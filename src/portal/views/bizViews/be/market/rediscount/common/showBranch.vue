    ...
    <div class="h-tree-search">
      ...
      <div class="h-modal-header-wrapper">
        <div class="h-modal-header-btn">
          <h-button type="primary" :disabled="expandDisabled" @click='expandTree(branchList)'>
            {{$t("m.i.common.expand")}}
          </h-button>
          <h-button type="primary" :disabled="!expandDisabled" @click='unExpandTree(branchList)'>
      ...
      <div class="h-tree-content" style="position:relative;" :style="{height: mTreeHeight + 'px'}">
        <h-tree v-if="true" :data="branchList" ref='branchTree' class='h-tree-self' onlyUpdateDown multiple show-checkbox></h-tree>
        <h-tree v-else :data="branchList" ref='branchTree' class='h-tree-self' ></h-tree>
