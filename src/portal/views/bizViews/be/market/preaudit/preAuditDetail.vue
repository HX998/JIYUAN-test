<template>
    <div>
        <h-msg-box maximize v-model="tempPreAuditDetailWin" width="1000" @on-close="handleClose">
            <p slot="header">
                <span>查看票据明细</span>
            </p>
            <div class="text-body">
                <h-row :gutter="16">
                    ...
                    <h-col span="6">
                        <h-row>
                            ...
                            <h-col span="15" class="val">{{format("TrDir",viewDatailForm.trDir)}}</h-col>
                        </h-row>
                    </h-col>
                    <h-col span="6">
                        <h-row>
                    ...
                    <h-col span="6">
                        <h-row>
                            ...
                            <h-col span="15" class="val">{{viewDatailForm.brchName}}</h-col>
                        </h-row>
                    </h-col>
                </h-row>
                <br>
                <h-row :gutter="16">
                    ...
                    <h-col span="6">
                        <h-row>
                            ...
                            <h-col span="15" class="val">{{viewDatailForm.custBrchCode}}</h-col>
                        </h-row>
                    </h-col>
                    <h-col span="6">
                        <h-row>
                            <h-col span="9" class="label">{{$t('m.i.be.custProductNo')}}：</h-col>
                            <h-col span="15" class="val">{{viewDatailForm.custProductName}}</h-col>
                        </h-row>
                    </h-col>
                </h-row>
                <br>
                <h-row :gutter="16">
                    <h-col span="6">
                        <h-row>
                            ...
                            <h-col span="15" class="val">{{viewDatailForm.custTraderName}}</h-col>
                        </h-row>
                    </h-col>
                    <h-col span="6">
                        <h-row>
                            <h-col span="9" class="label">{{$t('m.i.be.custTraderNo')}}：</h-col>
                            <h-col span="15" class="val">{{viewDatailForm.custTraderNo}}</h-col>
                        </h-row>
                    </h-col>
                    <h-col span="6">
                        <h-row>
                ...
                <div v-if="busiType=='BT02'">
                    ...
                    <h-row :gutter="16">
                        ...
                        <h-col span="6">
                            <h-row>
                                ...
                                <h-col span="15" class="val">{{format("CdSelectMode",viewDatailForm.selectType)}}
                                </h-col>
                            </h-row>
                        </h-col>
                        <h-col span="6">
                    ...
                    <h-row :gutter="16">
                        ...
                        <h-col span="6">
                            <h-row>
                                <h-col span="9" class="label">{{$t('m.i.be.settleAmtFirst')}}:</h-col>
                                <h-col span="15" class="val">{{formatMoney(viewDatailForm.firstSettleAmt)}}</h-col>
                            </h-row>
                        </h-col>
                        <h-col span="6">
                            <h-row>
                                <h-col span="9" class="label">{{$t('m.i.be.dueSettleAmt')}}:</h-col>
                ...
                <div v-if="busiType=='BT03'">
                    ...
                    <h-row :gutter="16">
                        <h-col span="6">
                            <h-row>
                                <h-col span="9" class="label">{{$t('m.i.be.dueTransRate')}}:</h-col>
            ...
            <h-datagrid :columns="billsColumns" :autoLoad="false" :url="billListURL" :bindForm="formItem" ref="bills">
...
<script>
  import { getDictListByGroups,formatNumber, getDictValueFromMap, post, formatBillRange } from "@/api/bizApi/commonUtil";
  ...
  export default {
    ...
    props: {
      ...
      downloadURL: {
        type: String,
        default() {
          return "/be/market/preaudit/preAuditMain/func_downloadPreAuditInfo";
        }
      },
      batchURL: {
        type: String,
        default() {
          return "/be/market/preaudit/preAuditMain/func_findPreAuditById";
        }
      },
      billListURL: {
        type: String,
        default() {
          return "/be/market/preaudit/preAuditMain/func_findBillByBatchIdByPage";
        }
      }
    },
    methods: {
    ...
    watch: {
      tempOpenMethod(val) {
        if (val) {
          post({id: this.batchId}, this.batchURL).then(res => {
            if (res.data.retCode === "000000") {
              if (res.data.retData != null) {
                this.tempPreAuditDetailWin = true;
          ...
          this.tempPreAuditDetailWin = false;
        }
      }
    },
    computed: {
