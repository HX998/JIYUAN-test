    ...
    <h-msg-box v-model="tempPickPackBillWin" width="1000" @on-close="handleClose"  class="h-form-table-layer" :maximize="true" :mask-closable="false">
      ...
      <div>
        ...
        <h-datagrid :columns="pickPackBillColumns"
                    url="/be/market/preaudit/preAuditMain/func_queryPacketBill"
                    highlight-row
                    :autoLoad="false"
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      pickPackBillClose(data){
        this.pickPackBillSelectList = [];
        this.$refs.pickPackBillForm.resetFields();
        this.$emit("pickPackBillWinClose", data);
      },
      pickPackBillInit(){
      ...
      pickPackBillStock(){
        ...
        let url="/be/market/preaudit/preAuditMain/func_pickPicketBill";
        post({ preAuditBatchId: batchId ,buyBatchId:list[0].id}, url).then(res => {
          if (res) {
            let retCode = res.data.retCode;
            if (retCode === "000000") {
              this.$msgTip.success(this);
              this.pickPackBillSelectList = [];
              this.pickPackBillClose(res.data.retData);
              this.submitFlag=false;
            } else {
              this.submitFlag=false;
