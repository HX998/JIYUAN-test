...
<template>
  <div class="layout">
      ...
      <h-col span="24">
        <div>
          ...
          <h-datagrid
            :columns="columns"
            highlightRow
            url="/be/market/anonyclick/sale/saleApplyConfirm/func_queryAnonySaleReply"
            :bindForm="formItem"
            :onCurrentChange="handleCurrentChange"
            :onCurrentChangeCancel="handleCurrentChangeCancel"
            ref="datagrid">
            <div slot="toolbar" class="pull-left">
              <h-button type="primary" @click="sendSaleApply()">{{$t("m.i.common.send")}}</h-button>
              <h-button type="primary" @click="returnSaleApply()">{{$t("m.i.be.returnApply")}}</h-button>
            </div>
          </h-datagrid>
        </div>
      </h-col>
    ...
    <!-- 详情页面 -->
    <sale-apply-detail :saleApplyDetailWin="saleApplyDetailWin" :batchParams="batchParams"
                        @saleApplyDetailWinClose="saleApplyDetailWinClose" ref="saleApplyDetail"></sale-apply-detail>
...
<script>
  ...
  export default {
    ...
    components: {
      SaleApplyDetail: () => import(/* webpackChunkName: "be/market/anonyclick/sale/saleApplyDetail" */`@/views/bizViews/be/market/anonyclick/sale/saleApplyDetail`)
    },
    data() {
      return {
        ratePctRule :[{
    ...
    mounted() {
      this.getDictListByGroups("AnonyClickBatchStatus,BusiType,TrDir,Yon,TradeRange,DraftTypeCode,CDMedia,ClearSpeed,SettleType,ClearType,CreditMajor,ClearMode,TenorVarieties,QuoteStatus,AnonyClickBillStatus,BillOrigin").then(res => {
    ...
    methods: {
        ...
        if (this.currentSelectRow != null && this.currentSelectRow.length !== 0) {
          this.$hMsgBox.confirm({
            ...
            onOk: () => {
              ...
              let url="/be/market/anonyclick/sale/saleApplyConfirm/func_sendAnonySaleApply";
      ...
      returnSaleApply(){
        if (this.currentSelectRow != null && this.currentSelectRow.length !== 0) {
          this.$hMsgBox.confirm({
            ...
            onOk: () => {
              let params={ id: this.currentSelectRow.id };
              let url="/be/market/anonyclick/sale/saleApplyConfirm/func_cancelAnonySaleApply";
              this.sendPost(params,url);
            }
          });
        } else {
      ...
      billDetail(id) {
        ...
        this.batchParams.batchUrl="/be/market/anonyclick/sale/saleApplyConfirm/func_findAnonySaleBatchById";
        this.batchParams.billsUrl="/be/market/anonyclick/sale/saleApplyConfirm/func_queryAnonySaleBillList";
        this.batchParams.exportUrl="/be/market/anonyclick/sale/saleApplyConfirm/func_downloadSaleInfo";
