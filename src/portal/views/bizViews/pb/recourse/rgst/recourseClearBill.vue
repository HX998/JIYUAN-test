<template>
  <div class="layout">
    <h-row name="flex" class="layout-menu-wrapper">
      <h-col span="24">
        <div>
          ...
          <div class="h-form-search-box">
            <h-panel class="clearfix" :class="{'h-form-overhd':!ifShowMore}">
              <h-form :model="formItem" :label-width="90" ref="formItem" cols="4" class="h-form-search">
                ...
                <h-form-item :label="$t('m.i.be.recName')" prop="recourseName">
                  <h-input v-model="formItem.recourseName" placeholder="" :maxlength=60 :lengthByByte="false"></h-input>
                </h-form-item>
                ...
                <h-form-item :label="$t('m.i.be.payDate')" prop="recPayDt">
                  <h-date-picker :value="recPayDt" format="yyyy-MM-dd" type="daterange" showFormat
          ...
          <h-datagrid
            :columns="columns"
            url="/pb/recourse/recourseClear/func_querySubmitRecourseClearBillByPage"
            :bindForm="formItem"
    ...
    <h-msg-box v-model="addOrEditWin" width="1000" class="h-form-table-layer" :maximize="true">
      ...
      <h-panel>
        <h-form :model="addForm" :label-width="115" ref="addForm" cols="2" class="h-form-search">
          <h-form-item :label="$t('m.i.be.rgstApplDt')" prop="recourseDt" placeholder=""  :required="isRequired">
            <h-date-picker v-if="type=='view'" readonly :value="addForm.recourseDt" placeholder="" showFormat :title="addForm.recourseDt"
                           type="date"></h-date-picker>
            <h-date-picker v-else v-model="addForm.recourseDt" :options="options1" showFormat type="date"></h-date-picker>
          </h-form-item>
          ...
          <h-form-item :label="$t('m.i.billInfo.billMoneyByUnit')" prop="billMoney" :required="isRequired">
          ...
          <h-form-item :label="$t('m.i.be.payDate')" prop="recPayDt" :required="isRequired">
          ...
          <h-form-item :label="$t('m.i.be.toRecName')" prop="rcvgName" :required="isRequired">
            <h-input v-model="addForm.rcvgName" :maxlength=60 readonly v-if="type=='add'" :lengthByByte="false"></h-input>
            <h-input v-model="addForm.rcvgName" :maxlength=60 readonly v-if="type=='modify'" :lengthByByte="false"></h-input>
          </h-form-item>
          <h-form-item :label="$t('m.i.be.recName')" prop="recourseName" :required="isRequired">
            <h-input v-model="addForm.recourseName" :maxlength=60 :lengthByByte="false"></h-input>
          </h-form-item>
          <h-form-item :label="$t('m.i.be.recAcctNo')" prop="recourseAcctNo">
            <h-input v-model="addForm.recourseAcctNo" :maxlength=32></h-input>
          </h-form-item>
          <show-cpes-branch :label="$t('m.i.be.recBankNo')"
                            :bankNo.sync="addForm.recourseBankNo"  :cpesBrchName.sync="addForm.recourseBankName"
                            :isQueryByBankNo="true"    :msgBoxWin="addOrEditWin"
                            datagridUrl="/cpes/branch/queryBranchList" prop="branchName"
                            queryUrl="/cpes/branch/queryBranchList"></show-cpes-branch>

          <h-form-item :label="$t('m.i.be.recSocCode')" prop="recourseSocCode" :required="isRequired" :validRules="socCodeRule"
                       class="h-form-long-label">
            <h-input v-model="addForm.recourseSocCode" :maxlength=18></h-input>
          </h-form-item>
          <h-form-item prop="recourseBankNo" v-show="false">
            <h-input v-model="addForm.recourseBankNo"></h-input>
          </h-form-item>
          ...
          <h-form-item :label="$t('m.i.be.recourseClearRemark')" prop="recourseRemark" class="h-form-height-auto">
            <h-input type="textarea" :autosize="{minRows: 1,maxRows: 3}" v-model="addForm.recourseRemark"
    ...
    <h-msg-box v-model="pickRecouBillWin" width="1000" :maximize="true" @on-maximize="onMaximize"
      ...
      <div class="h-form-overhd">
        <h-form :model="billFormItem" :label-width="90" ref="billFormItem" cols="3" class="h-form-search">
          ...
          <h-form-item :label="$t('m.i.billInfo.billType')" prop="billType">
            <h-select v-model="billFormItem.billType" placeholder="" showTitle>
              <h-option v-for="item in billTypeList" :value="item.key" :key="item.key">{{item.value }}</h-option>
            </h-select>
          </h-form-item>
      ...
      <h-datagrid
        highlight-row
        url="/pb/recourse/recourseClear/func_queryRecPickBill"
        :columns="billColumns"
      ...
      return {
        ...
        columns: [
          ...
          },
          {
            ...
            hiddenCol: false,
            render: (h, params) => {
              let recPayDt = params.row.recPayDt == null ? "" :this.$moment(params.row.recPayDt, "YYYYMMDD").format("YYYY-MM-DD");
        ...
        addForm: {
          ...
          rcvgBrchCode: "",
          recourseName: "",
          rcvgName: this.$store.getters.userInfo.brchName,
          recourseAcctNo: "",
          recourseBankNo: "",
          recourseBankName: "",
          recourseSocCode: "",
    ...
    methods: {
      ...
      addFormReset() {
        this.$nextTick(() => {
          ...
          this.addForm.id = "";
          this.addForm.recourseBankName = "";
          this.addForm.recourseBankNo = "";
          this.addForm.recourseDt = this.$moment(window.sessionStorage.getItem("workDate"), "YYYYMMDD").format("YYYY-MM-DD");
        });
      },
      handleAddForm(str) {
        this.type = str;
        ...
        if (this.type == "modify" || this.type == "view") {
          ...
          this.$nextTick(() => {
            ...
            this.addForm.rcvgMemberId = this.currentSelectRow.rcvgMemberId;
            ...
            this.addForm.riskStatus = this.currentSelectRow.riskStatus;
            ...
            this.addForm.rcvgBrchCode = this.currentSelectRow.rcvgBrchCode;
            this.addForm.recourseName = this.currentSelectRow.recourseName;
            this.addForm.recourseAcctNo = this.currentSelectRow.recourseAcctNo;
            this.addForm.recourseBankNo = this.currentSelectRow.recourseBankNo;
            this.addForm.recourseBankName = this.currentSelectRow.recourseBankName;
            this.addForm.recourseSocCode = this.currentSelectRow.recourseSocCode;
            ...
            this.addForm.recourseDt = this.currentSelectRow.recourseDt;
            this.addForm.transId = this.currentSelectRow.transId;
            if (this.currentSelectRow.recourseDt) {
      ...
      doubleHandleClick(arr) {
        ...
        this.billFormItem.billType = "AC01";
      },
      onMaximize() {
        setTimeout(() => {
      ...
      trunBill(info) {
        if (info === null) {
          ...
          this.addForm.hldrId = info.hldrId;
          ...
          this.addForm.riskStatus = info.riskStatus;
          if (this.addForm.recourseDt) {
            this.addForm.recourseDt = this.$moment(info.recourseDt, "YYYYMMDD").format("YYYY-MM-DD");
          } else {
            this.addForm.recourseDt = info.recourseDt;
          }
          if (info.dueDt) {
      ...
      handlerPayDtChange(value) {
        this.addForm.recPayDt = value.replace(/-/g, "");
      },
      formSearch() {
        /**清除多页选择的数据*/
      ...
      formSearchReset() {
        ...
        this.formItem.recourseName = "";
        this.formItem.minBillMoney = "";
        this.formItem.maxBillMoney = "";
        this.formItem.dueDate = "";
        this.formItem.minDueDt = "";
        this.formItem.maxDueDt = "";
        this.formItem.recPayDt = "";
        this.formItem.minRecPayDt = "";
        this.formItem.maxRecPayDt = "";
      ...
      submitForm() {
        ...
        this.$refs["addForm"].validate(valid => {
          if (valid) {
            // 新增或修改的url
            let url = this.type == "add" ? "/pb/recourse/recourseClear/func_saveRecourseClearBill" : "/pb/recourse/recourseClear/func_editRecourseClearBill";
            ...
            this.submitMsg = msg;
            this.addForm.recPayDt = this.$moment(this.addForm.recPayDt, "YYYYMMDD").format("YYYYMMDD");
            this.addForm.dueDt = this.$moment(this.addForm.dueDt, "YYYYMMDD").format("YYYYMMDD");
            let recourseDt = this.addForm.recourseDt;
            if (recourseDt) {
              this.addForm.recourseDt = this.$moment(recourseDt).format("YYYYMMDD");
            }
      ...
      handleComfirm() {
        ...
        if (list != null && list.length > 0) {
          ...
          this.$msgTip.info(this.$t("m.i.common.chooseAtLeastOneRecord"));
        }
      },
      //提交复核
      submitToReview() {
      ...
      submitRecheck() {
        ...
        post({ ids: ids }, "/pb/recourse/recourseClear/func_submitRecourseClearBill").then(res => {
          if (res) {
            let retCode = res.data.retCode;
            if (retCode == "000000") {
              this.$msgTip.success(this, { info: "操作成功" });
      ...
      handledel() {
        ...
        post({ ids: ids }, "/pb/recourse/recourseClear/func_deleteRecourseClearBill").then(res => {
          if (res) {
            let retCode = res.data.retCode;
            if (retCode == "000000") {
              this.$msgTip.success(this, { info: "操作成功" });
