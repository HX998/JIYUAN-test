<template>
  <div class="layout">
    <h-row name="flex" class="layout-menu-wrapper">
      <h-col span="24">
        <div>
            ...
            <h-panel class="clearfix" :class="{'h-form-overhd':!ifShowMore}">
              <h-form :model="formItem" :label-width="90" ref="formItem" cols="4" class="h-form-search">
                ...
                <h-form-item :label="$t('m.i.be.toRecName')" prop="rcvgName">
                  <h-input v-model="formItem.rcvgName" placeholder="" :maxlength="250"></h-input>
                </h-form-item>
          ...
          <h-datagrid
            :columns="columns"
            url="/pb/recourse/recourseRgst/func_queryRecourseBillForApply"
            :bindForm="formItem"
            :hasSelect="false"
            rowSelect
            :notSetWidth="true"
    ...
    <h-msg-box v-model="addOrEditWin" width="1000" class="h-form-table-layer" :maximize="true">
      ...
      <h-panel>
        <h-form :model="addForm" :label-width="115" ref="addForm" cols="2" class="h-form-search">
          <h-form-item :label="$t('m.i.be.rgstApplDt')" prop="recourseDt" :required="isRequired">
            ...
            <h-date-picker v-else v-model="addForm.recourseDt" placeholder="" readonly showFormat type="date"></h-date-picker>
          </h-form-item>
          ...
          <h-form-item :label="$t('m.i.common.remark')" prop="recourseRemark">
            <h-input v-model="addForm.recourseRemark" :maxlength="256" :lengthByByte="false"></h-input>
          </h-form-item>
          <h-form-item :label="$t('m.i.be.recName')" prop="recourseName" :required="isRequired">
            <h-input v-model="addForm.recourseName" :maxlength="60" v-if="type=='add'" :lengthByByte="false"></h-input>
            <h-input v-model="addForm.recourseName" :maxlength="60" v-if="type=='modify'" readonly :lengthByByte="false"></h-input>
          </h-form-item>
          <h-form-item :label="$t('m.i.be.toRecName')" prop="rcvgName" :required="isRequired">
            <h-input v-model="addForm.rcvgName" :maxlength="60" :lengthByByte="false"></h-input>
          </h-form-item>
          <h-form-item :label="$t('m.i.be.toRecAcctNo')" prop="rcvgAcctNo" :validRules="custAcctNoRule">
            <h-input v-model="addForm.rcvgAcctNo" :maxlength="32"></h-input>
          </h-form-item>
          <h-form-item :label="$t('m.i.be.toRecBankNo')" prop="rcvgBankNo" :validRules="bankNoRule" class="h-form-long-label">
            <h-input v-model="addForm.rcvgBankNo" :maxlength="12"></h-input>
          </h-form-item>
          <h-form-item :label="$t('m.i.be.toRecSocCode')" prop="rcvgSocCode" :required="isRequired" :validRules="socCodeRule"
                       class="h-form-long-label">
            <h-input v-model="addForm.rcvgSocCode" :maxlength=18 ></h-input>
          </h-form-item>
    ...
    <h-msg-box v-model="pickRecouBillWin" width="1000" :mask-closable="false" class="h-form-search-layer"
      ...
      <h-datagrid
        highlight-row
        url="/pb/recourse/recourseRgst/func_queryRecPickBill"
        :columns="billColumns"
      ...
      return {
        ...
        columns: [
          ...
          },
          {
            ...
            hiddenCol: false,
            render:(h, params)  => {
              let recourseDt = params.row.recourseDt ? this.$moment(params.row.recourseDt, "YYYY-MM-DD").format("YYYY-MM-DD") : "";
          ...
          },
          {
            ...
            hiddenCol: false,
            render: (h, params) => {
              let list = getDictValueFromMap(this.dictMap, "OffLineRecType", params.row.offLineRecType);
              return h("span",
                {
                  domProps: {
                    title: list
        ...
        addForm: {
          ...
          billClass: "",
          dueDt: "",
          recourseDt: this.$moment(window.sessionStorage.getItem("workDate"), "YYYYMMDD").format("YYYY-MM-DD"),
          recRepDt: "",
          recPayDt: "",
          acctTellerNo: "",
          applTellerNo: "",
    ...
    methods: {
      ...
      handleAddForm(str) {
        ...
        if (this.type == "modify" || this.type == "view") {
          ...
          this.$nextTick(() => {
            ...
            this.addForm.recourseMemberId = this.currentSelectRow.recourseMemberId;
            ...
            this.addForm.rcvgName = this.currentSelectRow.rcvgName;
            this.addForm.rcvgAcctNo = this.currentSelectRow.rcvgAcctNo;
            this.addForm.rcvgBankNo = this.currentSelectRow.rcvgBankNo;
            this.addForm.rcvgSocCode = this.currentSelectRow.rcvgSocCode;
      ...
      trunBill(info) {
        if (info === null) {
          ...
          this.addForm.recourseMemberId = info.recourseMemberId;
          this.addForm.transBrchNo = info.transBrchNo;
          this.addForm.brchName = info.brchName;
          this.addForm.rgstBrchCode = info.recourseBrchCode;
          ...
          this.addForm.ecdsBillStatus = info.ecdsBillStatus;
          this.addForm.cirStatus = info.cirStatus;
          if (this.addForm.cirStatus == 'TF0402_07'){
            this.addForm.offLineRecType = "OLRT02";
          }else{
            this.addForm.offLineRecType = "OLRT01";
          }
          this.addForm.recourseName = info.recourseName;
      ...
      submitForm() {
        this.$refs["addForm"].validate(valid => {
          if (valid) {
            let url = this.type == "add" ? "/pb/recourse/recourseRgst/func_saveRecourseBillRgst" : "/pb/recourse/recourseRgst/func_updateRecourseBillRgst";
      ...
      handledel() {
        ...
        post({ ids: ids }, "/pb/recourse/recourseRgst/func_delRecourseBillRgst").then(res => {
          if (res) {
            let retCode = res.data.retCode;
            if (retCode == "000000") {
      ...
      formatDt() {
        let recourseDt = this.addForm.recourseDt == null ? "" : this.$moment(new Date()).format("YYYY-MM-DD");
        return recourseDt;
      },
      submitRecheck() {
        let list = this.$refs.datagrid.selects;
        ...
        post({ ids: ids }, "/pb/recourse/recourseRgst/func_submitRecourseBillRgst").then(res => {
          if (res) {
            let retCode = res.data.retCode;
            if (retCode == "000000") {
              this.$msgTip.success(this, { info: "操作成功" });
      ...
      licenseBrchNoChange(info) {
          if(info.length>0 && this.licenseFlag===true){
              this.addForm.transBrchNo = info[0].transBrchNo;
              this.addForm.brchName = info[0].brchName;
          }
      },
    },
    mounted() {
      this.getDictListByGroups("BarterTrustStatus,OffLineRecType,CDMedia,DraftTypeCode,CpesFlowStatus,EcdsBillStatusCode,BillStatusCode").then(res => {
