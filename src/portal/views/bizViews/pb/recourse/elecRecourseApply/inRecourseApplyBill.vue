...
<!--                <show-cpes-branch :label="$t('m.i.billInfo.ownershipBankName')" class="h-form-long-label"
          ...
          <h-datagrid
            :columns="columns"
            url="/pb/recourse/elecRecourseApply/inRecourseApplyBill/func_queryRecourseableDraft"
            :bindForm="formItem"
  ...
  export default {
    ...
    data() {
      ...
      return {
        showFileWin:false,
        fileBatchParams:{},
        filePathUrl:"pb/recourse/elecRecourseApply/inRecourseApplyBill/",
...
/*          {
    ...
    methods: {
      ...
      submitToReview() {
        ...
        let flag = true;
        list.forEach(row => {
          if (ownedBrchNo !== "" && ownedBrchNo !== row.hldrBrchNo) {
            this.$msgTip.info(this, { info: "请选择相同的机构下的票据记账" });
            return flag = false;
          }
          ownedBrchNo = row.hldrBrchNo;
        });
      ...
      submitRecheck() {
        ...
        post({ hldrIdList: hldrIdList,billIdList: billIdList,recourseRemark: this.pendingRecourseApplyForm.recourseRemark,
          ownedBrchNo:this.pendingRecourseApplyForm.ownedBrchNo,licenseFlag:this.licenseFlag}, "/pb/recourse/elecRecourseApply/inRecourseApplyBill/func_saveRecourseApplyBillSend").then(res => {
          if (res) {
