            ...
            <h-panel class="clearfix" :class="{'h-form-overhd':!ifShowMore}">
              <h-form :model="billManagerFormItem" :label-width="90" ref="billManagerFormItem" cols="4"
                ...
                <h-form-item :label="$t('m.i.billInfo.billNo')" prop="reverseBillNo">
                  <h-input v-model="billManagerFormItem.reverseBillNo" :maxLength="30"></h-input>
                </h-form-item>
                <h-form-item :label="$t('m.i.billInfo.billMoney')" prop="billMoney">
                  <h-typefield type="money" v-model="billManagerFormItem.billMoney" :maxlength="20" placeholder="" bigTips ></h-typefield>
                </h-form-item>
...
<script>
  ...
  import ImpawnBatchDetail from "@/views/bizViews/ce/impawn/paper/paperUnImpawn/pUnImpawnAccountBatchDetail";

  export default {
    name: "impawnAccountBill",
    components: {
      ImpawnBatchDetail
    },
    data() {
    ...
    watch: {
      billManagerVue(val) {
        if (val) {
          ...
          this.dataUrl =  "/ce/impawn/paper/paperUnImpawn/pUnImpawnAccountMain/func_billlist";
          this.$nextTick(() => {
            this.columns = this.deepClone(this.pUnImpawnAccountColumns);
    ...
    methods: {
      ...
      //获取分录信息
      getEntryInfo(params) {
        this.acctRecordUrl =  "/ce/impawn/paper/paperUnImpawn/pUnImpawnAccountMain/func_getUnImpawnAccount";
      ...
      acctSubmitSure(info) {
        ...
        let url = "/ce/impawn/paper/paperUnImpawn/pUnImpawnAccountMain/func_unImpawnAccount";
      ...
      acctAuthorize(){
        ...
        this.acctSubmitFlag = true;
        let url =  "/ce/impawn/paper/paperUnImpawn/pUnImpawnAccountMain/func_addAcctAuthorize" ;
        post(params, url).then(res => {
          this.acctSubmitFlag = false;
          if (res) {
            let msg = res.data.retMsg;
      ...
      handleAccountBack(params) {
        let url =  "/ce/impawn/paper/paperUnImpawn/pUnImpawnAccountMain/func_unImpawnAccountBack";
        this.$hMsgBox.confirm({
          title: this.$t("m.i.common.confirm"),
          content: "确定记账退回吗",
          onOk: () => {
