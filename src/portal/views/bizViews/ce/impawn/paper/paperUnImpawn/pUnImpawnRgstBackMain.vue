      ...
      <h-col span="24">
        ...
        <div v-if="mainVue">
          ...
          <h-datagrid :columns="columns"
                      highlightRow
                      url="/ce/impawn/paper/paperUnImpawn/pUnImpawnRgstBackMain/func_list"
                      :bindForm="formItem"
        ...
        <!-- 子页面 -->
        <p-un-impawn-rgst-back-batch-info-detail :is-show="billManagerVue" :params="this.batchInfoParams"
                                            ref="pUnImpawnRgstBatchInfoDetail"></p-un-impawn-rgst-back-batch-info-detail>
        <div v-if="billManagerVue">
          <h-datagrid :columns="billManagerColumns"
                      url="/ce/impawn/paper/paperUnImpawn/pUnImpawnRgstBackMain/func_rgstBillList"
                      highlightRow
                      :auto-load="false"
            ...
            <div slot="toolbar" class="pull-left">
              <h-dropdown placement="bottom-start" @on-click="rgstCancelDetail">
                <h-button type="primary">撤回<h-icon name="unfold"></h-icon></h-button>
                <h-dropdown-menu slot="list">
                  <h-dropdown-item name="batch">批次撤回</h-dropdown-item>
                  <h-dropdown-item name="select">选择撤回</h-dropdown-item>
                </h-dropdown-menu>
              </h-dropdown>
...
<script>
  ...
  export default {
    name: "pUnImpawnRgstBackMain",
    components: {
      PUnImpawnRgstBackBatchInfoDetail:()=>import(/* webpackChunkName: "ce/impawn/paper/paperUnImpawn/pUnImpawnRgstBackBatchInfoDetail" */`@/views/bizViews/ce/impawn/paper/paperUnImpawn/pUnImpawnRgstBackBatchInfoDetail`),
    ...
    data() {
      ...
      return {
        formItem: {
          ...
          unImpawnRgstDt :window.sessionStorage.getItem("workDate"),
          queryFlag: "0",
        },
        billManagerFormItem: {
          batchId: '',
        ...
        batchInfoParams: {
          batchId: '',
          unImpawnRgstStatus: 'BTC04,BTC10',
          dictMap: new Map(),
          unImpawnRgstDt :window.sessionStorage.getItem("workDate")
        },
    ...
    methods: {
      ...
      //撤回
      rgstCancelDetail(str) {
        if(str === "batch") {
          this.batchRgstCancelConfirm();
        }
        if(str === "select") {
          this.selectRgstCancelConfirm();
        }
      },
      //批次撤回
      batchRgstCancelConfirm() {
        this.$hMsgBox.confirm({
          title: this.$t("m.i.common.confirm"),
          content: "确定批次撤回吗?",
          onOk: () => {
            this.batchRgstCancel();
          }
        });
      },
      batchRgstCancel() {
        let url = "/ce/impawn/paper/paperUnImpawn/pUnImpawnRgstBackMain/func_cancelRgst";
      ...
      rgstCancel() {
        ...
        let url = "/ce/impawn/paper/paperUnImpawn/pUnImpawnRgstBackMain/func_cancelRgst";
        post({unImpawnIds: ids}, url).then(res => {
          if (res) {
            let msg = res.data.retMsg;
            let retCode = res.data.retCode;
            if (retCode === "000000") {
