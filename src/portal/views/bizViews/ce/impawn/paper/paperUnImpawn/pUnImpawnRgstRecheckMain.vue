      ...
      <h-col span="24">
        ...
        <div v-if="mainVue">
          ...
          <h-datagrid :columns="columns"
                      highlightRow
                      url="/ce/impawn/paper/paperUnImpawn/pUnImpawnRgstRecheckMain/func_list"
                      :bindForm="formItem"
                      :auto-load="false"
        ...
        <!-- 子页面 -->
        <p-un-impawn-rgst-recheck-batch-info-detail :is-show="billManagerVue" :params="this.batchInfoParams"
                                            ref="pUnImpawnRgstBatchInfoDetail"></p-un-impawn-rgst-recheck-batch-info-detail>
        <div v-if="billManagerVue">
          <h-datagrid :columns="billManagerColumns"
                      url="/ce/impawn/paper/paperUnImpawn/pUnImpawnRgstRecheckMain/func_rgstBillList"
                      highlightRow
                      :auto-load="false"
            ...
            <div slot="toolbar" class="pull-left">
              <h-dropdown placement="bottom-start" @on-click="rgstPassDetail">
                <h-button type="primary">通过<h-icon name="unfold"></h-icon></h-button>
                <h-dropdown-menu slot="list">
                  <h-dropdown-item name="batch">批次通过</h-dropdown-item>
                  <h-dropdown-item name="select">选择通过</h-dropdown-item>
                </h-dropdown-menu>
              </h-dropdown>
              <h-dropdown placement="bottom-start" @on-click="rgstBackDetail">
        ...
        <!-- 退回原因录入弹窗 -->
        <h-msg-box v-model="rgstBackWin" width="400" @on-close="rgstBackWinClose" class="h-form-search-layer" :maximize="true" :mask-closable="false">
          <p slot="header">
            <span>退回原因录入</span>
          </p>
          <div>
            <h-form :model="rgstBackForm" :label-width="90" ref="rgstBackForm" cols="1" class="h-form-search">
              <h-form-item prop="returnReason" :label="$t('m.i.ce.paymentReturnReason')" required>
                <h-input v-model="rgstBackForm.returnReason" placeholder="" type="textarea" :rows="3" :canResize="false" :maxlength="100" :lengthByByte="false"></h-input>
              </h-form-item>
            </h-form>
          </div>
          <div slot="footer">
            <div slot="footer">
              ...
              <h-button type="primary" v-else @click="rgstBackSubmit()">{{$t("m.i.ce.returnBack")}}</h-button>
            </div>
          </div>
        </h-msg-box>
...
<script>
  ...
  export default {
    name: "pUnImpawnRgstRecheckMain",
    components: {
      PUnImpawnRgstRecheckBatchInfoDetail:()=>import(/* webpackChunkName: "ce/impawn/paper/paperUnImpawn/pUnImpawnRgstRecheckBatchInfoDetail" */`@/views/bizViews/ce/impawn/paper/paperUnImpawn/pUnImpawnRgstRecheckBatchInfoDetail`),
    ...
    data() {
      ...
      return {
        ...
        formItem: {
          ...
          billClass: 'ME01',
          unImpawnRgstStatusList: 'BTC02,BTC05',
          unimpawnRgstApplTellerNoNotEqual : this.$store.getters.userInfo.userNo,
          auditModeKey : 'ce.unimpawn.is_rgst_check_exclude',
          queryFlag: "0",
        },
        billManagerFormItem: {
          batchId: '',
        ...
        batchInfoParams: {
          batchId: '',
          unImpawnRgstStatus: 'BTC02,BTC05',
          dictMap: new Map(),
          unimpawnRgstApplTellerNoNotEqual : this.$store.getters.userInfo.userNo,
          auditModeKey : 'ce.unimpawn.is_rgst_check_exclude'
        },
      ...
      post({ paramKey: "ce.unimpawn.rgst_audit_mode" }, "/sm/auth/businessParamete/getBusinessParameter").then(res => {
        if (res) {
          ...
          if (retCode === "000000") {
            ...
            if(auditMode == '0'){
              this.formItem.unImpawnRgstStatusList =  'BTC02';
              this.batchInfoParams.unImpawnRgstStatus = 'BTC02';
            }
          }
        }
        this.handleSearch();
    ...
    methods: {
      ...
      rgstPassDetail(str) {
        if(str === "batch") {
          this.batchRgstPassConfirm();
        }
        if(str === "select") {
          this.selectRgstPassConfirm();
        }
      },
      //批次通过
      batchRgstPassConfirm() {
      ...
      batchRgstPass() {
        let url = "/ce/impawn/paper/paperUnImpawn/pUnImpawnRgstRecheckMain/func_checkPass";
      ...
      //退回
      rgstBackDetail(str) {
        this.rgstBackType = str;
        if(str === "batch") {
          this.batchRgstBack();
        }
        if(str === "select") {
          this.selectRgstBackConfirm();
        }
      },
      batchRgstBack() {
      ...
      rgstBackSubmit() {
        if(this.rgstBackType === "select") {
          ...
          this.rgstBackForm.unImpawnIds = ids;
        }
        this.rgstBackForm.batchId = this.batchId;
        this.rgstBackForm.unImpawnRgstStatus = this.formItem.unImpawnRgstStatusList;
        let url = "/ce/impawn/paper/paperUnImpawn/pUnImpawnRgstRecheckMain/func_checkBack";
        this.$refs["rgstBackForm"].validate(valid => {
          if (valid) {
            this.submitFlag = true;
            post(this.rgstBackForm, url).then(res => {
              ...
              if (res) {
                ...
                if (retCode === "000000") {
                  ...
                  if (this.rgstBackType === "batch" || this.$refs.billManagerDatagrid.total === this.$refs.billManagerDatagrid.selects.length) {
                    this.returnMain();
                  } else {
                    this.$refs.pUnImpawnRgstBatchInfoDetail.getBatchInfo();
                    this.$refs.billManagerDatagrid.dataChange(this.$refs.billManagerDatagrid.$refs.gridPage.currentPage);
      ...
      //退回弹窗关闭
      rgstBackWinClose() {
        this.$refs.rgstBackForm.resetFields();
        this.rgstBackForm.unImpawnIds = "";
        this.rgstBackForm.batchId = "";
