      ...
      <h-col span="24">
        ...
        <div v-if="mainVue">
          ...
          <h-datagrid :columns="columns"
                      highlightRow
                      url="/ce/impawn/paper/paperImpawn/pImpawnRgstApplyMain/func_queryImpawnBatch"
                      :bindForm="formItem"
            ...
            <div slot="toolbar" class="pull-left">
              <h-button type="primary" @click="cancelAccountMain()" >提交申请</h-button>
            </div>
          </h-datagrid>
        </div>
        <!-- 子页面 -->
        ...
        <div v-if="billManagerVue">
          <h-datagrid :columns="billManagerColumns"
                      url="/ce/impawn/paper/paperImpawn/pImpawnRgstApplyMain/func_pageQueryImpawnBillList"
                      highlightRow
                      :auto-load="false"
            ...
            <div slot="toolbar" class="pull-left">
              <h-dropdown placement="bottom-start" @on-click="impawnRgstDetail">
...
<script>
  ...
  import ImpawnBatchDetail from "@/views/bizViews/ce/impawn/paper/paperImpawn/pImpawnRgstApplyBatchDetail";

  export default {
    name: "pImpawnRgstApplyMain",
    components: {
      ImpawnBatchDetail,
      ...
      post({ paramKey: "ce.impawn.rgst_audit_mode" }, "/sm/auth/businessParamete/getBusinessParameter").then(res => {
        if (res) {
          ...
          if (retCode === "000000") {
            ...
            if(auditMode == '0'){
              this.formItem.impawnRgstStatusList = ['BTC01','BTC05','BTC06','BTC09'];
            }
          }
        }
        this.handleSearch();
    ...
    watch: {
      billManagerVue(val) {
        if (val) {
          ...
          this.dataUrl = "/ce/impawn/paper/paperImpawn/pImpawnRgstApplyMain/func_pageQueryImpawnBillList";
          this.$nextTick(() => {
            let columnsTmp = [];
    ...
    methods: {
      ...
      //撤销记账
      impawnRgstDetail(str) {
        if(str === "batch") {
          this.impawnRgstApply();
        }
        if(str === "select") {
      ...
      impawnRgstApply() {
        this.$hMsgBox.confirm({
          ...
          content: "确定批次提交复核吗?",
          onOk: () => {
            post({batchId: this.batchId, billOperType: "1", impawnRgstStatusList: this.formItem.impawnRgstStatusList, operSite : "1",
              ownedBrchNo:this.impawnBillParams.ownedBrchNo,licenseFlag:this.impawnBillParams.licenseFlag},
              "/ce/impawn/paper/paperImpawn/pImpawnRgstApplyMain/func_applImpawnCeRgst").then(res => {
              if(res) {
      ...
      selectCancelAccount() {
        ...
        this.$hMsgBox.confirm({
          ...
          content: "确定提交复核吗?",
          onOk: () => {
            post({ids: ids, billOperType: "0",batchId: this.batchId, impawnRgstStatusList: this.formItem.impawnRgstStatusList, ids: ids, operSite : "1",
              ownedBrchNo:this.impawnBillParams.ownedBrchNo,licenseFlag:this.impawnBillParams.licenseFlag},
              "/ce/impawn/paper/paperImpawn/pImpawnRgstApplyMain/func_applImpawnCeRgst").then(res => {
              if(res) {
                let msg = res.data.retMsg
