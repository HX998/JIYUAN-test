<template>
  <div>
    ...
    <h-datagrid :columns="billManagerColumns"
                url="/ce/impawn/elec/elecUnImpawn/eUnImpawnAccountMain/func_billlist"
                highlightRow
...
<script>
  ...
  export default {
    ...
    created(){
      ...
      this.billManagerFormItem.unImpawnIds=this.listIds;
      ...
      if (this.billClass ==="ME01") {
        this.acctRecordUrl = "/ce/impawn/paper/paperUnImpawn/pUnImpawnAccountMain/func_getUnImpawnAccount";  //纸票解质押记账授权
        this.acctUrl="/ce/impawn/paper/paperUnImpawn/pUnImpawnAccountMain/func_acctAuthorizePass";
        this.acctAuthBackUrl="/ce/impawn/paper/paperUnImpawn/pUnImpawnAccountMain/func_acctAuthorizeReturn";
      }else {
        this.acctRecordUrl = "/ce/impawn/elec/elecUnImpawn/eUnImpawnAccountMain/func_getUnImpawnAccount"; //电票解质押记账授权
        this.acctUrl="/ce/impawn/elec/elecUnImpawn/eUnImpawnAccountMain/func_acctAuthorizePass";
        this.acctAuthBackUrl="/ce/impawn/elec/elecUnImpawn/eUnImpawnAccountMain/func_acctAuthorizeReturn";
      }
      this.$nextTick(()=>{
    ...
    methods:{
      //记账
      account() {
        this.acctRecordParam = {batchId: this.batchId, operType: "0", impawnIds: this.listIds ,funcNo: "PICE031905"}
        this.showAcctRecordWin = true;
      },
      //确定记账
      acctSubmitSure(info) {
        let param = {};
        param = {acctAuthId:this.id,batchId: this.batchId, operType: "0", impawnIds: this.listIds, funcNo: "PICE031905"};
      ...
      getImpawnBatchInfo() {
        ...
        post(params, "/ce/impawn/elec/elecUnImpawn/eUnImpawnAccountMain/func_getUnImpawnBatch").then(res => {
          if (res) {
            let retCode = res.data.retCode;
            if (retCode === "000000") {
