      ...
      <h-col span="24">
        <div>
          ...
          <h-datagrid
            ...
            url="/bm/audit/prodAuditMain/func_pageQueryCeUnImpawnBillList"
            highlightRow
...
<script>
  ...
  export default {
      ...
      return {
        ...
        billManagerColumns: [
          ...
          },
          {
            ...
            hiddenCol: false,
            render:(h,params) => {
              if(params.row.billOrigin === "CS01"|| params.row.billOrigin === 'CS00'){
                return h("span", "-");
              }else{
                if (params.row.billRangeStart.toString() === '0' || params.row.billRangeEnd.toString() === '0') {
                  return h("span", "0");
                } else {
                  let billRange = params.row.billRangeStart.toString().padLeft(12, '0') + "," + params.row.billRangeEnd.toString().padLeft(12, '0');
                  return h("span", billRange);
                }
              }
            }
          },
    ...
    created() {
      ...
      this.impawnBillParams.unImpawnIds = this.listIds;
      this.impawnBillParams.accountType = "unImpawn";
    ...
    mounted() {
      this.$nextTick(() => {
        this.$refs.impawnBatchDetail.getImpawnBatchInfo("unImpawn", "audit", "/bm/audit/prodAuditMain/func_queryCeUnImpawnBatch");
      });
    },
    watch: {
      isShow(val) {
