      ...
      <h-col span="24">
        <div>
          ...
          <h-datagrid
            :columns="columns"
            highlightRow
            url="/ce/impawn/elec/elecUnImpawn/eUnImpawnApplyMain/func_list"
            :bindForm="formItem"
        ...
        <!-- 新增修改弹窗 -->
        <e-un-impawn-apply-add :impawnAddOrEditWin="impawnAddOrEditWin" :batchParams="batchParams" :licenseFlag="this.licenseFlag" :buttonAuth="btnAuth"
                            @impawnAddOrEditWinClose="impawnAddOrEditWinClose"></e-un-impawn-apply-add>
      </h-col>
    </h-row>
...
<script>
  ...
  export default {
    name: "eUnImpawnApplyMain",
    components: {
      EUnImpawnApplyAdd:()=>import(/* webpackChunkName: "ce/impawn/elec/elecUnImpawn/eUnImpawnApplyAdd" */`@/views/bizViews/ce/impawn/elec/elecUnImpawn/eUnImpawnApplyAdd`),
    ...
    methods: {
      ...
      noCommitButton() {
        ...
        this.formItem.funcNo = "PICE031206";
        this.formItem.submitFlag = "";
        this.handleSearch();
      },
      //已提交
      hasCommitButton() {
        this.formItem.isCommit = "hasCommit";
        this.formItem.funcNo = "PICE031107";
        this.formItem.submitFlag = "1";
        this.handleSearch();
      },
      //表单查询
      handleSearch(pageNo) {
      ...
      handleDel() {
        ...
        post({ id: id ,deleteFlag:"0" }, "/ce/impawn/elec/elecUnImpawn/eUnImpawnApplyMain/func_deleteBatch").then(res => {
          if (res) {
            let retCode = res.data.retCode;
            let retMsg = res.data.retMsg;
            if (retCode === "000000") {
        ...
        continueApply(id){
            let url=  "/ce/impawn/elec/elecUnImpawn/eUnImpawnApplyMain/func_deleteBatch";
      ...
      handleCancel() {
        let id = this.currentSelectRow.id;
        post({ id: id ,ownedBrchNo : this.currentSelectRow.ownedBrchNo,
          licenseFlag : this.licenseFlag,  }, "/ce/impawn/elec/elecUnImpawn/eUnImpawnApplyMain/func_cancelApply").then(res => {
          if (res) {
            let retCode = res.data.retCode;
            if (retCode === "000000") {
              this.$msgTip.success(this);
