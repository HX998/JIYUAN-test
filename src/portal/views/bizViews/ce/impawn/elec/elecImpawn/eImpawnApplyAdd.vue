...
<template>
  <div>
    <h-msg-box v-model="tempImpawnAddOrEditWin" :maximize="true" :mask-closable="false" @on-close="handleClose" width="1000" class="h-form-search-layer" top="10" @on-maximize="onMaximize">
      <p slot="header">
      ...
      <div>
        <h-form :model="addOrEditForm" :label-width="115" ref="addOrEditForm" cols="3" class="h-form-search">
          <div>
            <show-branch v-model="addOrEditForm.ownedBrchNo" :label="$t('m.i.common.ownedBrchNo')" prop="ownedBrchNo" title="机构名称"
                         :filterable="batchParams.batchType==='add'" required :showCheckBox="false" v-show="this.licenseFlag" :msgBoxWin="tempImpawnAddOrEditWin"
            ...
            <h-form-item prop="custAcctNo" :label="$t('m.i.common.custAcctNo')" required>
              <h-input v-model="addOrEditForm.custAcctNo" placeholder="" readonly :icon="batchParams.batchType==='add' ? 'android-search' : ''" @on-click="queryCustAcctNo()"></h-input>
            </h-form-item>
            ...
            <h-form-item prop="billType" :label="$t('m.i.billInfo.billType')" required>
              <h-select v-model="addOrEditForm.billType" placeholder="" :readonly="batchParams.batchType==='modify'||this.hadBatchSave">
            ...
            <h-form-item :label="$t('m.i.billInfo.billOrigin')"  required key="billOriginArray"  prop="billOriginArray">
              <h-select v-model="addOrEditForm.billOriginArray" :multiple="batchParams.isMixPack1" :readonly="batchParams.batchType==='modify'">
                <h-option v-for="item in batchParams.billOriginList" :value="item.key" :key="item.key">{{ item.value }}</h-option>
              </h-select>
            </h-form-item>
          </div>
        </h-form>
        ...
        <h-datagrid
          ...
          <div slot="toolbar" class="pull-left">
            <h-button type="primary" v-if="this.batchParams.isCommit === 'noCommit'" @click="impawnPickBill">{{$t("m.i.ce.pickBill")}}</h-button>
            ...
            <h-button type="primary" v-if="this.batchParams.isCommit === 'hasCommit'" @click="impawnBillCancel">{{$t("m.i.common.cancel")}}</h-button>
          </div>
        </h-datagrid>
      </div>
      <div slot="footer">
    ...
    <!-- 查询客户账号信息弹窗 -->
    <show-cust-acct-no :show-cust-acct-no-win="showCustAcctNoWin" :cust-no="this.addOrEditForm.custNo" :isAllBankAcct=true @showCustAcctNoWinClose="showCustAcctNoWinClose" @custAcctNoSelectSubmit="custAcctNoSelectSubmit"></show-cust-acct-no>
    ...
    methods: {
      ...
      submit(type) {
        ...
        if (type === "select") {
          ...
          let params = { batchId: this.batchId, impawnIds: impawnIds.join(","),
