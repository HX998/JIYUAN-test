...
<script>
  ...
  export default {
    ...
    mounted() {
      getDictListByGroups("DraftTypeCode,PayType,Yon").then(res => {
    ...
    methods: {
      ...
      acctSubmitSure(info) {
        ...
        post(param, "/ce/disc/paper/account/discAccountBatchMain/func_authorizeAcctPass").then(res => {
      ...
      cancelAccount() {
        this.$hMsgBox.confirm({
          ...
          content: "确定撤销记账吗?",
          onOk: () => {
            let param = {id:this.id,batchId: this.batchId,ids: this.listIds, operType: "0",acctType:"2"};
            post(param, "/ce/disc/paper/account/discAccountBatchMain/func_cancelAuthorizeAcctPass").then(res => {
              if(res) {
                let msg = res.data.retMsg;
                let retCode = res.data.retCode;
                if (retCode === "000000") {
      ...
      acctAuthorizeBack(){
        this.$hMsgBox.confirm({
          ...
          onOk: () => {
            ...
            post(param, "/ce/disc/paper/account/discAccountBatchMain/func_authorizeAcctBack").then(res => {
              if(res) {
                let msg = res.data.retMsg;
                let retCode = res.data.retCode;
                if (retCode === "000000") {
