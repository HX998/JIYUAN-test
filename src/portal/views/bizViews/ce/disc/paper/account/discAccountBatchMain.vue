  ...
  <div class="layout">
    <h-row name="flex" class="layout-menu-wrapper">
      <h-col span="24">
          ...
          <div>
            ...
            <div class="h-form-search-box">
              ...
              <h-datagrid
                ...
                highlight-row
                :bindForm="formItem"
                url="/ce/disc/paper/account/discAccountBatchMain/func_pageQueryDiscBatchList"
        ...
        <disc-batch-info-detail :is-show="discAccountBillMain" :batchId="this.discBatchId"
                                :queryBatchUrl="'/ce/disc/paper/apply/discApplyBatchMain/func_queryDiscBatch'"
                                :isQuery="isQuery" funcNo="PICE021302" ref="discBatchInfoDetail"></disc-batch-info-detail>
        <!-- 数据展示表格 -->
        <div v-if="discAccountBillMain" key="3">
          <h-datagrid
            ...
            url="/ce/disc/paper/account/discAccountBatchMain/func_pageQueryCeDiscBillList"
            :row-select="true"
            :has-select="hasSelect"
            ref="discAccountBillDatagrid">
            <div slot="toolbar">
              <account-btn @accountList="accountList"></account-btn>
              <account-back-btn @accountBackList="accountBackList"
                                v-if="this.childrenBtnAuth.accountBack === undefined ? true : this.childrenBtnAuth.accountBack.isShow"></account-back-btn>
...
<script>
  ...
  export default {
    ...
    mounted() {
      getDictListByGroups("DraftTypeCode,PayType,CorpScale,Yon,AssetIndustry").then(res => {
    ...
    methods: {
      ...
      //获取分录信息
      getEntryInfo(str) {
        this.acctRecordUrl = "/ce/disc/paper/account/discAccountBatchMain/func_getAcoreDiscAccount";
        if(str === "batch") {
      ...
      acctSubmitSure(info) {
        ...
        this.acctSubmitFlag = true;
        post(param, "/ce/disc/paper/account/discCancelAccountBatchMain/func_submitDiscAccount").then(res => {
          this.acctSubmitFlag = false;
          if(res) {
            let msg = res.data.retMsg;
            let retCode = res.data.retCode;
      ...
      accountSelectBatchBack(){
        ...
        let ids = this.$refs.discAccountBillDatagrid.selectIds.join(",");
        this.$hMsgBox.confirm({
          title: this.$t("m.i.common.confirm"),
          content: "确定选择记账退回吗?",
          onOk: () => {
            post({batchId:this.discBatchId , discIds:ids ,operType:"0"}, "/ce/disc/paper/account/discAccountBatchMain/func_discAccountBack").then(res => {
              if(res) {
                let msg = res.data.retMsg;
                let retCode = res.data.retCode;
                if (retCode === "000000") {
                  this.$msgTip.success(this);
                  if(this.$refs.discAccountBillDatagrid.total === this.$refs.discAccountBillDatagrid.selects.length ) {
                    this.returnBack();
                  }else{
                    this.$refs.discBatchInfoDetail.getBatchInfo();
                    this.$refs.discAccountBillDatagrid.dataChange(this.$refs.discAccountBillDatagrid.pageInfo.pageNo);
                    this.currentBillSelectList = [];
                    this.$refs.discAccountBillDatagrid.selects = [];
                    this.$refs.discAccountBillDatagrid.selectIds = [];
                  }
      ...
      handleAccountBatchBack() {
        post({ batchId:this.discBatchId ,operType:"1"}, "/ce/disc/paper/account/discAccountBatchMain/func_discAccountBack").then(res => {
          if (res) {
            let msg = res.data.retMsg;
            let retCode = res.data.retCode;
            if (retCode === "000000") {
