<template>
  ...
  <div class="layout">
    <h-row name="flex" class="layout-menu-wrapper">
      <h-col span="24">
        <div v-if="discPrintBatchMain" key="1">
          <div>
            <!--查询表单-->
            <div class="h-form-search-box">
              ...
              <h-datagrid
                ...
                highlight-row
                :bindForm="formItem"
                url="/ce/disc/paper/print/discPrintMain/func_pageQueryDiscBatchList"
        ...
        <div v-if="discPrintBillMain" key="2">
          <!--查询表单-->
          <div class="h-form-search-box">
            <h-panel class="clearfix">
              <h-form :model="billFormItem" :label-width="90" ref="billFormItem" cols="4" class="h-form-search">
                ...
                <query-btn :advanceShow='false' :formSearch='billManagerHandleSearch' :formSearchReset='billManagerResetSearch' formClassAdd="1"></query-btn>

              </h-form>
            </h-panel>
          </div>
        </div>
        <disc-batch-info-detail :is-show="discPrintBillMain" :batchId="this.discBatchId"
                                :queryBatchUrl="'/ce/disc/paper/apply/discApplyBatchMain/func_queryDiscBatch'"
                                :isQuery="isQuery" funcNo="" ></disc-batch-info-detail>
        <!-- 数据展示表格 -->
        <div v-if="discPrintBillMain" key="3">
          <h-datagrid
            :columns="billColumns"
            highlight-row
            :auto-load="false"
            :bindForm="billFormItem"
            url="/ce/disc/paper/print/discPrintMain/func_pageQueryCeDiscBillList"
            :row-select="true"
            :has-select="hasSelect"
            ref="discPrintBillDatagrid">
            <div slot="toolbar">
  ...
  export default {
    ...
    data() {
      ...
      return {
        ...
        dictMap : new Map(),
        prodMap : new Map(),
        discPrintBatchMain : true,
        discPrintBillMain : false,
    ...
    mounted() {
      getDictListByGroups("QueryType,ArbitrateMode,ProtectType,RateType,PayType,DraftTypeCode,CorpScale,Yon,DiscRgstStatus,discPrintMain,AssetIndustry").then(res => {
    ...
    methods: {
      ...
      billManagerHandleSearch(){
        this.$nextTick(() => {
          ...
          this.$refs.discPrintBillDatagrid.selects = [];
          this.$refs.discPrintBillDatagrid.selectIds = [];
          this.$refs.discPrintBillDatagrid.dataChange(1);
        });
      },
      //票据管理界面重置
      ...
      handleForm() {
        ...
        this.discPrintBatchMain = false;
        this.discBatchId = this.currentSelectList.id;
        this.billFormItem.batchId = this.currentSelectList.id;
        this.isQuery = true;
        this.discPrintBillMain = true;
        this.batchInfo.custName = this.currentSelectList.custName;
        this.batchInfo.custAcctNo = this.currentSelectList.custAcctNo;
        this.billManagerHandleSearch();
        this.$nextTick(() => {
          this.billColumns = this.deepClone(this.billTemp);
          this.$refs.discPrintBillDatagrid.selfAdaption();
          this.$refs.discPrintBillDatagrid.createShowCol(this.billTemp);
        });
      },
      //凭证打印
      voucherPrinting(printType){
          ...
          if(printType == "select"){
              //选择打印
              if(this.$refs.discPrintBillDatagrid.selects.length === 0) {
                  this.$msgTip.info(this, { info: this.$t("m.i.common.chooseOneData") });
                  return;
              }
              vocherData = this.deepClone(this.$refs.discPrintBillDatagrid.selects);
              this.queryTempCOnf(vocherData);
          }else{
              //批次打印
              let url="/ce/disc/paper/print/discPrintMain/batchPrint";
              let param = {batchId : this.discBatchId};
        ...
        assembleDiscParams(templateInfo,vocherData) {
          for (let i = 0; i < vocherData.length; i++) {
            ...
            vocherData[i].custName = this.batchInfo.custName;
            vocherData[i].custAcctNo = this.batchInfo.custAcctNo;
            vocherData[i].openBankNo = "";
            ...
            this.currentSelectList = [];
            this.$refs.discPrintBillDatagrid.selects = [];
            this.$refs.discPrintBillDatagrid.dataChange(this.$refs.discPrintBillDatagrid.pageInfo.pageNo);
          }
      },
      //返回
      returnBack() {
        ...
        this.$refs.discPrintBillDatagrid.selectIds = [];
        this.discBatchId = "";
        this.batchInfo.custName = "";
        this.batchInfo.custAcctNo = "";
        this.resetSearch();
        this.isQuery = false;
        this.discPrintBillMain = false;
        this.discPrintBatchMain = true;
        this.$nextTick(() => {
      ...
      //批次编号超链接
      batchNoMain(info) {
        this.discBatchId = info.id;
        this.billFormItem.batchId = this.discBatchId;
        this.isQuery = true;
        this.discPrintBatchMain = false;
        this.discPrintBillMain = true;
        this.batchInfo.custName = info.custName;
        this.batchInfo.custAcctNo = info.custAcctNo;
        this.$nextTick(() => {
          this.billColumns = this.deepClone(this.billTemp);
          this.$refs.discPrintBillDatagrid.selfAdaption();
          this.$refs.discPrintBillDatagrid.createShowCol(this.billTemp);
