<template>
  <div class="layout">
      ...
      <h-col span="24">
        <div>
          ...
          <h-datagrid :columns="columns"
                      url="/ce/disc/paper/register/discImgBatchRegisterApplyMain/func_pagingQueryAcptImgPackage"
                      highlightRow
                      :bindForm="formItem"
    ...
    <h-msg-box :title="$t('m.i.ce.batchUploadImg')" v-model="imgBatchUploadWin" :mask-closable="false"
               class="h-form-search-layer" height="350" width="650"
      ...
      <h-card style="margin-top:20px">
        <p style="padding-bottom: 10px">请将需要上传的所有贴现影像放入一个文件夹，然后将文件夹压缩为“.zip”或“.ZIP”格式再选择导入；</p>
        <p style="padding-bottom: 10px">影像压缩包请勿大于50MB；</p>
        <p style="padding-bottom: 10px">影像图片请符合“.jpg”或“.JPG”格式，单张影像图片请勿大于512KB；</p>
        <p style="padding-bottom: 10px">贴现正面影像：同一票据只能上传一张，命名规则：“票号_IM03.jpg”；</p>
        <p style="padding-bottom: 10px">贴现背面影像：同一票据可上传多张，命名规则：“票号_IM04_1.jpg”，“票号_IM04_2.jpg”...；</p>
        <p style="padding-bottom: 10px">贴现凭证影像：同一票据可上传多张，命名规则：“票号_IM05_1.jpg”，“票号_IM05_2.jpg”...；</p>
        <p style="padding-bottom: 10px">瑕疵证明影像：同一票据可上传多张，命名规则：“票号_IM06_1.jpg”，“票号_IM06_2.jpg”...。</p>
      </h-card>
    </h-msg-box>
  </div>
...
export default {
  name: "discImgBatchRegisterApplyMain",
  data() {
    return {
      //showBranchWin: false,
      checkFailInfoWin: false,
      isDisabled: false,
      fileComitUrl: window.LOCAL_CONFIG.API_HOME + "/ce/disc/paper/register/discImgBatchRegisterApplyMain/func_uploadAcptImgPackage",
      failInfoUrl: "/ce/disc/paper/register/discImgBatchRegisterApplyMain/func_pagingQueryAcptImgErrorInfo",
      fileSelect: true,
      btnShow: false,
  ...
  methods: {
    ...
    batchMatchImgRgst() {
      ...
      this.$hMsgBox.confirm({
        ...
        content: "确定匹配登记吗?",
        onOk: () => {
          let url = "/ce/disc/paper/register/discImgBatchRegisterApplyMain/func_matchAcptImgPackage";
    ...
    deleteImgRgstInfo() {
      ...
      this.$hMsgBox.confirm({
        ...
        onOk: () => {
          let url = "/ce/disc/paper/register/discImgBatchRegisterApplyMain/func_deleteAcptImgPackage";
          post({id: this.currentSelectRow.id}, url).then(res => {
            if (res) {
              let retCode = res.data.retCode;
              if (retCode === "000000") {
