...
<template>
  <div class="layout">
      ...
      <h-col span="24">
        ...
        <div>
          ...
          <h-datagrid :columns="columns"
                      highlightRow
                      url="/ce/disc/corp/discexpire/discDeductSearch/func_queryDiscBillExpireBatch"
                      :bindForm="formItem"
            ...
            <div slot="toolbar" class="pull-left">
              ...
              <h-button type="primary" @click="syncDiscDeduct()">{{$t("m.i.ce.syncDeductRst")}}</h-button>
            </div>
          </h-datagrid>
        </div>
      </h-col>
    ...
    <!-- 同步扣款结果 -->
    <h-msg-box v-model="syncWin" width="800" class="h-form-search-layer" :mask-closable="false" :maximize="true"
               @on-close="handleWinClose">
      <p slot="header">
        <span>同步扣款结果</span>
      </p>
      <h-panel>
        <h-form :model="syncForm" :label-width="115" ref="syncForm" cols="2" class="h-form-search">
          <h-form-item prop="deductDt" :label="$t('m.i.common.applDt')" required>
            <h-date-picker type="date" v-model="syncForm.deductDt" placeholder="" showFormat
  ...
  export default {
    ...
    data() {
      ...
      return {
        ...
        discDeductParams : {},
        syncForm : {
          deductDt: this.$moment(window.sessionStorage.getItem("workDate")).format("YYYYMMDD")
        },
        submitFlag : false,
        syncWin : false,
      };
      ...
      submitForm() {
        this.$refs["syncForm"].validate(valid => {
          if (valid) {
            ...
            let deductDt = this.syncForm.deductDt.replace(/-/g, "");
            post({ code: deductDt }, "/ce/disc/corp/discexpire/discDeductSearch/deductBatchFileQuery").then(res => {
