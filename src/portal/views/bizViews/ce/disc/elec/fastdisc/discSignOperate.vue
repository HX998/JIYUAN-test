          ...
          <div>
            ...
            <div v-if="signParams.optType==='edit' || (isRefuse!=null && isRefuse) || signParams.optType==='check'">
              <div v-if="!(discSignOperateForm.operTypeName === '新签约受理' && discSignOperateForm.operResult === '0')">
                ...
                <channel-select v-model="discSignOperateForm.channelNo" :label="$t('m.i.ce.signChannel')" prop="channelNo" required></channel-select>
                ...
                <h-form-item :label="$t('m.i.ce.adjustRate')" prop="adjustRate" class="h-form-height-auto"
                             :required="signParams.optType==='edit' || (isRefuse!=null && isRefuse)"
                             :validRules="adjustRateRule" cols="2" v-if="discSignOperateForm.operTypeName === '修改签约信息'">
                  <h-row>
                    <h-col span="11" style="padding-right: 28px;">
                      <h-typefield v-model="discSignOperateForm.adjustRate"  placeholder="" :readonly="(isBargainRefuse!=null && !isBargainRefuse) || signParams.optType==='check'"
                                   :integerNum="3" :suffixNum="4"></h-typefield>
                    </h-col>
                    <h-col span="12"><span style="color: red;" v-if="beforeDiscSignForm.adjustRate!==''">(修改前：{{beforeDiscSignForm.adjustRate}})</span>
                    </h-col>
                  </h-row>
                </h-form-item>
                <h-form-item :label="$t('m.i.ce.adjustRate')" prop="adjustRate"
                             :required="signParams.optType==='edit' || (isRefuse!=null && isRefuse)"
                             :validRules="adjustRateRule" v-else>
                  <h-typefield v-model="discSignOperateForm.adjustRate"  placeholder="" :integerNum="3" :suffixNum="4"></h-typefield>
                </h-form-item>
                ...
                <h-form-item :label="$t('m.i.ce.discInBrchName')" prop="discInBrchName" class="h-form-height-auto"
                  ...
                  <h-row>
                    <h-col span="11" style="padding-right: 28px;">
                      <h-input v-model="discSignOperateForm.discInBrchName" placeholder="" readonly :maxlength="125"
                               :icon="signParams.optType==='check' ? '' : 'android-search'"
                               @on-click="queryDiscInBrchNo('add')"></h-input>
                    </h-col>
                    <h-col span="12"><span
                ...
                <h-form-item :label="$t('m.i.ce.custArea')" prop="custArea" class="h-form-height-auto"
                             :required="signParams.optType==='edit' || (isRefuse!=null && isRefuse)" cols="2"
                             v-if="discSignOperateForm.operTypeName === '修改签约信息'">
                  <h-row>
                    <h-col span="11" style="padding-right: 28px;">
                      <h-select v-model="discSignOperateForm.custArea" placeholder=""
                                :readonly="signParams.optType==='check'">
                        <h-option v-for="item in signParams.custAreaList" :value="item.key" :key="item.key">
                          {{item.value}}
                        </h-option>
                      </h-select>
                    </h-col>
                    <h-col span="12"><span style="color: red;" v-if="beforeDiscSignForm.custArea!==''">(修改前：{{beforeDiscSignForm.custArea}})</span>
                    </h-col>
                  </h-row>
                </h-form-item>
                <h-form-item :label="$t('m.i.ce.custArea')" prop="custArea" v-else
                             :required="signParams.optType==='edit' || (isRefuse!=null && isRefuse)">
                  <h-select v-model="discSignOperateForm.custArea" placeholder=""
                            :readonly="signParams.optType==='check'">
                    <h-option v-for="item in signParams.custAreaList" :value="item.key" :key="item.key">{{item.value}}
                    </h-option>
                  </h-select>
                </h-form-item>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      synchAmt(){
        if (this.discSignOperateForm.signMode === '1' && (this.discSignOperateForm.custNo === ""
          || this.discSignOperateForm.custNo === null) ) {
          this.$msgTip.info(this, { info: "客户号不能为空" });
          return;
        } else if ( this.discSignOperateForm.signMode !== '1' && (this.discSignOperateForm.custNo === ""
          || this.discSignOperateForm.custAcctNo === "" || this.discSignOperateForm.custNo === null
          || this.discSignOperateForm.custAcctNo === null)) {
