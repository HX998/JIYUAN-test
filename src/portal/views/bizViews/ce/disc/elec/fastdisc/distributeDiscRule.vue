<template>
  <!--分配极速贴现规则弹出框-->
  <h-msg-box v-model="tempDistributeDiscRuleWin" width="1000" :mask-closable="false" @on-close="handleClose" top="10"
...
<script>
  ...
  export default {
    ...
    computed: {
      tempDistributeDiscRuleWin: {
        get() {
          return this.distributeDiscRuleWin;
        },
        set() {
        }
      },
    ...
    methods: {
      ...
      handleClose() {
        ...
        this.$emit("distributeDiscRuleWinClose", "");
      },
      submitForm() {
          ...
          this.$hMsgBox.confirm({
            ...
            onOk: () => {
              ...
              for (let i = 0; i < list.length; i++) {
                legalNos.push(list[i].legalNo);
              }
              let params = { ids: this.discRuleIds, legalNos: legalNos.join(",") };
              let url = "/ce/disc/elec/fastdisc/discRule/func_allocationRule";
              post(params, url).then(res => {
                ...
                if (res) {
                  ...
                  if (retCode === "000000") {
                    ...
                    this.$emit("distributeDiscRuleSubmit", null);
                  } else {
