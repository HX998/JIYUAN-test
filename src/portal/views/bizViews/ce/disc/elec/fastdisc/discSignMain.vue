        ...
        <div>
          ...
          <h-datagrid
            ...
            <div slot="toolbar" class="pull-left">
              <h-button type="primary" @click="handleAddForm()" v-if="isShowAddBtn">新增
              </h-button>
              <h-button type="primary" @click="signOperate('accept')" :disabled="isAcceptAllow">受理
              </h-button>
  ...
  export default {
    ...
    mounted() {
      ...
      this.getDictListByGroups("FDiscSignStatus,Yon,IndustryCategoryDetail,CustArea").then(res => {
        ...
        this.signStatusList = this.signStatusList.filter(item => item.key === "FD01" || item.key === "FD05" || item.key === "FD07" || item.key === "FD15" || (item.key === "FD20" && this.isAudit));
    ...
    methods: {
      ...
      handleCurrentChange(currentRow) {
        ...
        //签约待受理FD01：受理可用；签约已生效FD07：修改、暂停、终止可用；签约已暂停FD13：恢复、终止可用;签约审批通过待生效FD05,修改可用
