<template>
  <div class="h-tree-search">
    <div class="h-modal-header-wrapper">
      <h-msg-box v-model="tempShowEDiscInvoiceWin" :maximize="true" :mask-closable="false"
        ...
        <div>
          ...
          <h-form :model="addForm" :label-width="115" ref="addForm" cols="3" class="h-form-search">
            <div>
              ...
              <h-form-item :label="$t('m.i.ce.recvAcctNo')" prop="aoAcctNo" required >
                <h-input v-model="addForm['aoAcctNo']"  disabled></h-input>
              </h-form-item>
              <h-form-item :label="$t('m.i.ce.protectType')" prop="protectType" :required="isRequired" v-if="addForm.prodNo==='01060622' || addForm.prodNo==='01061022' || addForm.prodNo==='01060322' || addForm.prodNo==='01060722'">
              ...
              <h-form-item :label="$t('m.i.common.custAcctNo')" prop="custAcctNo" required>
                <h-input v-model="addForm['custAcctNo']" disabled :icon="addForm['prodNo'].indexOf('010611') === 0 ? 'android-search' : ''" ></h-input>
              </h-form-item>
              <h-form-item :label="$t('m.i.ce.guarntrNo')" prop="bailAcctNo" :required="true" v-if="addForm.prodNo === '01060322'">
                <h-input v-model="addForm.bailAcctNo" :placeholder="''" disabled icon="android-search" ></h-input>
              </h-form-item>
              <h-form-item :label="$t('m.i.ce.redeemOpenDt')" prop="redeemOpenDt" :required="isRequired" v-if="formItem.prodNo==='01060221' || formItem.prodNo==='01060222'">
                <h-date-picker v-model="addForm.redeemOpenDt" format="yyyy-MM-dd" placeholder="" showFormat :editable=false disabled></h-date-picker>
              </h-form-item>
              <h-form-item :label="$t('m.i.ce.redeemDueDt')" prop="redeemDueDt" :required="isRequired" v-if="formItem.prodNo==='01060221' || formItem.prodNo==='01060222'">
                <h-date-picker v-model="addForm.redeemDueDt" format="yyyy-MM-dd" placeholder="" showFormat :editable=false disabled></h-date-picker>
              </h-form-item>
              <h-form-item :label="$t('m.i.ce.clearMark')" prop="clearMark" :required="isRequired">
                <h-select v-model="addForm.clearMark" disabled placeholder="">
                  <h-option v-for="item in clearMarkList" :value="item.key" :key="item.key">{{ item.value }}</h-option>
                </h-select>
              </h-form-item>
              <h-form-item :label="$t('m.i.billInfo.banEndrsmtMark')" prop="banEndrsmtMark"  v-show="false">
                <h-input v-model="addForm.banEndrsmtMark" disabled></h-input>
              </h-form-item>
          ...
          <div>
            <h-datagrid
              ...
              getDataFunc
              :auto-load="false"
              url="/ce/disc/elec/invoice/eInvoiceReplenishMain/func_pageQueryCeDiscBillList"
              :bindForm="formItem"
...
<script>
  import { accDiv, accMul, cmycurd, formatNumber, getDictListByGroups, numToUpper, off, on, post , getDictValueFromMap, getBusinessParameter,formatBillRange } from "@/api/bizApi/commonUtil";
  ...
  export default {
    ...
    mounted() {
      getDictListByGroups("ProtectType,RateType,PayType,DraftTypeCode,CorpScale,Yon,SettlementMarkCode,SettleType,QueryType,ArbitrateMode,Industry,BanEndorsementMarkCode,AssetIndustry,BillOrigin").then(res => {
    ...
    computed: {
      tempShowEDiscInvoiceWin: {
        get() {
          return this.showEDiscInvoiceWin;
        },
        set() {
        }
      }
    ...
    methods: {
      ...
      queryObjById() {
        ...
        post({ id: id}, "/ce/disc/elec/apply/eDiscApplyBatchMain/func_queryDiscBatch").then(res => {
          if (res) {
      ...
      handleClose() {
        ...
        this.$emit("eDiscInvoiceWinClose", "");
      },
      ...
      formSave() {
        this.$refs["addForm"].validate (valid => {
          if(valid) {
            ...
            let url = "/ce/disc/elec/invoice/eInvoiceReplenishMain/func_invoiceReplenishFinish";
      ...
      billPickInvoiceChange(listId){
        if (this.billPickInvoiceParams.invoiceFlag === "1") {
          ...
          post({id:listId }, "/ce/disc/elec/invoice/eInvoiceReplenishMain/func_relatedInvoice").then(res => {
            if (res) {
              let retCode = res.data.retCode;
              if (retCode === "000000") {
                this.billPickInvoiceWinClose();
      ...
      invoiceManager() {
        ...
        if (invoiceFlag === "1") {
          ...
          this.relationInvoiceBillParams.cancelRelationUrl = "/ce/disc/elec/invoice/eInvoiceReplenishMain/func_cancelRelatedInvoice";
