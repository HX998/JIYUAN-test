      ...
      <h-col span="24">
        <div>
          ...
          <h-datagrid :columns="columns"
                      url="/ce/cpesrgst/umImpawnRgst/unImpawnRgstRecheck/func_pagingQueryPaperRgstCheck"
                      :bindForm="formItem"
                      :has-select="false"
                      :row-select="true"
                      ref="datagrid">
  ...
  export default {
    ...
    methods: {
      ...
      handleReCheckPass() {
        ...
        post({ ids: ids, isPass: "1" }, "/ce/cpesrgst/umImpawnRgst/unImpawnRgstRecheck/func_submitCheck").then(res => {
          if (res) {
      ...
      handleReCheckBack() {
        ...
        post({ ids: ids, isPass: "0" }, "ce/cpesrgst/umImpawnRgst/unImpawnRgstRecheck/func_submitCheck").then(res => {
          if (res) {
            let retCode = res.data.retCode;
            if (retCode === "000000") {
              this.$msgTip.success(this);
      ...
      //详细信息超链接展示
      showInfoDetail(str) {
        post({ id: str }, "ce/cpesrgst/umImpawnRgst/unImpawnRgstRecheck/func_queryPaperRgstById").then(res => {
          if (res) {
