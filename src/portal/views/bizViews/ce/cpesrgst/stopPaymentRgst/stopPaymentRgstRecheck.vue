      ...
      <h-col span="24">
        <div>
          ...
          <h-datagrid :columns="columns"
                      url="/ce/cpesrgst/stopPaymentRgst/stopPaymentRgstRecheck/func_pagingQueryPaperRgstCheck"
                      :bindForm="formItem"
                      :row-select="true"
                      :has-select="false"
                      ref="datagrid">
    ...
    <h-msg-box v-model="showInfoDetailWin" width="800" @on-close="showInfoDetailWinClose" class="h-form-search-layer"
      ...
      <div>
        <h-form :model="showInfoDetailForm" :label-width="115" ref="showInfoDetailForm" cols="2" class="h-form-search">
          ...
          <h-form-item prop="transFromSocCode" :label="$t('m.i.ce.sspdSocCode')" class="h-form-long-label">
...
<script>
  ...
  export default {
    name: "stopPaymentRgstRecheck",
    components: {},
    data() {
      return {
        //影像上传
        showUploadImageWin: false,
        propsVue: "stopPaymentRgstRecheck",
        id: "",
        authObject: {
          imageAdd: false,       //影像新增
        ...
        imageParams: {
          ...
          imgTypeList: [],
          imgType: "IM09",
          imageUrl: "/ce/cpesrgst/stopPaymentRgst/stopPaymentRgstRecheck/func_pagingQueryPaperRgstImg", //查询可查看影像url
          imgShowUrl: "/ce/cpesrgst/stopPaymentRgst/stopPaymentRgstRecheck/func_showImg?id=" //查看影像url
        },
        ifShowMore: false,
    ...
    mounted() {
      ...
      getDictListByGroups("Yon,ImgStatus,DraftTypeCode,AcptRgstStatus,SspdType").then(res => {
    ...
    methods: {
      ...
      handleReCheckPass() {
        ...
        post({
          ids: ids,
          isPass: "1"
        }, "/ce/cpesrgst/stopPaymentRgst/stopPaymentRgstRecheck/func_submitCheck").then(res => {
          if (res) {
            let retCode = res.data.retCode;
            if (retCode === "000000") {
