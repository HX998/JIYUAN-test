<template>
  <div class="layout">
    ...
    <h-msg-box v-model="addOrEditWin" width="800" @on-close="addOrEditWinClose" class="h-form-search-layer"
      ...
      <p slot="header">
        <span v-if="addOrEditType==='add'">新增质押登记信息</span>
        <span v-if="addOrEditType==='modify'">修改质押登记信息</span>
      </p>
      <div>
        <h-form :model="addOrEditForm" :label-width="115" ref="addOrEditForm" cols="2" class="h-form-search">
          ...
          <h-form-item prop="transFromAcctNo" :label="$t('m.i.ce.pledgorAcctNo')" required :validRules="custAcctNoRule">
          ...
          <h-form-item prop="transFromSocCode" :label="$t('m.i.ce.pledgorSocCode')" :validRules="sosCodeRule" required
          ...
          <h-form-item prop="transToBankName" :label="$t('m.i.ce.qualityBankName')">
  ...
  export default {
    ...
    data() {
      return {
        ...
        custAcctNoRule: [{ test: this.validcustAcctNo, trigger: "blur" }],
        //导入
        importWin: false,
        fileParams: {
          //导入excel数据解析接口
          loadExcelUrl: window.LOCAL_CONFIG.API_HOME + "/ce/cpesrgst/impawnRgst/impawnRgstApply/func_analysisExcelData",
          //excel数据导入接口
          fileUploadUrl: window.LOCAL_CONFIG.API_HOME + "/ce/cpesrgst/impawnRgst/impawnRgstApply/func_saveExcelData",
    ...
    mounted() {
      getDictListByGroups("DraftTypeCode,AcptRgstStatus,Yon").then(res => {
        this.billTypeList = res.get("DraftTypeCode");
        this.dictMap = res.get("map");
      });
    },
    methods: {
      validcustAcctNo(rule,val,callback){
        let re = /^[a-zA-Z0-9]{1,32}$/;
        if(re.test(val)){
          callback()
        }else{
          callback(new Error("非法输入，只能输入英文、数字,且最长不能超过32位"))
        }
      },
      ...
      showCpesBranchSubmit(info) {
        ...
        if (this.cpesBranchType === "acpt") {
          ...
          this.addOrEditForm.transFromBankName = info.brchFullNameZh;
          // this.addOrEditForm.transFromSocCode = info.socCode;
      ...
      singleAdd(type) {
        ...
        if (this.addOrEditType === "modify") {
          ...
          let id = list[0].id;
          post({ id: id }, "/ce/cpesrgst/impawnRgst/impawnRgstApply/func_queryPaperRgstById").then(res => {
            if (res) {
        ...
        this.$refs["addOrEditForm"].validate(valid => {
          if (valid) {
            let url = this.addOrEditType === "add" ?
              "/ce/cpesrgst/impawnRgst/impawnRgstApply/func_addImpawnRgstInfo" :
              "/ce/cpesrgst/impawnRgst/impawnRgstApply/func_updateImpawnPaperRgst";
      ...
      handleDel() {
        ...
        post({ ids: ids }, "/ce/cpesrgst/impawnRgst/impawnRgstApply/func_deleteImpawnPaperRgst").then(res => {
          if (res) {
            let retCode = res.data.retCode;
            if (retCode === "000000") {
              this.$msgTip.success(this);
      ...
      //模板下载
      templateDownload() {
        let url = window.LOCAL_CONFIG.API_HOME + "/ce/cpesrgst/impawnRgst/impawnRgstApply/func_downloadImpawnRgstModel";
      ...
      handleRecheck() {
        ...
        post({ ids: ids }, "/ce/cpesrgst/impawnRgst/impawnRgstApply/func_submitRgstApply").then(res => {
          if (res) {
            let retCode = res.data.retCode;
            if (retCode === "000000") {
              this.$msgTip.success(this);
