      ...
      <h-col span="24">
        ...
        <div v-if="mainVue" key="1">
          ...
          <h-datagrid :columns="columns"
                      highlightRow
                      url="/ce/acpt/paper/print/acptPrintBackRecheck/func_queryPageAcptBatchAndBill"
                      :bindForm="formItem"
        ...
        <acpt-batch-info-detail :is-show="billManagerVue" :params="batchInfoParams"
                                :batchUrl = "'/ce/acpt/paper/print/acptPrintBackRecheck/func_getAcptBatchByQueryDto'"
                                ref="acptBatchInfoDetail"></acpt-batch-info-detail>
        <!-- 数据展示表格 -->
        <div v-if="billManagerVue" key="3">
          <h-datagrid :columns="billManagerColumns"
                      url="/ce/acpt/paper/print/acptPrintBackRecheck/func_queryPageAcptBill"
                      highlightRow
                      :row-select="true"
                      :has-select="false"
        ...
        <!-- 退回原因弹窗 -->
        <h-msg-box v-model="recheckBackWin" width="400" class="h-form-search-layer" :maximize="true" :mask-closable="false">
          <p slot="header">
            <span>退回原因录入</span>
          </p>
          <div>
            <h-form :model="recheckBackForm" :label-width="115" ref="recheckBackForm" cols="1" class="h-form-search">
              <h-form-item prop="vochReturnReason" :label="$t('m.i.ce.paymentReturnReason')" required>
                <h-input type="textarea" v-model="recheckBackForm.vochReturnReason" placeholder="" :rows="2" :canResize="false" :maxlength="100"></h-input>
              </h-form-item>
            </h-form>
          </div>
          <div slot="footer">
            <div slot="footer">
              ...
              <h-button type="primary" @click="backSubmit()">{{$t("m.i.common.back")}}</h-button>
            </div>
          </div>
        </h-msg-box>
    ...
    data() {
      ...
      return {
        ...
        formItem: {
          ...
          billClass: 'ME01',
          acptStatus: '1',
          curFuncNo: 'PICE010404',
          applTellerNoNotEqual : this.$store.getters.userInfo.userNo,
          auditModeKey : 'ce.acpt.vochback.is_check_exclude',
          isLock:'1',
          deductStatus: '0',
          authPath:this.$route.path,
          ownedBrchNos:"",
        ...
        imageParams: {
          ...
          id:"",
          prodNo: "02020211",
          pageSearchUrl:"/ce/acpt/paper/print/acptPrintBackRecheck/func_pageQueryAcptImgWithProd",
          imgShowUrl:"/ce/acpt/paper/print/acptPrintBackRecheck/func_showAcptImgWithProd?id="
        },
        //查询保证金账户信息
    ...
    methods: {
      ...
      recheckPass() {
        ...
        post({batchId: this.batchId, ids: acptIds, billOperType: '1',licenseFlag :this.licenseFlag,
          ownedBrchNo :this.currentSelectRow.ownedBrchNo}, "/ce/acpt/paper/print/acptPrintBackRecheck/func_vochPrintReturnCheck").then(res => {
          if(res) {
      ...
      //退回确认
      backSubmit() {
        this.recheckBackForm.batchId = this.batchId;
        //集中授权参数赋值
        this.recheckBackForm.licenseFlag=this.licenseFlag;
        this.recheckBackForm.ownedBrchNo=this.currentSelectRow.ownedBrchNo;
        let list = this.$refs.billManagerDatagrid.selects;
        let acptIds = "";
        for(let i=0; i<list.length; i++) {
          acptIds += list[i].id;
        ...
        this.recheckBackForm.ids = acptIds;
        this.$refs["recheckBackForm"].validate(valid => {
          if (valid) {
            this.submitFlag = true;
            post(this.recheckBackForm, "/ce/acpt/paper/print/acptPrintBackRecheck/func_vochPrintReturnCheck").then(res => {
