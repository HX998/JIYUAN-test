<template>
  <div>
    <h-msg-box  v-model="tempInvoiceReplenishWin" :maximize="true" :mask-closable="false"
      ...
      <div>
        ...
        <h-datagrid
          ...
          highlight-row
          :auto-load="false"
          url="/ce/acpt/paper/invoice/invoiceReplenishMain/func_queryPageAcptBillByCondition"
          :bindForm="formItem"
    ...
    <bill-info-main :showBillInfoWin="showBillInfoWin" @billInfoWinClose="billInfoWinClose"  :billId="this.billId" :billNo="this.billNo"></bill-info-main>
...
<script>
  import { off, on, post, formatNumber, accMul, accDiv } from "@/api/bizApi/commonUtil";
  ...
  export default {
    ...
    methods: {
      ...
      queryObjById() {
        ...
        post(params, "/ce/acpt/paper/invoice/invoiceReplenishMain/func_getAcptBatchById").then(res => {
          if (res) {
      ...
      formSave() {
        this.$refs["invoiceReplenishForm"].validate (valid => {
            if(valid) {
              ...
              let url = "/ce/acpt/paper/invoice/invoiceReplenishMain/func_invoiceReplenishFinish";
      ...
      billPickInvoiceChange(listId) {
        if (this.billPickInvoiceParams.invoiceFlag === "1") {
          ...
          post({id:listId }, "/ce/acpt/paper/invoice/invoiceReplenishMain/func_relatedInvoice").then(res => {
            if (res) {
              let retCode = res.data.retCode;
              if (retCode === "000000") {
                this.billPickInvoiceWinClose();
      ...
      invoiceManager() {
        ...
        if (invoiceFlag === "1") {
          ...
          this.relationInvoiceBillParams.cancelRelationUrl = "/ce/acpt/paper/invoice/invoiceReplenishMain/func_cancelRelatedInvoice";
