      ...
      <h-col span="24">
        <div>
          ...
          <h-datagrid :columns="columns"
                      highlightRow
                      url="/ce/acpt/paper/stopPayment/stopPaymentRegisterMain/func_pagingQuery"
                      :bindForm="formItem"
        ...
        <!-- 止付信息登记界面 -->
        <h-msg-box v-model="stopPaymentRegisterWin" width="800" @on-close="stopPaymentRegisterWinClose" class="h-form-search-layer" :maximize="true" :mask-closable="false">
          <p slot="header">
            <span>止付信息登记</span>
          </p>
          <div>
...
export default {
    ...
    methods: {
      ...
      //影像上传按钮是否展示
      getUploadIsShow() {
        post({}, "/ce/acpt/paper/stopPayment/stopPaymentRegisterMain/func_getImageUploadBtnParam").then(res => {
          if (res) {
            let retCode = res.data.retCode;
            if (retCode === "000000") {
      ...
      queryCheckConfirm(billNo, payRgstStatus){
        post({code:billNo}, "/ce/acpt/paper/stopPayment/stopPaymentRegisterMain/func_queryCheckConfirm").then(res => {
          if(res) {
            ...
            if (retCode === "000000") {
              if(this.queryCheck === true){
                if(payRgstStatus === "1") {
                  confirmText = "已经查询查复且已经付款登记，是否继续？"
                }else {
                  confirmText = "已经查询查复，是否继续？"
                }
      ...
      stopPaymentRegisterSubmitNext() {
        ...
        this.$refs["stopPaymentRegisterForm"].validate(valid => {
          if (valid) {
            ...
            let url = "/ce/acpt/paper/stopPayment/stopPaymentRegisterMain/func_submit";
      ...
      cancelStopPayment() {
        ...
        let workDate = window.sessionStorage.getItem("workDate").replace(/-/g, "");
        if(this.currentSelectRow.sspdDt === parseInt(workDate) && this.currentSelectRow.acptRgstStatus !== "2-BTA04"){
          this.$msgTip.info(this, { info: "挂失止付日期与当前营业日期一致,请先进行止付信息登记" });
          return;
        }
        ...
        post({ id: this.currentSelectRow.sspdId }, "/ce/acpt/paper/stopPayment/stopPaymentRegisterMain/func_info").then(res => {
          this.submitFlag = false;
          if (res) {
            let retCode = res.data.retCode;
            if (retCode === "000000") {
      ...
      cancelStopPaymentSubmitNext() {
        ...
        this.$refs["cancelStopPaymentForm"].validate(valid => {
          if (valid) {
            this.submitFlag = true;
            let url = "/ce/acpt/paper/stopPayment/stopPaymentRegisterMain/func_cancel";
      ...
      getParams(){
        if (this.$route.path === "/ce/acpt/paper/stopPayment/stopPaymentRegisterMain") {
