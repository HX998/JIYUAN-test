      ...
      <h-col span="24">
        <div>
          ...
          <h-datagrid :columns="columns"
                      ...
                      :row-select="true"
                      :has-select="false"
                      url="/ce/acpt/paper/stopPayment/stopPaymentRecheck/func_waitCheckAcctList"
                      :bindForm="formItem"
                      ref="datagrid">
    ...
    methods: {
      ...
      account() {
        ...
        this.$hMsgBox.confirm({
          ...
          content: "确定记账吗?",
          onOk: () => {
              post(param, "/ce/acpt/paper/stopPayment/stopPaymentRecheck/func_isFeePrintEnable").then(res => {
                if(res) {
      ...
      //获取分录信息
      getEntryInfo() {
        this.acctRecordUrl = "/ce/acpt/paper/stopPayment/stopPaymentRecheck/func_queryAcctRecord";
      ...
      acctSubmitSure(info) {
        ...
        post({ code:this.ids ,accountType:"1",billClass:"ME01" }, "/ce/acpt/paper/stopPayment/stopPaymentRecheck/func_acpSspdAcct").then(res => {
      ...
      accountBack() {
        ...
        let acptId = this.$refs.datagrid.selects[0].id;
        this.$hMsgBox.confirm({
          title: this.$t("m.i.common.confirm"),
          content: "确定退回至上一岗吗?",
          onOk: () => {
            post({id: acptId, isBack: "1"}, "/ce/acpt/paper/backUnused/backUnusedAccount/func_acptDrwgBackAcct").then(res => {
              if(res) {
                let msg = res.data.retMsg;
                let retCode = res.data.retCode;
                if (retCode === "000000") {
