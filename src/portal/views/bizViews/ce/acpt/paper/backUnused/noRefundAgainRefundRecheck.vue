      ...
      <h-col span="24">
        <div>
            ...
            <h-panel class="clearfix" :class="{'h-form-overhd':!ifShowMore}">
              <h-form :model="formItem" :label-width="90" ref="formItem" cols="4" class="h-form-search">
                ...
                <h-form-item prop="drwgBackReturnsDt" :label="$t('m.i.ce.drwgBackReturnsDt')">
                  <h-date-picker type="date" v-model="formItem.drwgBackReturnsDt" placeholder="" @on-change="handleDrwgDtChange" :editable=false></h-date-picker>
                </h-form-item>
          ...
          <h-datagrid :columns="columns"
                      ...
                      :row-select="true"
                      :has-select="false"
                      url="/ce/acpt/paper/backUnused/noRefundAgainRefundRecheck/func_pageQueryAcptBillReturnFee"
                      :bindForm="formItem"
                      ref="datagrid">
...
export default {
    name: "noRefundAgainRefundRecheck",
    components: {
    },
    data() {
      return {
    ...
    mounted() {
      getDictListByGroups("AcptType,RiskStatus").then(res => {
    ...
    methods: {
      ...
      recheckPass() {
        ...
        post({ids: ids, billOperType: '1'}, "/ce/acpt/paper/backUnused/noRefundAgainRefundRecheck/func_acptDrwgBackRefundsCheck").then(res => {
          if(res) {
      ...
      recheckBack() {
        ...
        post({ids: ids, billOperType: '2'}, "/ce/acpt/paper/backUnused/noRefundAgainRefundRecheck/func_acptDrwgBackRefundsCheck").then(res => {
          if(res) {
            this.$loadingbox.close();
            let msg = res.data.retMsg;
            let retCode = res.data.retCode;
