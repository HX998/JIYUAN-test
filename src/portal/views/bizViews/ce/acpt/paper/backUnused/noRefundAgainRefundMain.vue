      ...
      <h-col span="24">
        <div>
          <div class="h-form-search-box">
            ...
            <h-panel class="clearfix" :class="{'h-form-overhd':!ifShowMore}">
              <h-form :model="formItem" :label-width="90" ref="formItem" cols="4" class="h-form-search">
                ...
                <h-form-item prop="drwgBackDt" :label="$t('m.i.ce.drwgBackDt')">
                  <h-date-picker type="date" v-model="formItem.drwgBackDt" placeholder=""  @on-change="handleDrwgBackDtChange" :editable=false></h-date-picker>
                </h-form-item>
                <h-form-item prop="isReturnIssueFee" :label="$t('m.i.ce.isReturnIssueFee')">
                  <h-select v-model="formItem.isReturnIssueFee" placeholder="" @on-change="changeIsReturnIssueFee">
          ...
          <h-datagrid :columns="columns"
                      ...
                      :row-select="true"
                      :has-select="false"
                      url="/ce/acpt/paper/backUnused/noRefundAgainRefundMain/func_pageQueryAcptBillReturnFee"
                      :bindForm="formItem"
                      ref="datagrid">
            <div slot="toolbar" class="pull-left">
              <h-button type="primary" @click="refund" :disabled="formItem.isReturnIssueFee == '1'">{{$t('m.i.ce.returnIssueFee')}}</h-button>
            </div>
          </h-datagrid>
        </div>
        <!-- 查询客户信息弹窗 -->
...
export default {
    ...
    data() {
      return {
        ...
        columns: [
          ...
          {
            title: this.$t("m.i.ce.isReturnIssueFee"),
            key: 'isReturnIssueFee',
            hiddenCol: false,
            render: (h, params) => {
              return h("span", this.getDictValueFromMap(this.dictMap,"Yon",params.row.isReturnIssueFee));
            }
          },
          {
    ...
    mounted() {
      this.getDictListByGroups("Yon,AcptType,RiskStatus").then(res => {
    ...
    methods: {
      ...
      handleDrwgBackDtChange(value) {
        this.formItem.drwgBackDt = value.replace(/-/g, "");
      },
      //表单查询
      handleSearch() {
      ...
      refund() {
        ...
        this.$hMsgBox.confirm({
          ...
          content: "确定退费吗?",
          onOk: () => {
            post({ids: ids}, "/ce/acpt/paper/backUnused/noRefundAgainRefundMain/func_acptDrwgBackRefundsApply").then(res => {
              if(res) {
