      ...
      <h-col span="24">
        <div>
          ...
          <h-datagrid :columns="columns"
                      ...
                      :row-select="true"
                      :has-select="false"
                      url="/ce/acpt/paper/paymentDue/acptSettleRegisterRecheck/func_query"
                      :bindForm="formItem"
                      ref="datagrid">
        ...
        <!-- 退回原因录入弹窗 -->
        <h-msg-box v-model="acptSettleRegisterBackWin" width="400" @on-close="acptSettleRegisterBackWinClose" class="h-form-search-layer" :maximize="true" :mask-closable="false">
          <p slot="header">
            <span>退回原因录入</span>
          </p>
          <div>
            <h-form :model="acptSettleRegisterBackForm" :label-width="115" ref="acptSettleRegisterBackForm" cols="1" class="h-form-search">
              <common-input v-model="acptSettleRegisterBackForm.settleRgstReturnReason" :label="$t('m.i.common.returnReason')" prop="settleRgstReturnReason"
                            :required="true" type="textarea" :rows="3" :canResize="false" :maxlength="100"></common-input>
            </h-form>
          </div>
          <div slot="footer">
            <div slot="footer">
              <h-button type="ghost" @click="acptSettleRegisterBackWinClose">{{$t("m.i.common.close")}}</h-button>
              <h-button type="primary" v-if="submitFlag" disabled>{{$t("m.i.common.submiting")}}</h-button>
              <h-button type="primary" v-else @click="acptSettleRegisterBackWinSubmit()">{{$t("m.i.common.back")}}</h-button>
            </div>
          </div>
        </h-msg-box>
...
export default {
    name: "acptSettleRegisterRecheck",
    components: {
    },
    data() {
      return {
        ...
        formItem: {
          ...
          minDueDt: "",
          maxDueDt: "",
          acptRgstStatusList : ['3-BTA02','3-BTA05'],
          curFuncNo : 'PICE011002',
          applTellerNoNotEqual : this.$store.getters.userInfo.userNo,
          auditModeKey : 'ce.acpt.payment.is_settle_pay_rgst_check_exclude',
          billClass: 'ME01',
        ...
        columns: [
          ...
          },
          {
            ...
            hiddenCol: false,
            render: (h, params) => {
              return h("span", getDictValueFromMap(this.dictMap,"RiskStatus",params.row.riskStatus));
            }
          },
          {
        ...
        ifShowMore: false,
        acptSettleRegisterBackWin: false,
        submitFlag: false,
        acptSettleRegisterBackForm: {
          ids: '',
          billOperType: '',
          settleRgstReturnReason: '',
        },
      }
    },
    mounted() {
    ...
    methods: {
      ...
      pass() {
        ...
        post({ids: ids, billOperType :"1",licenseFlag:this.licenseFlag,ownedBrchNo:this.$refs.datagrid.selects[0].ownedBrchNo}, "/ce/acpt/paper/paymentDue/acptSettleRegisterRecheck/func_check").then(res => {
          if (res) {
      ...
      refuse() {
        ...
        this.acptSettleRegisterBackWin = true;
      },

      //退回弹窗确认
      acptSettleRegisterBackWinSubmit() {
         this.$refs["acptSettleRegisterBackForm"].validate(valid => {
          if (valid) {
            let list = this.$refs.datagrid.selects;
            let acptIds = [];
            for(let i=0; i<list.length; i++) {
            ...
            this.acptSettleRegisterBackForm.ids = acptIds;
            this.acptSettleRegisterBackForm.billOperType = '2',
            this.submitFlag = true;
            post(this.acptSettleRegisterBackForm, "/ce/acpt/paper/paymentDue/acptSettleRegisterRecheck/func_check").then(res => {
              ...
              if(res) {
                ...
                if (retCode === "000000") {
                  this.$msgTip.success(this);
                  this.acptSettleRegisterBackWinClose();
                  this.$refs.datagrid.dataChange(this.$refs.datagrid.$refs.gridPage.currentPage);
                  this.$refs.datagrid.selects = [];
                } else {
                  this.$msgTip.error(this, { info: msg });
      ...
      //退回弹窗关闭
      acptSettleRegisterBackWinClose() {
        this.acptSettleRegisterBackWin = false;
        this.$nextTick(()=>{
          this.$refs.acptSettleRegisterBackForm.resetFields();
        })
      }
