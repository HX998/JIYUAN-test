      ...
      <h-col span="24">
        <div>
          ...
          <h-datagrid :columns="columns"
                      ...
                      :row-select="true"
                      :has-select="false"
                      url="/ce/acpt/paper/paymentDue/paymentRegisterRecheck/func_pageQueryPaymentRgstCheckList"
                      :bindForm="formItem"
                      ref="datagrid">
        ...
        <!-- 录入复核 -->
        <h-msg-box v-model="inputRecheckWin" width="450" @on-close="inputRecheckWinClose" class="h-form-search-layer" :maximize="true" :mask-closable="false">
          <p slot="header">
            <span>付款登记复核</span>
          </p>
          <div>
            <h-form :model="inputRecheckForm" :label-width="115" ref="inputRecheckForm" cols="1" class="h-form-search" onlyBlurRequire>
              <h-form-item prop="prsttnAcctNo" :label="$t('m.i.ce.entrustAcctNo')" required :validRules="prsttnAcctNoRule">
                <h-input v-model="inputRecheckForm.prsttnAcctNo" placeholder="" :maxlength="32"></h-input>
              </h-form-item>
              <h-form-item prop="prsttnName" :label="$t('m.i.ce.entrustName')" required :validRules="prsttnNameRule">
              ...
              <h-form-item prop="prsttnBankName" :label="$t('m.i.ce.entrustBankName')" required :validRules="prsttnBankNameRule">
...
export default {
    name: "paymentRegisterRecheck",
    components: {},
    data() {
      return {
        prsttnAcctNoRule: [{ test: this.validatePrsttnAcctNo, trigger: "blur" }],
        prsttnNameRule: [{ test: this.validatePrsttnName, trigger: "blur" }],
        prsttnBankNoRule: [{ test: this.validatePrsttnBankNo, trigger: "blur" }],
        prsttnBankNameRule: [{ test: this.validatePrsttnBankName, trigger: "blur" }],
    ...
    mounted() {
      getDictListByGroups("Yon,DraftTypeCode,AcptType,RiskStatus,DraftTypeCode,FastCashStatus").then(res => {
    ...
    methods: {
      validatePrsttnAcctNo(rule, value, callback){
        debugger
        let re = /^(?!^\-*$)([0-9a-zA-Z\-]{1,32})$/;
        if (re.test(value)) {
          if (value !== this.prsttnAcctNo) {
            callback(new Error("申请岗录入为：" + this.prsttnAcctNo));
          } else {
            callback();
          }
        }else {
          callback(new Error("由小于或等于32位数字或字母或-组成,不能全为-"));
        }
      },
      validatePrsttnName(rule, value, callback){
        debugger
        if (value !== this.prsttnName) {
          callback(new Error("申请岗录入为："+this.prsttnName));
        } else {
          callback();
        }
      },
      validatePrsttnBankNo(rule, value, callback){
        debugger
        let re = /^[0-9]{12}$/;
        if (re.test(value)) {
          if (value !== this.prsttnBankNo) {
            callback(new Error("申请岗录入为：" + this.prsttnBankNo));
          } else {
            callback();
          }
        }else {
          callback(new Error("请输入12位数字的行号"));
        }
      },
      validatePrsttnBankName(rule, value, callback){
        debugger
         if (value !== this.prsttnBankName) {
         callback(new Error("申请岗录入为："+this.prsttnBankName));
         } else {
         callback();
         }
      },
      ...
      getIfSecondRecheckBusiParam() {
        post({ paramKey: "ce.acpt.payment.register_recheck_mode" }, "/sm/auth/businessParamete/getBusinessParameter").then(res => {
          if (res) {
      ...
      inputRecheck() {
        ...
        this.prsttnAcctNo = this.$refs.datagrid.selects[0].prsttnAcctNo;
        this.prsttnName = this.$refs.datagrid.selects[0].prsttnName;
        this.prsttnBankNo = this.$refs.datagrid.selects[0].prsttnBankNo;
        this.prsttnBankName = this.$refs.datagrid.selects[0].prsttnBankName;

        this.inputRecheckForm.prsttnAcctNo = this.$refs.datagrid.selects[0].prsttnAcctNo;
        this.inputRecheckForm.prsttnName = this.$refs.datagrid.selects[0].prsttnName;
        this.inputRecheckForm.prsttnBankNo = this.$refs.datagrid.selects[0].prsttnBankNo;
        this.inputRecheckForm.prsttnBankName = this.$refs.datagrid.selects[0].prsttnBankName;
      ...
      inputRecheckWinSubmit() {
        ...
        this.$refs["inputRecheckForm"].validate(valid => {
          if (valid) {
            let url = "/ce/acpt/paper/paymentDue/paymentRegisterRecheck/func_paymentDueRgstCheckEnter";
      ...
      recheckConfirm() {
        ...
        this.$hMsgBox.confirm({
          ...
          content: content,
          onOk: () => {
            this.paymentBackForm.paymentReturnReason = '复核岗录入的信息与申请岗的不一致';
            this.paymentBackWinSubmit();
          },
          onCancel: () => {
            this.$refs.datagrid.dataChange(this.$refs.datagrid.$refs.gridPage.currentPage);
          }
      ...
      inputRecheckWinClose() {
        ...
        this.prsttnBankNo = "";
        this.prsttnBankName = "";
        this.inputRecheckForm.prsttnAcctNo = "";
        this.inputRecheckForm.prsttnName = "";
        this.inputRecheckForm.prsttnBankNo = "";
        this.inputRecheckForm.prsttnBankName = "";
      ...
      recheckPass() {
        ...
        this.$hMsgBox.confirm({
          ...
          onOk: () => {
            ...
            post({ ids: acptIds, billOperType: "1", billClass: "ME01" }, "/ce/acpt/paper/paymentDue/paymentRegisterRecheck/func_paymentDueRgstCheck").then(res => {
              if (res) {
      ...
      paymentBackWinSubmit() {
        this.$refs["paymentBackForm"].validate(valid => {
          if (valid) {
              ...
              post(this.paymentBackForm, "/ce/acpt/paper/paymentDue/paymentRegisterRecheck/func_paymentDueRgstCheck").then(res => {
