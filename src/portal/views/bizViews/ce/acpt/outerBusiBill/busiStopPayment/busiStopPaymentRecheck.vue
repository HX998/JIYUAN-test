      ...
      <h-col span="24">
        <div>
          ...
          <h-datagrid :columns="columns"
                      ...
                      :row-select="true"
                      :has-select="false"
                      url="/ce/acpt/outerBusiBill/busiStopPayment/busiStopPaymentRecheck/func_waitCheckList"
                      :bindForm="formItem"
                      ref="datagrid">
    ...
    data() {
      return {
           ...
           title: this.$t("m.i.common.showImg"),
            ...
            render: (h, params) => {
              return h('a', {
                on: {
                  click: () => {
                    this.openImgWin(params.row.id,params.row.sspdStatus)
                  }
                }
              }, "查看");
            }
        ...
        imageParams: {
          listId: '',
          imgBusiType: '',
          pageSearchUrl:"/ce/acpt/outerBusiBill/busiStopPayment/busiStopPaymentRecheck/func_imageListPage",
          //查看
          imgShowUrl:"/ce/acpt/outerBusiBill/busiStopPayment/busiStopPaymentRecheck/func_imageById?id="
        }
      }
    },
    mounted() {
    ...
    methods: {
      ...
      recheckPass() {
        ...
        for(let i=0; i<list.length; i++) {
          ids.push(list[i].id);
          acptIds.push(list[i].acptId);
          //止付待复核的数据且申请人不为行内客户
          if(list[i].isInnerAcct === "0" && list[i].sspdStatus==="止付待复核" && (list[i].sspdAndRelieveType === "挂失止付"|| list[i].sspdAndRelieveType === "公示催告")) {
            billNos.push(list[i].billNo);
          }
        }
        if (billNos.length===0){
          this.sendPass(ids,acptIds);
        }else {
          let billNosShow = "";
      ...
      sendPass(ids,acptIds){
        this.$loadingbox.open({
          title: "提交中，请稍后..."
        });
        post({ids:ids,acptIds:acptIds}, "/ce/acpt/outerBusiBill/busiStopPayment/busiStopPaymentRecheck/func_checkPass").then(res => {
          if(res) {
      ...
      recheckBack() {
        ...
        post({code:ids}, "/ce/acpt/outerBusiBill/busiStopPayment/busiStopPaymentRecheck/func_checkBack").then(res => {
          if(res) {
            this.$loadingbox.close();
            let msg = res.data.retMsg;
            let retCode = res.data.retCode;
