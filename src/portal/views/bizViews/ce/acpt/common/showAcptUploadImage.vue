  ...
  <h-msg-box v-model="tempShowUploadImageWin" :mask-closable="false" @on-close="handleClose"
    ...
    <div>
      <h-form :model="formItem" :label-width="100" ref="formItem" cols="4" class="h-form-search">
        <h-form-item :label="$t('m.i.common.imgType')" prop="imgType">
          <h-select v-model="formItem.imgType"  placeholder="" readonly>
    ...
    <h-datagrid
      ...
      <div slot="toolbar">
        ...
        <h-button type="primary" @click="registerSubmit">{{buttonName}}</h-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //影像新增
      openImgWin() {
        let tData = this.$refs.datagrid.tData;
        if(tData.length === 1 && tData[0].imgType === "IM01") {
          this.$msgTip.info(this,{info: this.$t('m.i.ce.openImgFailMsg'),});
          return;
        }
