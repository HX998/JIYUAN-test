<template>
  ...
  <h-msg-box v-model="tempShowUploadImageWin" :mask-closable="false" @on-close="handleClose"
    ...
    <h-datagrid
      ...
      highlight-row
      :height="300"
      url="/ce/acpt/paper/search/acptTrackSearch/func_pageQueryAcptImgList"
      :on-select-change="handlelRowClick"
      ref="datagrid">
      <div slot="toolbar">
        <h-button type="primary" @click="openImgWin" :disabled="isDisabled">{{$t("m.i.common.imageAdd")}}</h-button>
        <h-button type="primary" @click="registerSubmit" :disabled="isDisabled">{{buttonName}}</h-button>
        <h-button type="primary" @click="deleteDetailed" :disabled="isDisabled">{{$t("m.i.common.delete")}}</h-button>
      </div>
    </h-datagrid>
    <div slot="footer">
    ...
    <!-- 影像上传-->
    <common-upload-image :imageFormItem="this.imageFormItem" :imageFormatType="this.imageFormatType" :imageSize="512"
                         :uploadImageWin="uploadImageWin" :upload-url="this.uploadUrl" @uploadImageWinClose="uploadImageWinClose"></common-upload-image>
...
<script>
  ...
  export default {
    name: "showAcptSearchUploadImage",
    components:{
    },
    data() {
      return {
    ...
    methods: {
      ...
      registerSubmit(){
        ...
        this.$hMsgBox.confirm({
          ...
          onOk: () => {
            post({ id: id}, "/ce/acpt/paper/search/acptTrackSearch/func_sendAcptImgMsg").then(res => {
              if (res) {
      ...
      handleDeleteDetailed() {
        let id = this.currentSelectRow[0].id;
        post({ id: id}, "/ce/acpt/paper/search/acptTrackSearch/func_deleteAcptImg").then(res => {
          if (res) {
            let retCode = res.data.retCode;
            if (retCode === "000000") {
