...
<!--                </show-branch>-->
                <h-form-item prop="reverseBillNo" :label="$t('m.i.billInfo.billPackageNo')">
...
<!--                  <span class="h-more-input" @click="ifShowMore=!ifShowMore">{{$t('m.i.common.searchAdvanced')}}<i class="icon iconfont" :class="{'icon-arrow-down-screensv':!ifShowMore,'icon-arrow-up-screen':ifShowMore}"></i></span>-->
          ...
          <h-datagrid :columns="columns"
                      ...
                      :row-select="true"
                      :has-select="false"
                      url="/ce/acpt/elec/backUnused/eBackUnusedConfirmRecheck/func_pageQueryReplaceAcptDrwgBackOrPaymentAcct"
                      :bindForm="formItem"
                      ref="datagrid">
...
export default {
    name: "ePaymentAccountReplaceMain",
    components: {
    ...
    mounted() {
      getDictListByGroups("RemittanceChannel,DraftTypeCode,Yon,SettlementMarkCode,FastCashStatus,SettleType,BillOrigin").then(res => {
    ...
    methods: {
      ...
      paymentAccount() {
        ...
        this.$hMsgBox.confirm({
          ...
          content: "请确认并检查票号["+billNoStr+"]，代开行是否已完成付款记账，若未完成则不允许付款记账！",
          onOk: () => {
            this.getEntryInfo();
          }
        });
      ...
      //获取分录信息
      getEntryInfo() {
        this.acctRecordUrl = "/ce/acpt/elec/paymentDue/ePaymentAccountMain/func_generateAcctRecordReplace";
      ...
      acctSubmitSure(info) {
        ...
        post({code: this.acptIds,licenseFlag:this.licenseFlag,ownedBrchNo:this.$refs.datagrid.selects[0].ownedBrchNo}, "/ce/acpt/elec/paymentDue/ePaymentAccountMain/func_acctReplace").then(res => {
