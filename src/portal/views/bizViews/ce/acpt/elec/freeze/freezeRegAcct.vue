...
<!--                <common-input v-model="formItem.acptProtocalNo" :label="$t('m.i.billInfo.acptProtocalNo')" prop="acptProtocalNo" :maxlength="60"></common-input>-->
          ...
          <h-datagrid :columns="columns"
                      ...
                      :row-select="true"
                      :has-select="false"
                      url="/ce/acpt/elec/freeze/freezeRegAcct/func_queryFreezeRegAcctByPage"
                      :bindForm="formItem"
                      ref="datagrid">
  ...
  export default {
    ...
    data() {
      return {
        ...
        // 查看附件
        showFileWin: false,
        filePathUrl: "/ce/acpt/elec/freeze/freezeRegAcct/",
        fileBatchParams: {},
        columns: [
          {
            type: 'selection',
        ...
        // 记账
        acctRecordParam: {},
        acctRecordUrl:"/ce/acpt/elec/freeze/freezeRegAcct/func_queryFreezeRegAcctRecord",
    ...
    mounted() {
      getDictListByGroups("DraftTypeCode,SspdStatus,AcptStatus,FreezeRgstType,AttTypeCode,FileDetailStatus,PreRelationFlag").then(res => {
    ...
    methods: {
      ...
      account() {
        ...
        for(let i=0; i<list.length; i++) {
          if(list[i].sspdStatus !== sspdStatus){
            this.$msgTip.info(this, { info: this.$t("冻结和解冻结不能同时记账") });
            return;
          }
          ids += list[i].id;
          if(i<list.length-1) {
      ...
      //分录确认记账
      acctSubmitSure(info) {
        let acctRecordParam = {ids: this.ids};
        let url = "/ce/acpt/elec/freeze/freezeRegAcct/func_freezeRegAcct";
        this.acctSubmitFlag = true;
        post(acctRecordParam, url).then(res => {
