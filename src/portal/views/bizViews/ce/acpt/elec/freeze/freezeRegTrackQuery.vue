      ...
      <h-col span="24">
        <div>
          <div class="h-form-search-box">
            ...
            <h-panel class="clearfix" :class="{'h-form-overhd':!ifShowMore}">
              <h-form :model="formItem" :label-width="90" ref="formItem" cols="4" class="h-form-search">
                ...
                <!--承兑协议编号-->
                <common-input v-model="formItem.acptProtocalNo" :label="$t('m.i.billInfo.acptProtocalNo')" prop="acptProtocalNo" :maxlength="60"></common-input>
          ...
          <h-datagrid :columns="columns"
                      ...
                      :row-select="true"
                      :has-select="false"
                      url="/ce/acpt/elec/freeze/freezeRegTrackQuery/func_freezeRegTrackQueryByPage"
                      :bindForm="formItem"
                      ref="datagrid">
  ...
  export default {
    ...
    data() {
      return {
        ...
        // 查看附件
        showFileWin: false,
        filePathUrl: "/ce/acpt/elec/freeze/freezeRegTrackQuery/",
        fileBatchParams: {},
        //清单导出
        exportParam: "",
        exportRows: "",
        ...
        columns: [
          ...
          },
          {
            ...
            sortable: true,
            render: (h, params) => {
              return h('span', params.row.sspdAndRelieveRgstDt ? this.$moment(params.row.sspdAndRelieveRgstDt, "YYYY-MM-DD").format("YYYY-MM-DD") : "")
            }
          },
          {
    ...
    async mounted() {
      ...
      getDictListByGroups("DraftTypeCode,FreezeStatus,AcptStatus,FreezeRgstType,AttTypeCode,FileDetailStatus,PreRelationFlag").then(res => {
        ...
        this.freezeStatusList = res.get("FreezeStatus");
        this.dictMap = res.get("map");
        // 不需要复核
        if (!this.freezeIsNeedCheckFlag){
          this.freezeStatusList = this.freezeStatusList.filter(item => item.key !== "1" && item.key !== "2" && item.key !== "6" && item.key !== "7");
        }
        // 需要记账
        if (!this.freezeIsNeedAcctFlag){
          this.freezeStatusList = this.freezeStatusList.filter(item => item.key !== "4" && item.key !== "9");
        }
      });
    },
    methods: {
      ...
      showAttach(params) {
        ...
        if (params.row.sspdStatus === '0' && params.row.id === null) {
          this.fileBatchParams.busiId = params.row.acptId;
        } else {
      ...
      //清单导出
      listExport() {
        this.exportParam = "freezeRegTrackQueryExportInfo";
      ...
      exportList(field, exportType) {
        let url = window.LOCAL_CONFIG.API_HOME + "/ce/acpt/elec/freeze/freezeRegTrackQuery/func_exportFreezeRegTrackQueryList";
        let options = {};
        if (this.$refs.datagrid.selectIds.length !== 0) {
          options.params = {
