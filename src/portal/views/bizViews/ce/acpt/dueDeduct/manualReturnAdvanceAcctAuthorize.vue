...
<script>
  ...
  export default {
    name: "manualReturnAdvanceAcctAuthorize",
    components: {
    ...
    data() {
      return {
        ...
        billManagerColumns : [
          ...
          {
            title: this.$t("m.i.ce.returnAdvanceAmt"),
            key: this.authorizeAmt,
            sortable: true,
            render: (h, params) => {
              let authorizeAmt = formatNumber(this.authorizeAmt, 2, ',');
              return h("span", {
                domProps: {
                  title: authorizeAmt
                }
    ...
    mounted() {
      getDictListByGroups("AcptType,GuarntrType,DeductStatusType,Yon").then(res => {
    ...
    props: {
      ...
      queryBatchUrl: {
        type: String,
        default() {
          return "/ce/acpt/dueDeduct/manualDeductMain/func_pagingQueryReturnAdvanceAcptBatch";
        }
      }
    },
    methods: {
      getBatchInfo() {
        ...
        post({
          ...
          deductStatusList:'3',
          ids: this.listIds,
          deductApplFlag: '4',}, this.queryBatchUrl).then(res => {
          if (res) {
      ...
      account() {
        ...
        this.acctRecordParam = {batchId: this.batchId,acptIds: this.listIds , billOperType: "1",prodNo:"010112",advancedPayAmt:this.authorizeAmt}
        this.showAcctRecordWin = true;
      },
      //确定记账
      acctSubmitSure(info) {
        let param = {};
        param = {id:this.id,batchId: this.batchId, billOperType: "0", acptIds: this.listIds,isBack: "0",prodNo:"010112",advancedPayAmt:this.authorizeAmt};
        this.acctSubmitFlag = true;
        post(param, "/ce/acpt/dueDeduct/manualDeductMain/func_returnAdvanceAccountAuthorizePass").then(res => {
      ...
      submitAcctAuthorizeBack(){
        let param = {};
        param = {id:this.id,batchId: this.batchId, acptIds: this.listIds,prodNo:"010112"};
        post(param, "/ce/acpt/dueDeduct/manualDeductMain/func_advanceAccountAuthorizeReturn").then(res => {
          if(res) {
            let msg = res.data.retMsg;
            let retCode = res.data.retCode;
            if (retCode === "000000") {
