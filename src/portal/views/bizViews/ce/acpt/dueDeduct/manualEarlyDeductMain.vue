      ...
      <h-col span="24">
        ...
        <div v-if="mainVue" key="1">
            ...
            <h-panel class="clearfix" :class="{'h-form-overhd':!ifShowMore}">
              <h-form :model="formItem" :label-width="90" ref="formItem" cols="4" class="h-form-search">
                ...
                <h-form-item prop="dueDt" :label="$t('m.i.billInfo.dueDt')">
                  <h-date-picker type="date" v-model="formItem.dueDt" placeholder="" @on-change="handleDateChange" :options="dueOptions"
          ...
          <h-datagrid :columns="columns"
                      highlightRow
                      url="/ce/acpt/dueDeduct/manualEarlyDeductMain/func_pagingQueryEarlyDeductAcptBatch"
                      :bindForm="formItem"
        ...
        <acpt-batch-info-detail :is-show="billManagerVue" :params="batchInfoParams" url="/ce/acpt/dueDeduct/manualEarlyDeductMain/func_pagingQueryEarlyDeductAcptBatch"
                                ref="acptBatchInfoDetail"></acpt-batch-info-detail>
        <!-- 数据展示表格 -->
        <div v-if="billManagerVue" key="3">
          <h-datagrid :columns="billManagerColumns"
                      url="/ce/acpt/dueDeduct/manualEarlyDeductMain/func_pagingQueryEarlyDeductAcptBill"
        ...
        <h-msg-box v-model="showDeductAcctInfoWin" :maximize="true" :mask-closable="false" @on-close="showDeductAcctInfoWinClose"
          ...
          <h-datagrid
            ...
            :rowSelect=true
            url="/ce/acpt/dueDeduct/manualEarlyDeductMain/func_pagingQueryDeductAcctInfo"
            :auto-load="false"
            :height="350"
...
<script>
  import { getDictListByGroups, getDictValueFromMap, off, on, post, formatBillRange,formatNumber,accMul,getMoreParamValuesByKeys} from "@/api/bizApi/commonUtil";
  import { deepClone } from "@/utils/index";

  export default {
    name: "manualEarlyDeductMain",
    components: {
    ...
    mounted() {
      this.getDictListByGroups("CDMedia,custAcctKind,BillOrigin").then(res => {
    ...
    methods: {
      ...
      selectDeductSubmitNoForm() {
        ...
        post({ acptIds: ids, isBatchOper: "0" ,licenseFlag:this.licenseFlag,ownedBrchNo:this.batchInfoParams.ownedBrchNos}, "/ce/acpt/dueDeduct/manualEarlyDeductMain/func_earlyDeductAccount").then(res => {
          if (res) {
      ...
      batchDeductSubmitNoForm() {
        debugger
        this.acctRecordParam = {batchId: this.batchId};
        this.showAcctRecordWin = true;
        post({ batchId: this.batchId, isBatchOper: "1",licenseFlag:this.licenseFlag,ownedBrchNo:this.batchInfoParams.ownedBrchNos }, "/ce/acpt/dueDeduct/manualEarlyDeductMain/func_earlyDeductAccount").then(res => {
          if (res) {
            let msg = res.data.retMsg;
            let retCode = res.data.retCode;
            if (retCode === "000000") {
      ...
      deductConfirmSubmit() {
        ...
        post(params, "/ce/acpt/dueDeduct/manualEarlyDeductMain/func_earlyDeductAccount").then(res => {
