...
<script>
  ...
  export default {
    name: "manualAdvanceAcctAuthorize",
    components: {
    ...
    data() {
      return {
        ...
        billManagerColumns : [
          ...
          {
            title: this.$t("m.i.ce.adjustAdvancedPayAmt"),
            key: 'adjustAdvancedPayAmt',
            sortable: true,
            render: (h, params) => {
              let adjustAdvancedPayAmt = formatNumber(params.row.adjustAdvancedPayAmt, 2, ',');
              return h("span", {
                domProps: {
                  title: adjustAdvancedPayAmt
                }
              }, adjustAdvancedPayAmt);
            }
          },
          {
    ...
    props: {
      ...
      queryBatchUrl: {
        type: String,
        default() {
          return "/ce/acpt/dueDeduct/manualDeductMain/func_pagingQueryDeductAcptBatch";
        }
      }
    },
    methods: {
      getBatchInfo() {
        ...
        post({
          ...
          ids: this.listIds,
          deductApplFlag: '3',}, this.queryBatchUrl).then(res => {
          if (res) {
      ...
      account() {
        ...
        this.acctRecordParam = {batchId: this.batchId,acptIds: this.listIds , billOperType: "1",prodNo:"010111"}
        this.showAcctRecordWin = true;
      },
      //确定记账
      acctSubmitSure(info) {
        let param = {};
        param = {id:this.id,batchId: this.batchId, billOperType: "0", acptIds: this.listIds,isBack: "0",prodNo:"010111"};
        this.acctSubmitFlag = true;
        post(param, "/ce/acpt/dueDeduct/manualDeductMain/func_advanceAccountAuthorizePass").then(res => {
      ...
      submitAcctAuthorizeBack(){
        let param = {};
        param = {id:this.id,batchId: this.batchId, acptIds: this.listIds,prodNo:"010111"};
