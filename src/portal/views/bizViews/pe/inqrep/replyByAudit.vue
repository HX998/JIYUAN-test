      ...
      <h-col span="24">
        <div>
          ...
          <div class="h-form-search-box">
            <h-panel class="clearfix" :class="{'h-form-overhd':!ifShowMore}">
              <h-form :model="formItem" :label-width="90" ref="formItem" cols="4" class="h-form-search">
                <h-form-item :label="$t('m.i.pe.applMemberId')" prop="applMemberId" v-if="false">
                  <h-input v-model="formItem.applMemberId" :maxlength="6"></h-input>
                </h-form-item>
                <h-form-item :label="$t('m.i.pe.applMemberName')" prop="applMemberName">
                  <h-input v-model="formItem.applMemberName" readonly
                           icon="android-search" @on-click="queryApplMemberName()" :maxlength="60" clearable
                           @on-clear="clearApplMemberName()"></h-input>
                </h-form-item>
                <!--<h-form-item :label="$t('m.i.pe.applBrchCode')" prop="applBrchCode" v-if="false">
                  <h-input v-model="formItem.applBrchCode" :maxlength="9"></h-input>
                </h-form-item>
                <h-form-item :label="$t('m.i.pe.applBrchName')" prop="applBrchName">
                  <h-input v-model="formItem.applBrchName" readonly
                           icon="android-search" @on-click="queryApplBrchName('query')" :maxlength="60" clearable
                           @on-clear="clearApplBrchName('query')"></h-input>
                </h-form-item>-->
                <show-cpes-branch v-model="formItem.applBrchCode" :label="$t('m.i.pe.applBrchName')"
          ...
          <h-datagrid
            :columns="columns"
            highlight-row
            url="/pe/inqrep/queryAnswer/findQueryAnswerRcvByAudit"
            :bindForm="formItem"
            ...
            <div slot="toolbar" class="pull-left">
              <h-button type="primary" @click="handleAudit('pass')" v-if="authObj.auditRcvPass">
                {{$t("m.i.common.recheckPass")}}
              </h-button>
              <h-button type="primary" @click="handleAudit('refuse')" v-if="authObj.auditRcvRefuse">
  ...
  export default {
    ...
    data() {
      return {
        ...
        //给按钮增加权限
        authObj: {
          auditRcvPass: true,        //复核通过
          auditRcvRefuse: true,      //复核拒绝
    ...
    methods: {
      clearApplMemberName() {
        this.formItem.applMemberId = "";
        this.formItem.applMemberName = "";
      },
      //查询会员名称
      queryApplMemberName() {
      ...
      memberIdWinSubmit(info) {
        ...
        this.formItem.applMemberName = info.memberName;
        this.showCpesMemberWin = false;
      },
     /* clearApplBrchName(brchType) {
        if (brchType === "query") {
     ...
     /* queryApplBrchName(brchType) {
      ...
      handleAudit(str) {
        if (this.currentSelectRow != null && this.currentSelectRow.length !== 0) {
          let title = str === "pass" ? "发起查复报文复核通过" : "发起查复报文复核拒绝";
          let content = str === "pass" ? "确定要复核通过吗?" : "确定要复核拒绝吗?";
          let url = str === "pass" ? "/pe/inqrep/queryAnswer/auditRcvPass" : "/pe/inqrep/queryAnswer/auditRcvRefuse";
            ...
            onOk: () => {
              post({ id: this.currentSelectRow.id }, url).then(res => {
                if (res) {
                  ...
                  let msg = str === "pass" ? this.$t("m.i.common.recheckPass") : this.$t("m.i.common.recheckRefuse");
                  if (retCode === "000000") {
    ...
    mounted() {
      this.getDictListByGroups("QueryAnswerStatus,QueryAnswerAuditFlag").then(res => {
