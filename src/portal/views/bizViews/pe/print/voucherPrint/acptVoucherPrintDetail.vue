<!-- 承兑凭证打印明细页面-按明细打印-挂失止付\解除挂失止付\解付签收\拒绝付款理由书-->
<template>
  <div>
    <div v-if="isShow">
...
<script>
  ...
  export default {
      ...
      post({},"/pe/print/voucherPrint/voucherPrintManger/func_queryVouchreTypeDict").then(res=>{
        if (res) {
          if (res.data.retCode === '000000') {
            ...
            this.getDictListByGroups("DraftTypeCode,AcptType,GuarntrType,VoucherType,BillOrigin").then(res => {
    ...
    methods: {
      printWinClose() {
        this.printFormSearchReset();
        this.printForm.vochId = "";
        this.printWin = false;
      },
      //行选中
      handleRowClick(arr) {
      ...
      getPrintNumMap() {
        ...
        post({
          ...
          "backFlowNo": this.billManagerParams.backFlowNo
        }, url).then(res => {
            if (res) {
      ...
      getBillManagerDataList() {
        let url = "/pe/print/voucherPrint/voucherPrintManger/func_pageQueryAcptBillList";
        post({
          "batchId": this.batchId,
          "ids": this.listIds,
