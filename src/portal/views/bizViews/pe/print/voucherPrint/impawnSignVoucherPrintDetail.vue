...
<script>
  ...
  export default {
    name: "impawnSignVoucherPrintDetail",
    data() {
      return {
        currentSelectRow: [],
        printObj: "",
    ...
    methods: {
      ...
      getBillManagerDataList() {
        let url = "/ce/impawn/elec/elecImpawn/eImpawnTrackSearch/func_pageQueryImpawnBillList";
        post({
          "batchId": this.batchId,
      ...
      //获取批次详细信息
      getBatchInfo() {
        post({id:this.batchId,impawnIds:this.listIds}, "/ce/impawn/elec/elecImpawn/eImpawnSignMain/func_getImpawnBatchInfo").then(res => {
          if(res) {
            let msg = res.data.retMsg;
            let retCode = res.data.retCode;
            if (retCode === "000000") {
