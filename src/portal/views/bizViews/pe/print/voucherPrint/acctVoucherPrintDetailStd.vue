...
<template>
  <div>
    ...
    <h-msg-box v-model="printWin" width="1000" class="h-form-search" :maximize="true" @on-close="printWinClose">
      <p slot="header">
        <span>打印记录</span>
      </p>
      <div class="h-form-search-box">
...
<script>
  ...
  export default {
    name: "acctVoucherPrintDetailStd",
    components: {
    ...
    methods: {
      ...
      handleShowBillInfo() {
        ...
        let foreFlowNo = list[0].foreFlowNo;
        window.sessionStorage.setItem('foreFlowNo', foreFlowNo);
        this.params.foreFlowNo = list[0].foreFlowNo;
        this.params.acctDate = list[0].rgstDt;
