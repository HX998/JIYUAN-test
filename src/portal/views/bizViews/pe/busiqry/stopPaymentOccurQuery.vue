...
<template>
  <div class="layout">
          ...
          <div class="h-form-search-box">
            ...
            <h-panel class="clearfix" :class="{'h-form-overhd':!ifShowMore}">
              <h-form :model="formItem" :label-width="90" ref="formItem" cols="4" class="h-form-search">
                <!--                <h-form-item :label="$t('m.i.pe.acptBrch')" prop="acptBrchNameList">
                                  <h-input v-model="formItem.acptBrchNameList" readonly icon="android-search" :title="formItem.acptBrchNameList"
                                           @on-click="queryBrchNo()" clearable @on-clear="clearVal('acptBrchNameList')"></h-input>
                                </h-form-item>-->
                <show-branch v-model="formItem.acptBrchNoList" :label="$t('m.i.pe.acptBrch')" prop="acptBrchNoList" isNeedChecked
    ...
    <!--<show-branch :showBranchWin="showBranchWin" title="机构查询" @brchNoChange="brchNoChange" ifcheck="true"
  ...
  export default {
    ...
    data: function() {
      return {
        ...
        dataUrl: "/pe/busiqry/stopPaymentOccurQuery/func_pageQueryAcptSspdBillList",
        gridData: {},
        sumMoney: "",
        sumCount: "",
        remitDt: [],
            ...
            render: (h, params) => {
              return h("div", [
                h("a", {
                  ...
                  on: {
                    click: () => {
                      this.openImgWin(params.row.id, "IM10");
                    }
                  }
                }, "查看")
              ]);
        ...
        imageParams: {
          ...
          imgType: "IM09",
          //查看
          viewUrl: window.LOCAL_CONFIG.API_HOME + "/ce/acpt/paper/stopPayment/stopPaymentRegisterApply/func_imageById"
        },
        //清单导出
        selectExportType: "",
    ...
    mounted() {
      this.getDictListByGroups("DraftTypeCode,AcptStatus,SspdStatus,SspdType,AnlgSspdType,CertificateType,CDMedia").then(res => {
    ...
    methods: {
      ...
      /* //查询机构弹出框
       ...
       brchNoChange(info) {
         ...
         for (let i = 0; i < info.length; i++) {
           acptBrchNoList.push(info[i].id);
           acptBrchNameList.push(info[i].title);
         }
         this.formItem.acptBrchNoList = acptBrchNoList.join(",");
         this.formItem.acptBrchNameList = acptBrchNameList.join(",");
      ...
      getStopPaymentDataList() {
        ...
        post(params, this.dataUrl).then(res => {
          if (res) {
      ...
      queryBill() {
        ...
        this.$refs.datagrid.selects = [];
        this.$refs.datagrid.selectIds = [];
        this.dataUrl = "/pe/busiqry/stopPaymentOccurQuery/func_pageQueryAcptSspdBillList";
        this.$nextTick(() => {
          this.columns = this.deepClone(this.billColumns);
          this.$refs.datagrid.createShowCol(this.billColumns);
          this.$refs.datagrid.dataChange(1);
      ...
      queryDetail(acptId) {
        ...
        this.$refs.datagrid.selects = [];
        this.$refs.datagrid.selectIds = [];
        this.dataUrl = "/pe/busiqry/stopPaymentOccurQuery/func_pageQueryAcptSspdRegList";
        this.$nextTick(() => {
      ...
      excelExport(selectExportType) {
        ...
        if (selectExportType === "bill") {
          this.exportParam = "acptSspdBillOccurExportInfo";
        } else {
          this.exportParam = "acptSspdRegOccurExportInfo";
        }
      ...
      exportList(field, exportType) {
        ...
        if (this.selectExportType === "bill") {
          exportUrl = window.LOCAL_CONFIG.API_HOME + "/pe/busiqry/stopPaymentOccurQuery/func_exportAcptSspdBillExcel";
        } else {
          exportUrl = window.LOCAL_CONFIG.API_HOME + "/pe/busiqry/stopPaymentOccurQuery/func_exportAcptSspdRegExcel";
        }
        let options = {
          params: {}
        };
