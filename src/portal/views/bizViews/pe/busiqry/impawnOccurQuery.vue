<template>
  <div class="layout">
            ...
            <h-panel class="clearfix" :class="{'h-form-overhd':!ifShowMore}">
              <h-form :model="formItem" :label-width="90" ref="formItem" cols="4" class="h-form-search">
                <show-branch v-model="formItem.transBrchNoList" :label="$t('m.i.common.brchName')" prop="transBrchNoList"
                             :showCheckBox="true" :brchNo.sync="formItem.transBrchNoList" isNeedChecked :brchName.sync="formItem.transBrchNameList"></show-branch>
                ...
                <show-cpes-branch v-model="formItem.plegdorBrchCode" :label="$t('m.i.pe.plegdorBrchName')" v-if="isShowBrch"
                ...
                <show-cpes-branch v-model="formItem.pledgeeBrchCode" :label="$t('m.i.pe.pledgeeBrchName')" v-if="!isShowBrch"
                ...
                <h-form-item :label="$t('m.i.pe.impawnDt')" prop="impawnDt">
                  <h-date-picker :value="impawnDt" type="daterange" autoPlacement showFormat :editable=false
    ...
    <!--点击新增/修改弹出窗-->
    <h-msg-box v-model="impawnTrackQueryWin" width="1000" height="400" class="h-form-table-layer"
               @on-close="impawnTrackQueryWinClose" :maximize="true" @on-maximize="onMaximize" :mask-closable="false"
               :footerHide="true">
      <p slot="header">
      ...
      <h-panel>
        <h-form :model="viewForm" :label-width="120" ref="viewForm" cols="2" class="h-form-search">
          ...
          <h-form-item :label="$t('m.i.pe.plegdorName')" prop="brchName">
            <h-input v-model="viewForm.brchName" :maxlength=60 readonly></h-input>
          </h-form-item>
          <h-form-item :label="$t('m.i.pe.plegdorBrchCode')" prop="applBrchCode">
            <h-input v-model="viewForm.applBrchCode" :maxlength=60 readonly></h-input>
          </h-form-item>
          <h-form-item :label="$t('m.i.pe.pledgeeName')" prop="custName">
            <h-input v-model="viewForm.custName" :maxlength=60 readonly></h-input>
          </h-form-item>
          <h-form-item :label="$t('m.i.pe.pledgeeBrchCode')" prop="custBrchCode">
      ...
      <h-datagrid
        ...
        highlight-row
        :bindForm="formItem"
        :onSelectChange="handleSelectClickBill"
        :gridData="billList"
        :row-select="true"
        :auto-load="false"
  ...
  export default {
    ...
    data() {
      let batchColumns = [
        ...
        },
        {
          ...
          hiddenCol: false,
          render: (h, params) => {
            let impawnDt = params.row.impawnDt ?this.$moment(params.row.impawnDt, "YYYYMMDD").format("YYYY-MM-DD"):"";
      ...
      return {
        ...
        impawnTrackQueryWin: false,
        readonly: false,
        batchId: "",
        tempShowExcelTemplateWin: false,
        queryUrl: "/pb/trust/impawn/impawnOccurQuery/func_findOccurImpawnBatch",
        ...
        impawnBill: {
          billColumns: [
            ...
            {
              title: this.$t("m.i.pe.impawnRemark"),
              key: "respRemark",
              hiddenCol: false
            }
          ]
    ...
    methods: {
      ...
      formSearchReset() {
        ...
        this.formItem.minImpawnDt = "";
        this.formItem.maxImpawnDt = "";
      ...
      //批次明细当前选择明细记录
      handleSelectClickBill(arr, selectInx) {
        this.currentSelectBillList = arr;
        this.currentSelectBill = selectInx;
      ...
      handleImpawnApplDtChange(arr) {
        if(arr && arr.length == 2){
          this.formItem.minImpawnDt = arr[0].replace(/-/g, "");
          this.formItem.maxImpawnDt = arr[1].replace(/-/g, "");
          this.impawnDt = [arr[0], arr[1]];
        }else{
          this.formItem.minImpawnDt = "";
          this.formItem.maxImpawnDt = "";
          this.impawnDt = [];
      ...
      queryImpawnOccurBatch() {
        ...
        this.queryUrl = "/pb/trust/impawn/impawnOccurQuery/func_findOccurImpawnBatch";
      ...
      queryImpawnOccurBill() {
        this.$refs.datagrid.url = null;
        this.queryUrl = "/pb/trust/impawn/impawnOccurQuery/func_findOccurImpawnBill";
      ...
      showBillList(list) {
        ...
        this.readonly = true;
        this.impawnTrackQueryWin = true;
      },
      impawnTrackQueryWinClose() {
        this.impawnTrackQueryWin = false;
      },
      // 查询票据明细
      queryBillByBatchId(id) {
        ...
        post(obj, "/pb/trust/impawn/impawnOccurQuery/func_queryImpawnBatchBill").then(res => {
          if (res) {
            let retCode = res.data.retCode;
            if (retCode === "000000") {
              this.billList = res.data.retData;
      ...
      tempShowExcelTemplateWinOpen(type) {
        ...
        if (type === "batch") {
          this.param = "impawnBatchOccurExportInfo";
        } else {
          this.param = "impawnBillOccurExportInfo";
        }
      ...
      exportList(field, exportType) {
        ...
        if (this.selectExportType === "batch") {
          exportUrl = window.LOCAL_CONFIG.API_HOME + "/pb/trust/impawn/impawnOccurQuery/func_exportImpawnBatchOccurExcel";
        } else {
          exportUrl = window.LOCAL_CONFIG.API_HOME + "/pb/trust/impawn/impawnOccurQuery/func_exportImpawnBillOccurExcel";
        }
        let options = {
          params: {}
        };
        if (this.$refs.datagrid.selectIds.length !== 0) {
          if (this.selectExportType === "batch") {
            ...
            options.params.infoIds = this.$refs.datagrid.selectIds;
          }
