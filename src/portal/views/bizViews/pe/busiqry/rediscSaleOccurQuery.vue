<template>
  <div class="layout">
        ...
        <div>
          ...
          <div class="h-form-search-box">
            <h-panel class="clearfix" :class="{'h-form-overhd':!ifShowMore}">
              <h-form :model="formItem" :label-width="90" ref="formItem" cols="4" class="h-form-search">
                <show-branch v-model="formItem.transBrchNos" :label="$t('m.i.pe.ownBrchName')" prop="transBrchNos"
                             isNeedChecked
                ...
                <common-type-field-range v-model="formItem" :label="$t('m.i.pe.dealRate')" :integerNum="3"
                                         :suffixNum="4" :validRules="ratePctRule" :bigTips="false"
                                         :rangeProps="['minRatePct','maxRatePct']"></common-type-field-range>
                <h-form-item :label="$t('m.i.common.batchNo')" prop="rediscountApplNo">
                  <h-input v-model="formItem.batchNo" placeholder=""></h-input>
                </h-form-item>
                <h-form-item :label="$t('m.i.pe.rediscountApplNo')" prop="rediscountApplNo">
                ...
                <h-form-item :label="$t('m.i.be.assetIndustry')" v-if="openI9flag" prop="assetIndustry">
              ...
              <h-form :label-width="90" class="h-form-search" cols="4">
                <div class="h-search-form-row">
                  ...
                  <h-form-item :label="$t('m.i.ce.totalPayInterest')"  class="h-form-long-label">
                    <h-input v-model="sumFirstPayInterest" placeholder="" readonly></h-input>
                  </h-form-item>
                  <h-form-item :label="$t('m.i.be.totalsettleAmt')" class="h-form-long-label">
                    <h-input v-model="sumSettleAmtFirst" placeholder="" readonly></h-input>
                  </h-form-item>
                  <h-form-item :label="$t('m.i.pe.averageDiscRate')" class="h-form-long-label"
                               v-show="this.ifDisabledExport">
                    <h-input v-model="averageDiscRate" placeholder="" readonly></h-input>
                  </h-form-item>
          ...
          <h-datagrid
            ...
            <div slot="toolbar" class="pull-left">
              <h-button type="primary" @click="queryRediscSaleOccurBatch()">{{$t("m.i.pe.queryAcptOccurBatch")}}
              </h-button>
              <h-button type="primary" @click="queryRediscSaleOccurBill">{{$t("m.i.pe.queryAcptOccurBill")}}</h-button>
    ...
    <redis-bill-detil :batchid="batchid" :dealStatus="dealStatus" :dictMap="dictMap" :showBills="showBills"
                      :batchUrl="batchUrl"
                      :billsUrl="billsUrl"
...
<script>
  import {post, on, off, formatNumber, exportList, accMul} from "@/api/bizApi/commonUtil";

  export default {
    name: "rediscSaleOccurQuery",
    components: {
    ...
    data() {
      let batchColumns = [
        ...
        {
          title: this.$t("m.i.pe.rediscountApplNo"),
          key: "rediscountApplNo",
          hiddenCol: false
        },
        {
          title: this.$t("m.i.pe.applRelId"),
          key: "applRelId",
          hiddenCol: false
        },
        {
          ...
          exportRender(cellData) {
            return this.getDictValueFromMap(this.dictMap, "RediscBusiType", cellData);
          }
        },
        {
        ...
        },
        {
          ...
          hiddenCol: false,
          render: (h, params) => {
            let dictValue = this.getDictValueFromMap(this.dictMap, "RediscApplyStatus", params.row.dealStatus);
            return h("span", {
              domProps: {
                title: dictValue
              }
        ...
        {
          title: this.$t("m.i.pe.buyBrchName"),
          key: "buyBrchName",
          sortable: true,
          hiddenCol: false
        }, {
          title: this.$t("m.i.pe.buyTraderName"),
          key: "buyTraderName",
          sortable: true,
          hiddenCol: false
        },
        {
          title: this.$t("m.i.pe.buyCheckTraderName"),
          key: "buyCheckTraderName",
          sortable: true,
          hiddenCol: false
        }, {
          title: this.$t("m.i.pe.buyApproveTraderName"),
        ...
        },
        {
          ...
          exportRender(cellData) {
            return this.getDictValueFromMap(this.dictMap, "ClearSpeed", cellData);
          }
        },
        {
      ...
      return {
        ...
        billColumns: [
          ...
          {
            title: this.$t("m.i.pe.isDiscKcqy"),
            key: "isDiscKcqy",
            sortable: false,
            hiddenCol: false,
            render: (h, params) => {
              let dictValue = this.getDictValueFromMap(this.dictMap, "Yon", params.row.isDiscKcqy);
              return h("span", {
                domProps: {
                  title: dictValue
                }
          ...
          {
            title: this.$t("m.i.pe.discRatePct"),
            key: "discInterestRate",
            hiddenCol: false,
            render: (h, params) => {
              let discInterestRate = formatNumber(accMul(params.row.discInterestRate, 100), 4, ",");
              return h("span", discInterestRate);
            }
          },
        ],
        rgstDtHisWin: false,
        ...
        queryUrl: "/be/market/rediscount/rediscountTrackQuery/rediscSaleOccurQuery/func_findOccurRediscSaleBatch",
        ...
        sumSettleAmtFirst: "",
        averageDiscRate: "",
        queryRediscSaleOccurUrl: "/be/market/rediscount/rediscountTrackQuery/rediscSaleOccurQuery/func_querySumRediscSaleOccurByBatch"
      };
    },
    mounted() {
      this.buttonAuthShow();
      this.openI9flag = window.sessionStorage.getItem('isOpenI9') == '1' ? true : false;
      this.getDictListByGroups("RediscBusiType,QuoteBatchStatus,RedisBusiTypec,RediscApplyStatus,AssetIndustry,CreditDeductRule,DraftTypeCode,CdSelectMode,CDMedia,QuoteBusiType,AcctFlag,QuoteStatus,SettleType,PreTradeBhvr,Yon,CorpScale,Industry,ClearSpeed,ClearType,StlResult,MmbStlStatus,PreRelationFlag,BillOrigin").then(res => {
        ...
        let list = res.get("AssetIndustry");
        this.busiClassifyList = list.slice(0);
        this.busiClassifyList=this.busiClassifyList.filter(item => item.key != "0");
        console.log(this.busiClassifyList)
        console.log(list)
      });
    },
    watch: {
    ...
    methods: {
      ...
      getSumMoneyAndTotal(params) {
        ...
        if (statDatas && statDatas.indexOf(",") !== -1) {
          ...
          if (statDataArry[1]) {
            this.sumFirstPayInterest = formatNumber(statDataArry[1], 2, ",");
          } else {
            this.sumFirstPayInterest = "0.00";
          }
          if (statDataArry[2]) {
            this.sumSettleAmtFirst = formatNumber(statDataArry[2], 2, ",");
          } else {
            this.sumSettleAmtFirst = "0.00";
          }
          if (statDataArry[3]) {
            this.averageDiscRate = formatNumber(statDataArry[3], 4, ",");
          } else {
            this.averageDiscRate = "0.0000"
          }
        } else {
          this.sumFirstPayInterest = "0.00";
          this.sumSettleAmtFirst = "0.00";
      ...
      getOccurData() {
        post(this.formItem, this.queryRediscSaleOccurUrl).then(res => {
            if (res) {
              ...
              if (retCode === "000000") {
                ...
                if (data.sumFirstPayInterest) {
                  this.sumFirstPayInterest = formatNumber(data.sumFirstPayInterest, 2, ",");
                } else {
                  this.sumFirstPayInterest = "0.00"
                }
                if (data.sumFirstSettleAmt) {
                  this.sumSettleAmtFirst = formatNumber(data.sumFirstSettleAmt, 2, ",");
                } else {
                  this.sumSettleAmtFirst = "0.00"
                }
                if (data.averageDiscRate) {
                  this.averageDiscRate = formatNumber(accMul(data.averageDiscRate, 100), 4, ",");
                } else {
                  this.averageDiscRate = "0.0000"
                }
              } else {
      ...
      queryRediscSaleOccurBatch() {
        this.queryUrl = "/be/market/rediscount/rediscountTrackQuery/rediscSaleOccurQuery/func_findOccurRediscSaleBatch";
        this.queryRediscSaleOccurUrl = "/be/market/rediscount/rediscountTrackQuery/rediscSaleOccurQuery/func_querySumRediscSaleOccurByBatch"
      ...
      queryRediscSaleOccurBill() {
        this.queryUrl = "/be/market/rediscount/rediscountTrackQuery/rediscSaleOccurQuery/func_findOccurRediscSaleBill";
        this.queryRediscSaleOccurUrl = "/be/market/rediscount/rediscountTrackQuery/rediscSaleOccurQuery/func_querySumRediscSaleOccurByBill"
      ...
      tempShowExcelTemplateWinOpen(type) {
        ...
        if (type === "batch") {
          this.param = "rediscSaleBatchOccurExportInfo";
        } else {
          this.param = "rediscSaleBillOccurExportInfo";
        }
      ...
      exportList(field, exportType) {
        ...
        if (this.selectExportType === "batch") {
          exportUrl = window.LOCAL_CONFIG.API_HOME + "/be/market/rediscount/rediscountTrackQuery/rediscSaleOccurQuery/func_exportRediscSaleBatchOccurExcel";
        } else {
          exportUrl = window.LOCAL_CONFIG.API_HOME + "/be/market/rediscount/rediscountTrackQuery/rediscSaleOccurQuery/func_exportRediscSaleBillOccurExcel";
        }
        let options = {};
        if (this.$refs.datagrid.selectIds.length !== 0) {
          if (this.selectExportType === "batch") {
