...
<template>
  <div class="layout">
      ...
      <h-col span="24">
        <div>
          ...
          <h-datagrid
            :columns="columns"
            highlightRow
            url="/bs/corpstdbill/underwrite/stdUnderwriteResultReg/func_queryStdUnderwriteResultRegByPage"
            :bindForm="formItem"
            :onCurrentChange="handleCurrentChange"
            :onCurrentChangeCancel="handleCurrentChangeCancel"
            ref="datagrid">
            <div slot="toolbar" class="pull-left">
              <h-button type="primary" @click="stdUnderwriteResultReg()" :disabled="hasCommit">{{$t("m.i.bs.register")}}</h-button>
              <h-button type="primary" @click="stdUnderwriteResultCancel()" :disabled="noCommit">{{$t("m.i.bs.cancel")}}</h-button>
            </div>
          </h-datagrid>
        </div>
      </h-col>
    ...
    <!-- 新增修改弹窗 -->
    <std-underwrite-result-detail :stdUnderwriteResultDetailWin="stdUnderwriteResultDetailWin" :batchParams="batchParams"
                      @stdUnderwriteResultDetailWinClose="stdUnderwriteResultDetailWinClose" ref="stdUnderwriteResultDetail"></std-underwrite-result-detail>
    <!--标票产品名称-->
...
<script>
  ...
  export default {
    name: "stdUnderwriteResultReg",
    components: {
      ShowCpesBranch:()=>import(/* webpackChunkName: "shcpe/cpes/branch/showCpesBranch" */`@/views/bizViews/shcpe/cpes/branch/showCpesBranch`),
      StdUnderwriteResultDetail:() =>import(/* webpackChunkName: "bs/corpstdbill/underwrite/stdUnderwriteResultDetail" */`@/views/bizViews/bs/corpstdbill/underwrite/stdUnderwriteResultDetail`),
    },
    data() {
      return {
        creationAmtRule:[{
            ...
            render: (h, params) => {
              return h("div", [
                h("a", {
                  on: {
                    click: () => {
                      this.openStdUnderwriteResultDetailWin(params.row.id, params.row.flowStatus);
                    }
                  }
                }, params.row.batchNo)
              ]);
        ...
        transPlaceList:[],
        purchaseResultList:[],
        stdUnderwriteResultDetailWin:false,
        batchParams: {},

      };
    },
    mounted() {
      this.getDictListByGroups("BaseAssetType,TransPlace,PurchaseResult").then(res => {
    ...
    methods: {
      ...
      //打开认购结果登记详情页面
      stdUnderwriteResultReg(){
        if (this.currentSelectRow.length === 0) {
          this.$msgTip.info(this, { info: this.$t("m.i.common.chooseDataFirst") });
          return;
        }
        this.batchParams.batchId = this.currentSelectRow.id;
        this.batchParams.flowStatus = this.currentSelectRow.flowStatus;
        this.assembleParams();
      },
      //关闭认购结果登记详情弹窗
      stdUnderwriteResultDetailWinClose() {
        this.stdUnderwriteResultDetailWin = false;
        let pageNo = this.$refs.datagrid.pageInfo.pageNo;
        this.handleSearch(pageNo);
        this.batchParams = {};
      },
      //认购结果撤销
      stdUnderwriteResultCancel(){
        if (this.currentSelectRow != null && this.currentSelectRow.length !== 0) {
          this.$hMsgBox.confirm({
            title: "撤销信息",
            content: "确定要撤销登记吗？",
            onOk: () => {
              let params={ ids: this.currentSelectRow.id };
              let url="/bs/corpstdbill/underwrite/stdUnderwriteResultReg/func_cancelStdUnderwriteResult";
      ...
      //点击批次编号超链接,认购结果登记详情弹窗
      openStdUnderwriteResultDetailWin(batchId,flowStatus) {
        //流程状态为待认购，打开认购结果登记详情弹窗(包含提交与取消按钮)，否则隐藏这两个按钮
        this.batchParams.batchId = batchId;
        this.batchParams.flowStatus = flowStatus;
        this.assembleParams();
      ...
      assembleParams() {
        ...
        this.stdUnderwriteResultDetailWin = true;
      }
    }
  };
</script>
