      ...
      <h-col span="24">
        <div>
          ...
          <h-datagrid
            :columns="columns"
            highlightRow
            url="/bs/corpstdbill/creation/stdCreationReply/func_queryStdCreationReply"
            :bindForm="formItem"
            :onCurrentChange="handleCurrentChange"
            :onCurrentChangeCancel="handleCurrentChangeCancel"
            ref="datagrid">
            <div slot="toolbar" class="pull-left">
              <h-button type="primary" @click="stdCreationConfirm()">{{$t("m.i.common.confirm")}}</h-button>
              <h-button type="primary" @click="stdCreationReturnBack()">{{$t("m.i.be.returnBack")}}</h-button>
            </div>
          </h-datagrid>
        </div>
      </h-col>
...
<script>
  ...
  export default {
    ...
    mounted() {
      this.getDictListByGroups("BaseAssetType,Yon,DraftTypeCode,CDMedia,TransPlace").then(res => {
    ...
    methods: {
        ...
        if (this.currentSelectRow != null && this.currentSelectRow.length !== 0) {
          this.$hMsgBox.confirm({
            ...
            onOk: () => {
              ...
              let url="/bs/corpstdbill/creation/stdCreationReply/func_agreeStdCreation";
      ...
      stdCreationReturnBack(){
        if (this.currentSelectRow != null && this.currentSelectRow.length !== 0) {
          this.$hMsgBox.confirm({
            ...
            onOk: () => {
              let params={ id: this.currentSelectRow.id };
              let url="/bs/corpstdbill/creation/stdCreationReply/func_refuseStdCreation";
              this.sendPost(params,url);
            }
          });
        } else {
      ...
      stdCreationDetailOpen(id){
        this.batchId = id;
        this.batchParams.batchUrl="/bs/corpstdbill/creation/stdCreationReply/func_jumpEditStdCreation";
        this.batchParams.dataGridUrl="";
        this.batchParams.creditMainUrl = "/bs/corpstdbill/creation/stdCreationReply/func_queryStdCreditMainPage";
        this.batchParams.orgHolderUrl = "/bs/corpstdbill/creation/stdCreationReply/func_queryStdCorpOrgHolderPage";
