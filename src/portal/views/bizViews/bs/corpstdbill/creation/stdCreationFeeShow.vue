...
<template>
  <div>
    <h-msg-box v-model="tempStdCreationFeeShowWin" :maximize="true" :mask-closable="false" @on-close="handleClose"
      ...
      <div>
        ...
        <h-datagrid
          ...
          highlight-row
          :auto-load="false"
          url="/bs/corpstdbill/creation/stdCreationFeeAccount/func_queryStdCreationBill"
          :bindForm="dataGridFormItem"
          :has-select="false"
          ref="datagrid">
          <div slot="toolbar" class="pull-left">
            <h-button type="primary" @click="feeCount()" >{{$t('m.i.bs.feeCount')}}</h-button>
            <h-button type="primary" @click="feeAccount()" >{{$t('m.i.bs.feeAcct')}}</h-button>
  ...
  export default {
    ...
    data() {
      return {
        ...
        feeDetailParams:{},
        param : {},
        acctRecord : "/bs/corpstdbill/creation/stdCreationFeeAccount/func_queryStdCreationFeeRecord",
        accountUrl : "/bs/corpstdbill/creation/stdCreationFeeAccount/func_stdCreationFeeAccount",
    ...
    computed: {
      tempStdCreationFeeShowWin: {
        get() {
          return this.stdCreationFeeShowWin;
        },
        set() {
        }
      }
    ...
    methods: {
      ...
      queryObjById() {
        ...
        let url = "/bs/corpstdbill/creation/stdCreationFeeAccount/func_jumpStdCreationFee";
      ...
      feeAccount() {
        if (this.feeAccountForm.interestStatus != 1) {
          this.$msgTip.error(this, { info: this.$t("m.i.bs.countFeeFirst") });
          return;
        }
        this.$hMsgBox.confirm({
          title: this.$t("m.i.common.confirm"),
          content: this.$t("确认要收费记账吗") + "?",
          onOk: () => {
...
// 关闭记账窗口
      ...
      handleClose() {
        ...
        this.batchId="";
        this.$emit("stdCreationFeeShowWinClose", "");
      },
