<template>
  <div class="layout">
      ...
      <h-col span="24">
        <div>
          ...
          <h-datagrid
            ...
            @cleanSelectedData="onCurrentChangeCancle"
            ref="datagrid">
    ...
    <h-msg-box v-model="draftInfoWin" width="1000" class="h-form-search" :mask-closable="true" :maximize="true"
               @on-close="closeForm">
      <p slot="header">
        <span>报文详情</span>
      </p>
        ...
        <h-form :model="draftInfoForm" :label-width="115" ref="draftInfoForm" cols="1" class="h-form-search">
          <div class="h-search-form-row">
            <h-form-item :label="$t('m.i.shcpe.draftContent')" prop="draftContent" class="h-form-height-auto">
              ...
              <textarea id="ecdsBusiDraftContentId" readonly cols="110" rows="10"
                        style="overflow-y:auto;overflow-x:auto"></textarea>
            </h-form-item>
          </div>
        </h-form>
      </h-panel>
      ...
      return {
        ...
        columns: [
          ...
          },
          {
            ...
            hiddenCol: false,
            render: (h, params) => {
              let draftTm = params.row.draftTm;
              let len = draftTm.toString().length;
              if (len < 6) {
                for (let i = 0; i < 6 - len; i++) {
                  draftTm = "0" + draftTm;
                }
              }
              draftTm = this.$moment(draftTm, "HH:mm:ss").format("HH:mm:ss");
        ...
        post({ regLogId: this.currentSelectRow.id }, "/ecds/draft/ecdsDraftLog/func_getDraftLogContent").then(res => {
          if (res) {
            ...
            if (retCode === "000000") {
              ...
              this.$nextTick(() => {
                document.getElementById("ecdsBusiDraftContentId").innerHTML = retData.draftContent;
              });
            } else {
