...
<template>
  <div class="layout">
        ...
        <div>
              ...
              <h-form :model="formItem" :label-width="90" ref="formItem" cols="4" class="h-form-search">
                ...
                <h-form-item :label="$t('m.i.common.bankType')" prop="bankType">
                  <h-select v-model="formItem.bankType" placeholder="">
                    <h-option v-for="item in bankTypeList" :value="item.key" :key="item.key">{{ item.value }}
                    </h-option>
                  </h-select>
                </h-form-item>
          ...
          <h-datagrid
            ...
            <div slot="toolbar" class="pull-left">
              ...
              <h-button type="primary" @click="handleBankLevelForm()">{{$t("m.i.shcpe.mainTainBankLevel")}}</h-button>
              <h-button type="primary" @click="handleBankTypeForm()">{{$t("m.i.shcpe.mainTainBankType")}}</h-button>
              <h-button type="primary" @click="handleBankNameForm()">{{$t("m.i.shcpe.mainTainBankName")}}</h-button>
      ...
      <h-form :model="editForm" :label-width="130" ref="editForm" cols="2" class="h-form-search">
        ...
        <div class="h-search-form-row">
          ...
          <h-form-item :label="$t('m.i.common.bankLevel')" prop="bankLevel" :required="isRequired">
            <h-select v-model="editForm.bankLevel" placeholder="">
        ...
        <div class="h-search-form-row">
          <h-form-item :label="$t('m.i.shcpe.reserve1')" prop="reserve1">
            <h-input v-model="editForm.reserve1"></h-input>
          </h-form-item>
        </div>
      </h-form>
      <div slot="footer">
    ...
    <!--批量维护银行级别弹出窗-->
    <h-msg-box v-model="bankLevelWin" width="400" :mask-closable="false" @on-close="bankLevelWinClose"
               :maximize="true">
      <p slot="header">
        <span>批量维护银行级别</span>
      </p>
      <h-form :model="editForm" :label-width="80" ref="bankLevelForm" cols="1" class="h-form-search">
        <div class="h-search-form-row">
          <h-form-item :label="$t('m.i.common.bankLevel')" prop="bankLevel" :required="isRequired">
            <h-select v-model="editForm.bankLevel" placeholder="">
              <h-option v-for="item in bankLevelList" :value="item.key" :key="item.key">{{ item.value }}</h-option>
      ...
      <div slot="footer">
        <h-button type="ghost" @click="bankLevelWinClose">{{$t("m.i.common.close")}}</h-button>
        <h-button type="primary" v-if="submitBankLevelFlag" disabled>{{$t("m.i.common.submiting")}}</h-button>
        <h-button type="primary" v-else @click="bankLevelSubmitForm">{{$t("m.i.common.commit")}}</h-button>

      </div>
    </h-msg-box>
    <!--批量维护银行类型弹出窗-->
    <h-msg-box v-model="bankTypeWin" width="400" :mask-closable="false" @on-close="bankTypeWinClose"
               :maximize="true">
      <p slot="header">
        <span>批量维护银行类型</span>
      </p>
      <h-form :model="editForm" :label-width="80" ref="bankTypeForm" cols="1" class="h-form-search">
        <div class="h-search-form-row">
          <h-form-item :label="$t('m.i.common.bankType')" prop="bankType" :required="isRequired">
            <h-select v-model="editForm.bankType" placeholder="">
      ...
      <div slot="footer">
        <h-button type="ghost" @click="bankTypeWinClose">{{$t("m.i.common.close")}}</h-button>
        <h-button type="primary" v-if="submitBankTypeFlag" disabled>{{$t("m.i.common.submiting")}}</h-button>
        <h-button type="primary" v-else @click="bankTypeSubmitForm">{{$t("m.i.common.commit")}}</h-button>
      </div>
    </h-msg-box>

    <!--维护银行简称弹出窗-->
    <h-msg-box v-model="bankNameWin" width="400" :mask-closable="false" @on-close="bankNameWinClose"
               :maximize="true">
      <p slot="header">
        <span>维护银行简称</span>
      </p>
      <h-form :model="editForm" :label-width="80" ref="bankNameForm" cols="1" class="h-form-search">
        <div class="h-search-form-row">
          <h-form-item :label="$t('m.i.shcpe.reserve1')" prop="reserve1" :required="isRequired">
            <h-input v-model="editForm.reserve1"></h-input>
          </h-form-item>
        </div>
      </h-form>
      <div slot="footer">
        <h-button type="ghost" @click="bankNameWinClose">{{$t("m.i.common.close")}}</h-button>
        <h-button type="primary" v-if="submitBankNameFlag" disabled>{{$t("m.i.common.submiting")}}</h-button>
        <h-button type="primary" v-else @click="bankNameSubmitForm">{{$t("m.i.common.commit")}}</h-button>
      </div>
    </h-msg-box>
  ...
  export default {
    ...
    data() {
      return {
        ...
        columns: [
          ...
          {
            title: this.$t("m.i.shcpe.reserve1"),
            key: "reserve1",
            hiddenCol: false
          },
          {
        ...
        ifShowMore: false,
        submitFlag: false,
        submitBankLevelFlag: false,
        submitBankTypeFlag: false,
        submitBankNameFlag: false,
        bankLevelList: [],
        bankTypeList: [],
        id: null,
        param: null,
        ...
        importWin: false,
        fileParams: {
          fileUploadUrl: window.LOCAL_CONFIG.API_HOME + "/shcpe/cust/bankinfo/func_batchAdd",  //excel数据导入接口
    ...
    methods: {
      ...
      handleEditForm() {
        ...
        this.editForm.bankLevelName = this.$refs.datagrid.selects[0].bankLevelName;
        ...
        this.editForm.reserve1 = this.$refs.datagrid.selects[0].reserve1;
        this.editWin = true;
      },
      handleBankLevelForm() {
      ...
      handleBankNameForm() {
        ...
        if (list.length === 1) {
          ...
          this.editForm.reserve1 = "";
        }

        this.editForm.memberIdStr = memberIdStr;
        this.bankNameWin = true;
      },
      submitForm() {
      ...
      bankLevelSubmitForm() {
        this.$refs["bankLevelForm"].validate(valid => {
          if (valid) {
            this.submitBankLevelFlag = true;
            let url = "/shcpe/cust/bankinfo/updateBankLevelBatch";
            let msg = this.$t("m.i.common.modifySuccess");
            post(this.editForm, url).then(res => {
              this.submitBankLevelFlag = false;
              if (res) {
                let retCode = res.data.retCode;
                if (retCode == "000000") {
                  this.$msgTip.success(this, { info: msg });
                  this.bankLevelWin = false;
                  this.$refs.datagrid.dataChange(1);
                  this.$refs.datagrid.selects = [];
                  this.$refs.datagrid.selectIds = [];
                  this.$refs.bankLevelForm.resetFields();
                } else {
                  let msg = this.$t("m.i.common.modifyFailed");
                  this.$msgTip.error(this, { info: msg + ":" + res.data.retMsg });
                }
      ...
      bankTypeSubmitForm() {
        this.$refs["bankTypeForm"].validate(valid => {
          if (valid) {
            this.submitBankTypeFlag = true;
            let url = "/shcpe/cust/bankinfo/updateBankTypeBatch";
            let msg = this.$t("m.i.common.modifySuccess");
            post(this.editForm, url).then(res => {
              this.submitBankTypeFlag = false;
              if (res) {
                let retCode = res.data.retCode;
                if (retCode == "000000") {
                  this.$msgTip.success(this, { info: msg });
      ...
      bankNameSubmitForm() {
        this.$refs["bankNameForm"].validate(valid => {
          if (valid) {
            this.submitBankNameFlag = true;
            let url = "/shcpe/cust/bankinfo/updateBankName";
            let msg = this.$t("m.i.common.modifySuccess");
            post(this.editForm, url).then(res => {
              this.submitBankNameFlag = false;
              if (res) {
                let retCode = res.data.retCode;
                if (retCode == "000000") {
                  this.$msgTip.success(this, { info: msg });
                  this.bankNameWin = false;
                  this.$refs.datagrid.dataChange(1);
                  this.$refs.datagrid.selects = [];
                  this.$refs.datagrid.selectIds = [];
                  this.$refs.bankNameForm.resetFields();
                } else {
                  let msg = this.$t("m.i.common.modifyFailed");
                  this.$msgTip.error(this, { info: msg + ":" + res.data.retMsg });
                }
      ...
      tempShowExcelTemplateWinOpen() {
        this.param = "custTopBankInfoListExport";
