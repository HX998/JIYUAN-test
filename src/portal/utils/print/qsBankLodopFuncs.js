 ...
 * @returns {*}
 */
function  printListsHtm(_this, headHtml, bodyHtml, options, footerHtml, printStyles) {
  let LODOP = getLodop(null, null, _this);
  if (LODOP) {
    LODOP.PRINT_INIT("");
    LODOP.SET_PRINT_PAGESIZE(2, 0, 0, "");

    for (let j = 0; j < headHtml.length; j++) {
      LODOP.ADD_PRINT_TABLE(options[j].bodyTop || "27%", options[j].bodyLeft || "1%", options[j].bodyWidth || "99%", options[j].bodyHeight || "75%", printStyles + bodyHtml[j]);// 添加表格打印,能自动实现分页
      LODOP.SET_PRINT_STYLEA(0, "Vorient", 3);
      LODOP.ADD_PRINT_HTM(options[j].headTop || "1%", options[j].headLeft || "6%", options[j].headWidth || "90%", options[j].headHeight || "20%", printStyles + headHtml[j]);
      LODOP.SET_PRINT_STYLEA(0, "ItemType", 1);
      LODOP.SET_PRINT_STYLEA(0, "LinkedItem", options[j].linkedItem);
    ...
    LODOP.SET_SHOW_MODE("LANDSCAPE_DEFROTATED", 1);
    LODOP.SET_PRINT_MODE("AUTO_CLOSE_PREWINDOW", 1);// 打印后自动关闭预览窗口
    var isPrint = LODOP.PREVIEW();// 获取打印次数
 ...
 * @returns {*}
 */
function printListsHtmVertical(_this, headHtml, bodyHtml, options, footerHtml, printStyles = styles) {
  let LODOP = getLodop(null, null, _this);
...
export {getLodop,printHtm,printListHtm,printListsHtm,printListHtmVertical,printListsHtmVertical,printListHtmNoHead,printListHtmNoHeadTwice,printMultiTableHtm,printListHtmNoHeadList,printDataHandler,vochPrint,vouchPrint,detailListPrint,vouchTemplatePrint,vouchTemplatePrintDetail,detailPrint}
