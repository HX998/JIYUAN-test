    ...
    <div v-show="showDialog" class="hfq-dialog-carousel" :ref="id">
      <v-touch @pan="onPan">
        <div ref="hfq-dialog" id="hfq-dialog" class="hfq-dialog">
          <a class="h-modal-close" @click="handlerCloseDialog"><i class="h-icon iconfont icon-close h-icon-close"></i></a>
          ...
          <div class="hfq-dialog-content">
            <div class="hfq-carousel-img-content" v-for="(url,index) in data.imgUrls"
              ...
              <img class="carousel-img animated fadeIn" ref="hfq-carousel-img" :index="index" :src="url"
                   :key="url.substring(url.indexOf('=')+1, url.indexOf('&'))">
            </div>
          </div>
          ...
          <div class="hfq-dialog-bottom">
            ...
            <div class="icons-container">
              <h-button type="primary" class="btn scaleL" @click="scaleL">
                <img src="./img/add.png" title="放大" width="24" height="24">
              </h-button>
              <h-button type="primary" class="btn scaleS" @click="scaleS">
                <img src="./img/min.png" title="缩小" width="24" height="24">
              </h-button>
              <h-button type="primary" @click="pre">
                <img src="./img/pre.png" title="上一张" width="24" height="24">
              </h-button>
              <h-button type="primary" @click="next">
                <img src="./img/next.png" title="下一张" width="24" height="24">
              </h-button>
              <h-button type="primary" class="btn rotateL" @click="rotateL">
                <img src="./img/left-r.png" title="左转" width="24" height="24">
              </h-button>
              <h-button type="primary" class="btn rotateR" @click="rotateR">
...
<script>
  import ImageView from '@/plugins/dialogCarousel/ImageView'
  ...
  export default{
    ...
    watch: {
      ...
      data: {
        handler(val){
          if (!!val.imgUrls) {
            for(let i=0; i < val.imgUrls.length; i++) {
              val.imgUrls[i] = val.imgUrls[i] + "&key=" + Math.floor((Math.random()*100));
            }
          }
        },
        deep: true
    ...
    methods: {
      ...
      initDrag(){
        ...
        let dialogStyle = this.hstore.getItem('dialogStyle');
        if (dialogStyle.left && dialogStyle.left > 0) {
      ...
      onPan(e){
        ...
        if (e.isFinal) {
          ...
          this.hstore.setItem('dialogStyle', this.dialogStyle);
        }
      }
    }
  }
...
<style scoped>
  ...
  .hfq-dialog-carousel .icons-container .h-btn{
    padding: 0;
    border-radius: 50%;
    width: 26px;
    height: 26px;
