...
rm(path.join(config.build.assetsRoot, config.build.assetsSubDirectory), err => {
  ...
  webpack(webpackConfig, (err, stats) => {
    ...
    fse.readFile(path.join(config.build.assetsRoot, 'index.ejs'), 'utf-8', function(err, html) {
      ...
      fse.writeFileSync(path.join(config.build.assetsRoot, 'index.ejs'), content, 'utf-8')
    })
